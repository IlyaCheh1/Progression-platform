...
<!-- SHIP-FLOW:START -->
## Ship flow (канон OnlyGames / GFF)

Порядок для любой задачи по коду (fail-closed; при fail — возврат на нужный ранний шаг):

1. Task
2. Plan + research
3. Plan review
4. **Confirm пользователя** — до этого ни тестов, ни кода
5. Tests **RED** (сначала падающие тесты)
6. Implement **GREEN** (+ refactor)
7. Code review
8. Builds + tests (реальный прогон / CI)
9. Deploy — только после green review+CI **и** явного ок пользователя

### Hard rules (backend/service)
1. Логирование повсеместно (зеркало sibling-сервисов)
2. БД: явные транзакции, без N+1
3. Межсервис только через внутренние ключи
4. Gateway/middleware: auth, rate limit, CORS, internal-key на новых/изменённых роутах
