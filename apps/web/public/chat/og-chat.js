/**
* @vue/shared v3.5.20
* (c) 2018-present Yuxi (Evan) You and Vue contributors
* @license MIT
**/
/*! #__NO_SIDE_EFFECTS__ */
// @__NO_SIDE_EFFECTS__
function hs(e) {
  const t = /* @__PURE__ */ Object.create(null);
  for (const n of e.split(",")) t[n] = 1;
  return (n) => n in t;
}
const ae = {}, Ht = [], Yt = () => {
}, Ki = () => !1, ur = (e) => e.charCodeAt(0) === 111 && e.charCodeAt(1) === 110 && // uppercase letter
(e.charCodeAt(2) > 122 || e.charCodeAt(2) < 97), ds = (e) => e.startsWith("onUpdate:"), Pe = Object.assign, Ni = (e, t) => {
  const n = e.indexOf(t);
  n > -1 && e.splice(n, 1);
}, ja = Object.prototype.hasOwnProperty, re = (e, t) => ja.call(e, t), _ = Array.isArray, zt = (e) => fr(e) === "[object Map]", Li = (e) => fr(e) === "[object Set]", le = (e) => typeof e == "function", be = (e) => typeof e == "string", kt = (e) => typeof e == "symbol", we = (e) => e !== null && typeof e == "object", Wi = (e) => (we(e) || le(e)) && le(e.then) && le(e.catch), Ji = Object.prototype.toString, fr = (e) => Ji.call(e), Pa = (e) => fr(e).slice(8, -1), hr = (e) => fr(e) === "[object Object]", ps = (e) => be(e) && e !== "NaN" && e[0] !== "-" && "" + parseInt(e, 10) === e, hn = /* @__PURE__ */ hs(
  // the leading comma is intentional so empty string "" is also included
  ",key,ref,ref_for,ref_key,onVnodeBeforeMount,onVnodeMounted,onVnodeBeforeUpdate,onVnodeUpdated,onVnodeBeforeUnmount,onVnodeUnmounted"
), dr = (e) => {
  const t = /* @__PURE__ */ Object.create(null);
  return ((n) => t[n] || (t[n] = e(n)));
}, Ka = /-(\w)/g, He = dr(
  (e) => e.replace(Ka, (t, n) => n ? n.toUpperCase() : "")
), Na = /\B([A-Z])/g, Ge = dr(
  (e) => e.replace(Na, "-$1").toLowerCase()
), qi = dr((e) => e.charAt(0).toUpperCase() + e.slice(1)), Rr = dr(
  (e) => e ? `on${qi(e)}` : ""
), Ct = (e, t) => !Object.is(e, t), Ln = (e, ...t) => {
  for (let n = 0; n < e.length; n++)
    e[n](...t);
}, Zi = (e, t, n, r = !1) => {
  Object.defineProperty(e, t, {
    configurable: !0,
    enumerable: !1,
    writable: r,
    value: n
  });
}, Lr = (e) => {
  const t = parseFloat(e);
  return isNaN(t) ? e : t;
}, qs = (e) => {
  const t = be(e) ? Number(e) : NaN;
  return isNaN(t) ? e : t;
};
let Zs;
const pr = () => Zs || (Zs = typeof globalThis < "u" ? globalThis : typeof self < "u" ? self : typeof window < "u" ? window : typeof global < "u" ? global : {});
function Te(e) {
  if (_(e)) {
    const t = {};
    for (let n = 0; n < e.length; n++) {
      const r = e[n], s = be(r) ? qa(r) : Te(r);
      if (s)
        for (const i in s)
          t[i] = s[i];
    }
    return t;
  } else if (be(e) || we(e))
    return e;
}
const La = /;(?![^(]*\))/g, Wa = /:([^]+)/, Ja = /\/\*[^]*?\*\//g;
function qa(e) {
  const t = {};
  return e.replace(Ja, "").split(La).forEach((n) => {
    if (n) {
      const r = n.split(Wa);
      r.length > 1 && (t[r[0].trim()] = r[1].trim());
    }
  }), t;
}
function me(e) {
  let t = "";
  if (be(e))
    t = e;
  else if (_(e))
    for (let n = 0; n < e.length; n++) {
      const r = me(e[n]);
      r && (t += r + " ");
    }
  else if (we(e))
    for (const n in e)
      e[n] && (t += n + " ");
  return t.trim();
}
const Za = "itemscope,allowfullscreen,formnovalidate,ismap,nomodule,novalidate,readonly", Ha = /* @__PURE__ */ hs(Za);
function Hi(e) {
  return !!e || e === "";
}
const zi = (e) => !!(e && e.__v_isRef === !0), Ae = (e) => be(e) ? e : e == null ? "" : _(e) || we(e) && (e.toString === Ji || !le(e.toString)) ? zi(e) ? Ae(e.value) : JSON.stringify(e, Xi, 2) : String(e), Xi = (e, t) => zi(t) ? Xi(e, t.value) : zt(t) ? {
  [`Map(${t.size})`]: [...t.entries()].reduce(
    (n, [r, s], i) => (n[Fr(r, i) + " =>"] = s, n),
    {}
  )
} : Li(t) ? {
  [`Set(${t.size})`]: [...t.values()].map((n) => Fr(n))
} : kt(t) ? Fr(t) : we(t) && !_(t) && !hr(t) ? String(t) : t, Fr = (e, t = "") => {
  var n;
  return (
    // Symbol.description in es2019+ so we need to cast here to pass
    // the lib: es2016 check
    kt(e) ? `Symbol(${(n = e.description) != null ? n : t})` : e
  );
};
/**
* @vue/reactivity v3.5.20
* (c) 2018-present Yuxi (Evan) You and Vue contributors
* @license MIT
**/
let Se;
class za {
  constructor(t = !1) {
    this.detached = t, this._active = !0, this._on = 0, this.effects = [], this.cleanups = [], this._isPaused = !1, this.parent = Se, !t && Se && (this.index = (Se.scopes || (Se.scopes = [])).push(
      this
    ) - 1);
  }
  get active() {
    return this._active;
  }
  pause() {
    if (this._active) {
      this._isPaused = !0;
      let t, n;
      if (this.scopes)
        for (t = 0, n = this.scopes.length; t < n; t++)
          this.scopes[t].pause();
      for (t = 0, n = this.effects.length; t < n; t++)
        this.effects[t].pause();
    }
  }
  /**
   * Resumes the effect scope, including all child scopes and effects.
   */
  resume() {
    if (this._active && this._isPaused) {
      this._isPaused = !1;
      let t, n;
      if (this.scopes)
        for (t = 0, n = this.scopes.length; t < n; t++)
          this.scopes[t].resume();
      for (t = 0, n = this.effects.length; t < n; t++)
        this.effects[t].resume();
    }
  }
  run(t) {
    if (this._active) {
      const n = Se;
      try {
        return Se = this, t();
      } finally {
        Se = n;
      }
    }
  }
  /**
   * This should only be called on non-detached scopes
   * @internal
   */
  on() {
    ++this._on === 1 && (this.prevScope = Se, Se = this);
  }
  /**
   * This should only be called on non-detached scopes
   * @internal
   */
  off() {
    this._on > 0 && --this._on === 0 && (Se = this.prevScope, this.prevScope = void 0);
  }
  stop(t) {
    if (this._active) {
      this._active = !1;
      let n, r;
      for (n = 0, r = this.effects.length; n < r; n++)
        this.effects[n].stop();
      for (this.effects.length = 0, n = 0, r = this.cleanups.length; n < r; n++)
        this.cleanups[n]();
      if (this.cleanups.length = 0, this.scopes) {
        for (n = 0, r = this.scopes.length; n < r; n++)
          this.scopes[n].stop(!0);
        this.scopes.length = 0;
      }
      if (!this.detached && this.parent && !t) {
        const s = this.parent.scopes.pop();
        s && s !== this && (this.parent.scopes[this.index] = s, s.index = this.index);
      }
      this.parent = void 0;
    }
  }
}
function Xa() {
  return Se;
}
function _i(e, t = !1) {
  Se && Se.cleanups.push(e);
}
let fe;
const Qr = /* @__PURE__ */ new WeakSet();
class $i {
  constructor(t) {
    this.fn = t, this.deps = void 0, this.depsTail = void 0, this.flags = 5, this.next = void 0, this.cleanup = void 0, this.scheduler = void 0, Se && Se.active && Se.effects.push(this);
  }
  pause() {
    this.flags |= 64;
  }
  resume() {
    this.flags & 64 && (this.flags &= -65, Qr.has(this) && (Qr.delete(this), this.trigger()));
  }
  /**
   * @internal
   */
  notify() {
    this.flags & 2 && !(this.flags & 32) || this.flags & 8 || to(this);
  }
  run() {
    if (!(this.flags & 1))
      return this.fn();
    this.flags |= 2, Hs(this), no(this);
    const t = fe, n = ze;
    fe = this, ze = !0;
    try {
      return this.fn();
    } finally {
      ro(this), fe = t, ze = n, this.flags &= -3;
    }
  }
  stop() {
    if (this.flags & 1) {
      for (let t = this.deps; t; t = t.nextDep)
        vs(t);
      this.deps = this.depsTail = void 0, Hs(this), this.onStop && this.onStop(), this.flags &= -2;
    }
  }
  trigger() {
    this.flags & 64 ? Qr.add(this) : this.scheduler ? this.scheduler() : this.runIfDirty();
  }
  /**
   * @internal
   */
  runIfDirty() {
    Wr(this) && this.run();
  }
  get dirty() {
    return Wr(this);
  }
}
let eo = 0, dn, pn;
function to(e, t = !1) {
  if (e.flags |= 8, t) {
    e.next = pn, pn = e;
    return;
  }
  e.next = dn, dn = e;
}
function gs() {
  eo++;
}
function ms() {
  if (--eo > 0)
    return;
  if (pn) {
    let t = pn;
    for (pn = void 0; t; ) {
      const n = t.next;
      t.next = void 0, t.flags &= -9, t = n;
    }
  }
  let e;
  for (; dn; ) {
    let t = dn;
    for (dn = void 0; t; ) {
      const n = t.next;
      if (t.next = void 0, t.flags &= -9, t.flags & 1)
        try {
          t.trigger();
        } catch (r) {
          e || (e = r);
        }
      t = n;
    }
  }
  if (e) throw e;
}
function no(e) {
  for (let t = e.deps; t; t = t.nextDep)
    t.version = -1, t.prevActiveLink = t.dep.activeLink, t.dep.activeLink = t;
}
function ro(e) {
  let t, n = e.depsTail, r = n;
  for (; r; ) {
    const s = r.prevDep;
    r.version === -1 ? (r === n && (n = s), vs(r), _a(r)) : t = r, r.dep.activeLink = r.prevActiveLink, r.prevActiveLink = void 0, r = s;
  }
  e.deps = t, e.depsTail = n;
}
function Wr(e) {
  for (let t = e.deps; t; t = t.nextDep)
    if (t.dep.version !== t.version || t.dep.computed && (so(t.dep.computed) || t.dep.version !== t.version))
      return !0;
  return !!e._dirty;
}
function so(e) {
  if (e.flags & 4 && !(e.flags & 16) || (e.flags &= -17, e.globalVersion === bn) || (e.globalVersion = bn, !e.isSSR && e.flags & 128 && (!e.deps && !e._dirty || !Wr(e))))
    return;
  e.flags |= 2;
  const t = e.dep, n = fe, r = ze;
  fe = e, ze = !0;
  try {
    no(e);
    const s = e.fn(e._value);
    (t.version === 0 || Ct(s, e._value)) && (e.flags |= 128, e._value = s, t.version++);
  } catch (s) {
    throw t.version++, s;
  } finally {
    fe = n, ze = r, ro(e), e.flags &= -3;
  }
}
function vs(e, t = !1) {
  const { dep: n, prevSub: r, nextSub: s } = e;
  if (r && (r.nextSub = s, e.prevSub = void 0), s && (s.prevSub = r, e.nextSub = void 0), n.subs === e && (n.subs = r, !r && n.computed)) {
    n.computed.flags &= -5;
    for (let i = n.computed.deps; i; i = i.nextDep)
      vs(i, !0);
  }
  !t && !--n.sc && n.map && n.map.delete(n.key);
}
function _a(e) {
  const { prevDep: t, nextDep: n } = e;
  t && (t.nextDep = n, e.prevDep = void 0), n && (n.prevDep = t, e.nextDep = void 0);
}
let ze = !0;
const io = [];
function St() {
  io.push(ze), ze = !1;
}
function It() {
  const e = io.pop();
  ze = e === void 0 ? !0 : e;
}
function Hs(e) {
  const { cleanup: t } = e;
  if (e.cleanup = void 0, t) {
    const n = fe;
    fe = void 0;
    try {
      t();
    } finally {
      fe = n;
    }
  }
}
let bn = 0;
class $a {
  constructor(t, n) {
    this.sub = t, this.dep = n, this.version = n.version, this.nextDep = this.prevDep = this.nextSub = this.prevSub = this.prevActiveLink = void 0;
  }
}
class ws {
  // TODO isolatedDeclarations "__v_skip"
  constructor(t) {
    this.computed = t, this.version = 0, this.activeLink = void 0, this.subs = void 0, this.map = void 0, this.key = void 0, this.sc = 0, this.__v_skip = !0;
  }
  track(t) {
    if (!fe || !ze || fe === this.computed)
      return;
    let n = this.activeLink;
    if (n === void 0 || n.sub !== fe)
      n = this.activeLink = new $a(fe, this), fe.deps ? (n.prevDep = fe.depsTail, fe.depsTail.nextDep = n, fe.depsTail = n) : fe.deps = fe.depsTail = n, oo(n);
    else if (n.version === -1 && (n.version = this.version, n.nextDep)) {
      const r = n.nextDep;
      r.prevDep = n.prevDep, n.prevDep && (n.prevDep.nextDep = r), n.prevDep = fe.depsTail, n.nextDep = void 0, fe.depsTail.nextDep = n, fe.depsTail = n, fe.deps === n && (fe.deps = r);
    }
    return n;
  }
  trigger(t) {
    this.version++, bn++, this.notify(t);
  }
  notify(t) {
    gs();
    try {
      for (let n = this.subs; n; n = n.prevSub)
        n.sub.notify() && n.sub.dep.notify();
    } finally {
      ms();
    }
  }
}
function oo(e) {
  if (e.dep.sc++, e.sub.flags & 4) {
    const t = e.dep.computed;
    if (t && !e.dep.subs) {
      t.flags |= 20;
      for (let r = t.deps; r; r = r.nextDep)
        oo(r);
    }
    const n = e.dep.subs;
    n !== e && (e.prevSub = n, n && (n.nextSub = e)), e.dep.subs = e;
  }
}
const Xn = /* @__PURE__ */ new WeakMap(), Tt = Symbol(
  ""
), Jr = Symbol(
  ""
), An = Symbol(
  ""
);
function Ie(e, t, n) {
  if (ze && fe) {
    let r = Xn.get(e);
    r || Xn.set(e, r = /* @__PURE__ */ new Map());
    let s = r.get(n);
    s || (r.set(n, s = new ws()), s.map = r, s.key = n), s.track();
  }
}
function ft(e, t, n, r, s, i) {
  const o = Xn.get(e);
  if (!o) {
    bn++;
    return;
  }
  const a = (l) => {
    l && l.trigger();
  };
  if (gs(), t === "clear")
    o.forEach(a);
  else {
    const l = _(e), c = l && ps(n);
    if (l && n === "length") {
      const u = Number(r);
      o.forEach((h, g) => {
        (g === "length" || g === An || !kt(g) && g >= u) && a(h);
      });
    } else
      switch ((n !== void 0 || o.has(void 0)) && a(o.get(n)), c && a(o.get(An)), t) {
        case "add":
          l ? c && a(o.get("length")) : (a(o.get(Tt)), zt(e) && a(o.get(Jr)));
          break;
        case "delete":
          l || (a(o.get(Tt)), zt(e) && a(o.get(Jr)));
          break;
        case "set":
          zt(e) && a(o.get(Tt));
          break;
      }
  }
  ms();
}
function el(e, t) {
  const n = Xn.get(e);
  return n && n.get(t);
}
function Wt(e) {
  const t = ne(e);
  return t === e ? t : (Ie(t, "iterate", An), Je(e) ? t : t.map(Ce));
}
function gr(e) {
  return Ie(e = ne(e), "iterate", An), e;
}
const tl = {
  __proto__: null,
  [Symbol.iterator]() {
    return Ur(this, Symbol.iterator, Ce);
  },
  concat(...e) {
    return Wt(this).concat(
      ...e.map((t) => _(t) ? Wt(t) : t)
    );
  },
  entries() {
    return Ur(this, "entries", (e) => (e[1] = Ce(e[1]), e));
  },
  every(e, t) {
    return ct(this, "every", e, t, void 0, arguments);
  },
  filter(e, t) {
    return ct(this, "filter", e, t, (n) => n.map(Ce), arguments);
  },
  find(e, t) {
    return ct(this, "find", e, t, Ce, arguments);
  },
  findIndex(e, t) {
    return ct(this, "findIndex", e, t, void 0, arguments);
  },
  findLast(e, t) {
    return ct(this, "findLast", e, t, Ce, arguments);
  },
  findLastIndex(e, t) {
    return ct(this, "findLastIndex", e, t, void 0, arguments);
  },
  // flat, flatMap could benefit from ARRAY_ITERATE but are not straight-forward to implement
  forEach(e, t) {
    return ct(this, "forEach", e, t, void 0, arguments);
  },
  includes(...e) {
    return Dr(this, "includes", e);
  },
  indexOf(...e) {
    return Dr(this, "indexOf", e);
  },
  join(e) {
    return Wt(this).join(e);
  },
  // keys() iterator only reads `length`, no optimization required
  lastIndexOf(...e) {
    return Dr(this, "lastIndexOf", e);
  },
  map(e, t) {
    return ct(this, "map", e, t, void 0, arguments);
  },
  pop() {
    return ln(this, "pop");
  },
  push(...e) {
    return ln(this, "push", e);
  },
  reduce(e, ...t) {
    return zs(this, "reduce", e, t);
  },
  reduceRight(e, ...t) {
    return zs(this, "reduceRight", e, t);
  },
  shift() {
    return ln(this, "shift");
  },
  // slice could use ARRAY_ITERATE but also seems to beg for range tracking
  some(e, t) {
    return ct(this, "some", e, t, void 0, arguments);
  },
  splice(...e) {
    return ln(this, "splice", e);
  },
  toReversed() {
    return Wt(this).toReversed();
  },
  toSorted(e) {
    return Wt(this).toSorted(e);
  },
  toSpliced(...e) {
    return Wt(this).toSpliced(...e);
  },
  unshift(...e) {
    return ln(this, "unshift", e);
  },
  values() {
    return Ur(this, "values", Ce);
  }
};
function Ur(e, t, n) {
  const r = gr(e), s = r[t]();
  return r !== e && !Je(e) && (s._next = s.next, s.next = () => {
    const i = s._next();
    return i.value && (i.value = n(i.value)), i;
  }), s;
}
const nl = Array.prototype;
function ct(e, t, n, r, s, i) {
  const o = gr(e), a = o !== e && !Je(e), l = o[t];
  if (l !== nl[t]) {
    const h = l.apply(e, i);
    return a ? Ce(h) : h;
  }
  let c = n;
  o !== e && (a ? c = function(h, g) {
    return n.call(this, Ce(h), g, e);
  } : n.length > 2 && (c = function(h, g) {
    return n.call(this, h, g, e);
  }));
  const u = l.call(o, c, r);
  return a && s ? s(u) : u;
}
function zs(e, t, n, r) {
  const s = gr(e);
  let i = n;
  return s !== e && (Je(e) ? n.length > 3 && (i = function(o, a, l) {
    return n.call(this, o, a, l, e);
  }) : i = function(o, a, l) {
    return n.call(this, o, Ce(a), l, e);
  }), s[t](i, ...r);
}
function Dr(e, t, n) {
  const r = ne(e);
  Ie(r, "iterate", An);
  const s = r[t](...n);
  return (s === -1 || s === !1) && bs(n[0]) ? (n[0] = ne(n[0]), r[t](...n)) : s;
}
function ln(e, t, n = []) {
  St(), gs();
  const r = ne(e)[t].apply(e, n);
  return ms(), It(), r;
}
const rl = /* @__PURE__ */ hs("__proto__,__v_isRef,__isVue"), ao = new Set(
  /* @__PURE__ */ Object.getOwnPropertyNames(Symbol).filter((e) => e !== "arguments" && e !== "caller").map((e) => Symbol[e]).filter(kt)
);
function sl(e) {
  kt(e) || (e = String(e));
  const t = ne(this);
  return Ie(t, "has", e), t.hasOwnProperty(e);
}
class lo {
  constructor(t = !1, n = !1) {
    this._isReadonly = t, this._isShallow = n;
  }
  get(t, n, r) {
    if (n === "__v_skip") return t.__v_skip;
    const s = this._isReadonly, i = this._isShallow;
    if (n === "__v_isReactive")
      return !s;
    if (n === "__v_isReadonly")
      return s;
    if (n === "__v_isShallow")
      return i;
    if (n === "__v_raw")
      return r === (s ? i ? go : po : i ? ho : fo).get(t) || // receiver is not the reactive proxy, but has the same prototype
      // this means the receiver is a user proxy of the reactive proxy
      Object.getPrototypeOf(t) === Object.getPrototypeOf(r) ? t : void 0;
    const o = _(t);
    if (!s) {
      let l;
      if (o && (l = tl[n]))
        return l;
      if (n === "hasOwnProperty")
        return sl;
    }
    const a = Reflect.get(
      t,
      n,
      // if this is a proxy wrapping a ref, return methods using the raw ref
      // as receiver so that we don't have to call `toRaw` on the ref in all
      // its class methods
      xe(t) ? t : r
    );
    return (kt(n) ? ao.has(n) : rl(n)) || (s || Ie(t, "get", n), i) ? a : xe(a) ? o && ps(n) ? a : a.value : we(a) ? s ? wr(a) : vr(a) : a;
  }
}
class co extends lo {
  constructor(t = !1) {
    super(!1, t);
  }
  set(t, n, r, s) {
    let i = t[n];
    if (!this._isShallow) {
      const l = Mt(i);
      if (!Je(r) && !Mt(r) && (i = ne(i), r = ne(r)), !_(t) && xe(i) && !xe(r))
        return l || (i.value = r), !0;
    }
    const o = _(t) && ps(n) ? Number(n) < t.length : re(t, n), a = Reflect.set(
      t,
      n,
      r,
      xe(t) ? t : s
    );
    return t === ne(s) && (o ? Ct(r, i) && ft(t, "set", n, r) : ft(t, "add", n, r)), a;
  }
  deleteProperty(t, n) {
    const r = re(t, n);
    t[n];
    const s = Reflect.deleteProperty(t, n);
    return s && r && ft(t, "delete", n, void 0), s;
  }
  has(t, n) {
    const r = Reflect.has(t, n);
    return (!kt(n) || !ao.has(n)) && Ie(t, "has", n), r;
  }
  ownKeys(t) {
    return Ie(
      t,
      "iterate",
      _(t) ? "length" : Tt
    ), Reflect.ownKeys(t);
  }
}
class uo extends lo {
  constructor(t = !1) {
    super(!0, t);
  }
  set(t, n) {
    return !0;
  }
  deleteProperty(t, n) {
    return !0;
  }
}
const il = /* @__PURE__ */ new co(), ol = /* @__PURE__ */ new uo(), al = /* @__PURE__ */ new co(!0), ll = /* @__PURE__ */ new uo(!0), qr = (e) => e, On = (e) => Reflect.getPrototypeOf(e);
function cl(e, t, n) {
  return function(...r) {
    const s = this.__v_raw, i = ne(s), o = zt(i), a = e === "entries" || e === Symbol.iterator && o, l = e === "keys" && o, c = s[e](...r), u = n ? qr : t ? _n : Ce;
    return !t && Ie(
      i,
      "iterate",
      l ? Jr : Tt
    ), {
      // iterator protocol
      next() {
        const { value: h, done: g } = c.next();
        return g ? { value: h, done: g } : {
          value: a ? [u(h[0]), u(h[1])] : u(h),
          done: g
        };
      },
      // iterable protocol
      [Symbol.iterator]() {
        return this;
      }
    };
  };
}
function Gn(e) {
  return function(...t) {
    return e === "delete" ? !1 : e === "clear" ? void 0 : this;
  };
}
function ul(e, t) {
  const n = {
    get(s) {
      const i = this.__v_raw, o = ne(i), a = ne(s);
      e || (Ct(s, a) && Ie(o, "get", s), Ie(o, "get", a));
      const { has: l } = On(o), c = t ? qr : e ? _n : Ce;
      if (l.call(o, s))
        return c(i.get(s));
      if (l.call(o, a))
        return c(i.get(a));
      i !== o && i.get(s);
    },
    get size() {
      const s = this.__v_raw;
      return !e && Ie(ne(s), "iterate", Tt), s.size;
    },
    has(s) {
      const i = this.__v_raw, o = ne(i), a = ne(s);
      return e || (Ct(s, a) && Ie(o, "has", s), Ie(o, "has", a)), s === a ? i.has(s) : i.has(s) || i.has(a);
    },
    forEach(s, i) {
      const o = this, a = o.__v_raw, l = ne(a), c = t ? qr : e ? _n : Ce;
      return !e && Ie(l, "iterate", Tt), a.forEach((u, h) => s.call(i, c(u), c(h), o));
    }
  };
  return Pe(
    n,
    e ? {
      add: Gn("add"),
      set: Gn("set"),
      delete: Gn("delete"),
      clear: Gn("clear")
    } : {
      add(s) {
        !t && !Je(s) && !Mt(s) && (s = ne(s));
        const i = ne(this);
        return On(i).has.call(i, s) || (i.add(s), ft(i, "add", s, s)), this;
      },
      set(s, i) {
        !t && !Je(i) && !Mt(i) && (i = ne(i));
        const o = ne(this), { has: a, get: l } = On(o);
        let c = a.call(o, s);
        c || (s = ne(s), c = a.call(o, s));
        const u = l.call(o, s);
        return o.set(s, i), c ? Ct(i, u) && ft(o, "set", s, i) : ft(o, "add", s, i), this;
      },
      delete(s) {
        const i = ne(this), { has: o, get: a } = On(i);
        let l = o.call(i, s);
        l || (s = ne(s), l = o.call(i, s)), a && a.call(i, s);
        const c = i.delete(s);
        return l && ft(i, "delete", s, void 0), c;
      },
      clear() {
        const s = ne(this), i = s.size !== 0, o = s.clear();
        return i && ft(
          s,
          "clear",
          void 0,
          void 0
        ), o;
      }
    }
  ), [
    "keys",
    "values",
    "entries",
    Symbol.iterator
  ].forEach((s) => {
    n[s] = cl(s, e, t);
  }), n;
}
function mr(e, t) {
  const n = ul(e, t);
  return (r, s, i) => s === "__v_isReactive" ? !e : s === "__v_isReadonly" ? e : s === "__v_raw" ? r : Reflect.get(
    re(n, s) && s in r ? n : r,
    s,
    i
  );
}
const fl = {
  get: /* @__PURE__ */ mr(!1, !1)
}, hl = {
  get: /* @__PURE__ */ mr(!1, !0)
}, dl = {
  get: /* @__PURE__ */ mr(!0, !1)
}, pl = {
  get: /* @__PURE__ */ mr(!0, !0)
}, fo = /* @__PURE__ */ new WeakMap(), ho = /* @__PURE__ */ new WeakMap(), po = /* @__PURE__ */ new WeakMap(), go = /* @__PURE__ */ new WeakMap();
function gl(e) {
  switch (e) {
    case "Object":
    case "Array":
      return 1;
    case "Map":
    case "Set":
    case "WeakMap":
    case "WeakSet":
      return 2;
    default:
      return 0;
  }
}
function ml(e) {
  return e.__v_skip || !Object.isExtensible(e) ? 0 : gl(Pa(e));
}
function vr(e) {
  return Mt(e) ? e : yr(
    e,
    !1,
    il,
    fl,
    fo
  );
}
function ys(e) {
  return yr(
    e,
    !1,
    al,
    hl,
    ho
  );
}
function wr(e) {
  return yr(
    e,
    !0,
    ol,
    dl,
    po
  );
}
function mo(e) {
  return yr(
    e,
    !0,
    ll,
    pl,
    go
  );
}
function yr(e, t, n, r, s) {
  if (!we(e) || e.__v_raw && !(t && e.__v_isReactive))
    return e;
  const i = ml(e);
  if (i === 0)
    return e;
  const o = s.get(e);
  if (o)
    return o;
  const a = new Proxy(
    e,
    i === 2 ? r : n
  );
  return s.set(e, a), a;
}
function Xt(e) {
  return Mt(e) ? Xt(e.__v_raw) : !!(e && e.__v_isReactive);
}
function Mt(e) {
  return !!(e && e.__v_isReadonly);
}
function Je(e) {
  return !!(e && e.__v_isShallow);
}
function bs(e) {
  return e ? !!e.__v_raw : !1;
}
function ne(e) {
  const t = e && e.__v_raw;
  return t ? ne(t) : e;
}
function vl(e) {
  return !re(e, "__v_skip") && Object.isExtensible(e) && Zi(e, "__v_skip", !0), e;
}
const Ce = (e) => we(e) ? vr(e) : e, _n = (e) => we(e) ? wr(e) : e;
function xe(e) {
  return e ? e.__v_isRef === !0 : !1;
}
function L(e) {
  return vo(e, !1);
}
function Xs(e) {
  return vo(e, !0);
}
function vo(e, t) {
  return xe(e) ? e : new wl(e, t);
}
class wl {
  constructor(t, n) {
    this.dep = new ws(), this.__v_isRef = !0, this.__v_isShallow = !1, this._rawValue = n ? t : ne(t), this._value = n ? t : Ce(t), this.__v_isShallow = n;
  }
  get value() {
    return this.dep.track(), this._value;
  }
  set value(t) {
    const n = this._rawValue, r = this.__v_isShallow || Je(t) || Mt(t);
    t = r ? t : ne(t), Ct(t, n) && (this._rawValue = t, this._value = r ? t : Ce(t), this.dep.trigger());
  }
}
function ee(e) {
  return xe(e) ? e.value : e;
}
const yl = {
  get: (e, t, n) => t === "__v_raw" ? e : ee(Reflect.get(e, t, n)),
  set: (e, t, n, r) => {
    const s = e[t];
    return xe(s) && !xe(n) ? (s.value = n, !0) : Reflect.set(e, t, n, r);
  }
};
function wo(e) {
  return Xt(e) ? e : new Proxy(e, yl);
}
function yo(e) {
  const t = _(e) ? new Array(e.length) : {};
  for (const n in e)
    t[n] = Al(e, n);
  return t;
}
class bl {
  constructor(t, n, r) {
    this._object = t, this._key = n, this._defaultValue = r, this.__v_isRef = !0, this._value = void 0;
  }
  get value() {
    const t = this._object[this._key];
    return this._value = t === void 0 ? this._defaultValue : t;
  }
  set value(t) {
    this._object[this._key] = t;
  }
  get dep() {
    return el(ne(this._object), this._key);
  }
}
function Al(e, t, n) {
  const r = e[t];
  return xe(r) ? r : new bl(e, t, n);
}
class xl {
  constructor(t, n, r) {
    this.fn = t, this.setter = n, this._value = void 0, this.dep = new ws(this), this.__v_isRef = !0, this.deps = void 0, this.depsTail = void 0, this.flags = 16, this.globalVersion = bn - 1, this.next = void 0, this.effect = this, this.__v_isReadonly = !n, this.isSSR = r;
  }
  /**
   * @internal
   */
  notify() {
    if (this.flags |= 16, !(this.flags & 8) && // avoid infinite self recursion
    fe !== this)
      return to(this, !0), !0;
  }
  get value() {
    const t = this.dep.track();
    return so(this), t && (t.version = this.dep.version), this._value;
  }
  set value(t) {
    this.setter && this.setter(t);
  }
}
function Vl(e, t, n = !1) {
  let r, s;
  return le(e) ? r = e : (r = e.get, s = e.set), new xl(r, s, n);
}
const Yn = {}, $n = /* @__PURE__ */ new WeakMap();
let Ot;
function Cl(e, t = !1, n = Ot) {
  if (n) {
    let r = $n.get(n);
    r || $n.set(n, r = []), r.push(e);
  }
}
function El(e, t, n = ae) {
  const { immediate: r, deep: s, once: i, scheduler: o, augmentJob: a, call: l } = n, c = (V) => s ? V : Je(V) || s === !1 || s === 0 ? ht(V, 1) : ht(V);
  let u, h, g, p, w = !1, m = !1;
  if (xe(e) ? (h = () => e.value, w = Je(e)) : Xt(e) ? (h = () => c(e), w = !0) : _(e) ? (m = !0, w = e.some((V) => Xt(V) || Je(V)), h = () => e.map((V) => {
    if (xe(V))
      return V.value;
    if (Xt(V))
      return c(V);
    if (le(V))
      return l ? l(V, 2) : V();
  })) : le(e) ? t ? h = l ? () => l(e, 2) : e : h = () => {
    if (g) {
      St();
      try {
        g();
      } finally {
        It();
      }
    }
    const V = Ot;
    Ot = u;
    try {
      return l ? l(e, 3, [p]) : e(p);
    } finally {
      Ot = V;
    }
  } : h = Yt, t && s) {
    const V = h, v = s === !0 ? 1 / 0 : s;
    h = () => ht(V(), v);
  }
  const Q = Xa(), k = () => {
    u.stop(), Q && Q.active && Ni(Q.effects, u);
  };
  if (i && t) {
    const V = t;
    t = (...v) => {
      V(...v), k();
    };
  }
  let M = m ? new Array(e.length).fill(Yn) : Yn;
  const G = (V) => {
    if (!(!(u.flags & 1) || !u.dirty && !V))
      if (t) {
        const v = u.run();
        if (s || w || (m ? v.some((C, E) => Ct(C, M[E])) : Ct(v, M))) {
          g && g();
          const C = Ot;
          Ot = u;
          try {
            const E = [
              v,
              // pass undefined as the old value when it's changed for the first time
              M === Yn ? void 0 : m && M[0] === Yn ? [] : M,
              p
            ];
            M = v, l ? l(t, 3, E) : (
              // @ts-expect-error
              t(...E)
            );
          } finally {
            Ot = C;
          }
        }
      } else
        u.run();
  };
  return a && a(G), u = new $i(h), u.scheduler = o ? () => o(G, !1) : G, p = (V) => Cl(V, !1, u), g = u.onStop = () => {
    const V = $n.get(u);
    if (V) {
      if (l)
        l(V, 4);
      else
        for (const v of V) v();
      $n.delete(u);
    }
  }, t ? r ? G(!0) : M = u.run() : o ? o(G.bind(null, !0), !0) : u.run(), k.pause = u.pause.bind(u), k.resume = u.resume.bind(u), k.stop = k, k;
}
function ht(e, t = 1 / 0, n) {
  if (t <= 0 || !we(e) || e.__v_skip || (n = n || /* @__PURE__ */ new Set(), n.has(e)))
    return e;
  if (n.add(e), t--, xe(e))
    ht(e.value, t, n);
  else if (_(e))
    for (let r = 0; r < e.length; r++)
      ht(e[r], t, n);
  else if (Li(e) || zt(e))
    e.forEach((r) => {
      ht(r, t, n);
    });
  else if (hr(e)) {
    for (const r in e)
      ht(e[r], t, n);
    for (const r of Object.getOwnPropertySymbols(e))
      Object.prototype.propertyIsEnumerable.call(e, r) && ht(e[r], t, n);
  }
  return e;
}
/**
* @vue/runtime-core v3.5.20
* (c) 2018-present Yuxi (Evan) You and Vue contributors
* @license MIT
**/
function Fn(e, t, n, r) {
  try {
    return r ? e(...r) : e();
  } catch (s) {
    br(s, t, n);
  }
}
function wt(e, t, n, r) {
  if (le(e)) {
    const s = Fn(e, t, n, r);
    return s && Wi(s) && s.catch((i) => {
      br(i, t, n);
    }), s;
  }
  if (_(e)) {
    const s = [];
    for (let i = 0; i < e.length; i++)
      s.push(wt(e[i], t, n, r));
    return s;
  }
}
function br(e, t, n, r = !0) {
  const s = t ? t.vnode : null, { errorHandler: i, throwUnhandledErrorInProduction: o } = t && t.appContext.config || ae;
  if (t) {
    let a = t.parent;
    const l = t.proxy, c = `https://vuejs.org/error-reference/#runtime-${n}`;
    for (; a; ) {
      const u = a.ec;
      if (u) {
        for (let h = 0; h < u.length; h++)
          if (u[h](e, l, c) === !1)
            return;
      }
      a = a.parent;
    }
    if (i) {
      St(), Fn(i, null, 10, [
        e,
        l,
        c
      ]), It();
      return;
    }
  }
  Sl(e, n, s, r, o);
}
function Sl(e, t, n, r = !0, s = !1) {
  if (s)
    throw e;
  console.error(e);
}
const ke = [];
let rt = -1;
const _t = [];
let bt = null, qt = 0;
const bo = /* @__PURE__ */ Promise.resolve();
let er = null;
function Ne(e) {
  const t = er || bo;
  return e ? t.then(this ? e.bind(this) : e) : t;
}
function Il(e) {
  let t = rt + 1, n = ke.length;
  for (; t < n; ) {
    const r = t + n >>> 1, s = ke[r], i = xn(s);
    i < e || i === e && s.flags & 2 ? t = r + 1 : n = r;
  }
  return t;
}
function As(e) {
  if (!(e.flags & 1)) {
    const t = xn(e), n = ke[ke.length - 1];
    !n || // fast path when the job id is larger than the tail
    !(e.flags & 2) && t >= xn(n) ? ke.push(e) : ke.splice(Il(t), 0, e), e.flags |= 1, Ao();
  }
}
function Ao() {
  er || (er = bo.then(Vo));
}
function Ml(e) {
  _(e) ? _t.push(...e) : bt && e.id === -1 ? bt.splice(qt + 1, 0, e) : e.flags & 1 || (_t.push(e), e.flags |= 1), Ao();
}
function _s(e, t, n = rt + 1) {
  for (; n < ke.length; n++) {
    const r = ke[n];
    if (r && r.flags & 2) {
      if (e && r.id !== e.uid)
        continue;
      ke.splice(n, 1), n--, r.flags & 4 && (r.flags &= -2), r(), r.flags & 4 || (r.flags &= -2);
    }
  }
}
function xo(e) {
  if (_t.length) {
    const t = [...new Set(_t)].sort(
      (n, r) => xn(n) - xn(r)
    );
    if (_t.length = 0, bt) {
      bt.push(...t);
      return;
    }
    for (bt = t, qt = 0; qt < bt.length; qt++) {
      const n = bt[qt];
      n.flags & 4 && (n.flags &= -2), n.flags & 8 || n(), n.flags &= -2;
    }
    bt = null, qt = 0;
  }
}
const xn = (e) => e.id == null ? e.flags & 2 ? -1 : 1 / 0 : e.id;
function Vo(e) {
  try {
    for (rt = 0; rt < ke.length; rt++) {
      const t = ke[rt];
      t && !(t.flags & 8) && (t.flags & 4 && (t.flags &= -2), Fn(
        t,
        t.i,
        t.i ? 15 : 14
      ), t.flags & 4 || (t.flags &= -2));
    }
  } finally {
    for (; rt < ke.length; rt++) {
      const t = ke[rt];
      t && (t.flags &= -2);
    }
    rt = -1, ke.length = 0, xo(), er = null, (ke.length || _t.length) && Vo();
  }
}
let We = null, Co = null;
function tr(e) {
  const t = We;
  return We = e, Co = e && e.type.__scopeId || null, t;
}
function Wn(e, t = We, n) {
  if (!t || e._n)
    return e;
  const r = (...s) => {
    r._d && ri(-1);
    const i = tr(t);
    let o;
    try {
      o = e(...s);
    } finally {
      tr(i), r._d && ri(1);
    }
    return o;
  };
  return r._n = !0, r._c = !0, r._d = !0, r;
}
function xs(e, t) {
  if (We === null)
    return e;
  const n = Cr(We), r = e.dirs || (e.dirs = []);
  for (let s = 0; s < t.length; s++) {
    let [i, o, a, l = ae] = t[s];
    i && (le(i) && (i = {
      mounted: i,
      updated: i
    }), i.deep && ht(o), r.push({
      dir: i,
      instance: n,
      value: o,
      oldValue: void 0,
      arg: a,
      modifiers: l
    }));
  }
  return e;
}
function Ut(e, t, n, r) {
  const s = e.dirs, i = t && t.dirs;
  for (let o = 0; o < s.length; o++) {
    const a = s[o];
    i && (a.oldValue = i[o].value);
    let l = a.dir[r];
    l && (St(), wt(l, n, 8, [
      e.el,
      a,
      e,
      t
    ]), It());
  }
}
const Bl = Symbol("_vte"), kl = (e) => e.__isTeleport, Rl = Symbol("_leaveCb");
function Vs(e, t) {
  e.shapeFlag & 6 && e.component ? (e.transition = t, Vs(e.component.subTree, t)) : e.shapeFlag & 128 ? (e.ssContent.transition = t.clone(e.ssContent), e.ssFallback.transition = t.clone(e.ssFallback)) : e.transition = t;
}
/*! #__NO_SIDE_EFFECTS__ */
// @__NO_SIDE_EFFECTS__
function Me(e, t) {
  return le(e) ? (
    // #8236: extend call and options.name access are considered side-effects
    // by Rollup, so we have to wrap it in a pure-annotated IIFE.
    Pe({ name: e.name }, t, { setup: e })
  ) : e;
}
function Fl() {
  const e = Ms();
  return e ? (e.appContext.config.idPrefix || "v") + "-" + e.ids[0] + e.ids[1]++ : "";
}
function Ql(e) {
  e.ids = [e.ids[0] + e.ids[2]++ + "-", 0, 0];
}
function gn(e, t, n, r, s = !1) {
  if (_(e)) {
    e.forEach(
      (w, m) => gn(
        w,
        t && (_(t) ? t[m] : t),
        n,
        r,
        s
      )
    );
    return;
  }
  if (mn(r) && !s) {
    r.shapeFlag & 512 && r.type.__asyncResolved && r.component.subTree.component && gn(e, t, n, r.component.subTree);
    return;
  }
  const i = r.shapeFlag & 4 ? Cr(r.component) : r.el, o = s ? null : i, { i: a, r: l } = e, c = t && t.r, u = a.refs === ae ? a.refs = {} : a.refs, h = a.setupState, g = ne(h), p = h === ae ? Ki : (w) => re(g, w);
  if (c != null && c !== l) {
    if (be(c))
      u[c] = null, p(c) && (h[c] = null);
    else if (xe(c)) {
      c.value = null;
      const w = t;
      w.k && (u[w.k] = null);
    }
  }
  if (le(l))
    Fn(l, a, 12, [o, u]);
  else {
    const w = be(l), m = xe(l);
    if (w || m) {
      const Q = () => {
        if (e.f) {
          const k = w ? p(l) ? h[l] : u[l] : l.value;
          if (s)
            _(k) && Ni(k, i);
          else if (_(k))
            k.includes(i) || k.push(i);
          else if (w)
            u[l] = [i], p(l) && (h[l] = u[l]);
          else {
            const M = [i];
            l.value = M, e.k && (u[e.k] = M);
          }
        } else w ? (u[l] = o, p(l) && (h[l] = o)) : m && (l.value = o, e.k && (u[e.k] = o));
      };
      o ? (Q.id = -1, Oe(Q, n)) : Q();
    }
  }
}
pr().requestIdleCallback;
pr().cancelIdleCallback;
const mn = (e) => !!e.type.__asyncLoader, Ul = (e) => e.type.__isKeepAlive;
function Dl(e, t, n = Ye, r = !1) {
  if (n) {
    const s = n[e] || (n[e] = []), i = t.__weh || (t.__weh = (...o) => {
      St();
      const a = Bs(n), l = wt(t, n, e, o);
      return a(), It(), l;
    });
    return r ? s.unshift(i) : s.push(i), i;
  }
}
const Cs = (e) => (t, n = Ye) => {
  (!Cn || e === "sp") && Dl(e, (...r) => t(...r), n);
}, tn = Cs("m"), Ol = Cs(
  "bum"
), Qn = Cs("um"), Gl = Symbol.for("v-ndc");
function Vt(e, t, n, r) {
  let s;
  const i = n, o = _(e);
  if (o || be(e)) {
    const a = o && Xt(e);
    let l = !1, c = !1;
    a && (l = !Je(e), c = Mt(e), e = gr(e)), s = new Array(e.length);
    for (let u = 0, h = e.length; u < h; u++)
      s[u] = t(
        l ? c ? _n(Ce(e[u])) : Ce(e[u]) : e[u],
        u,
        void 0,
        i
      );
  } else if (typeof e == "number") {
    s = new Array(e);
    for (let a = 0; a < e; a++)
      s[a] = t(a + 1, a, void 0, i);
  } else if (we(e))
    if (e[Symbol.iterator])
      s = Array.from(
        e,
        (a, l) => t(a, l, void 0, i)
      );
    else {
      const a = Object.keys(e);
      s = new Array(a.length);
      for (let l = 0, c = a.length; l < c; l++) {
        const u = a[l];
        s[l] = t(e[u], u, l, i);
      }
    }
  else
    s = [];
  return s;
}
const Zr = (e) => e ? jo(e) ? Cr(e) : Zr(e.parent) : null, vn = (
  // Move PURE marker to new line to workaround compiler discarding it
  // due to type annotation
  /* @__PURE__ */ Pe(/* @__PURE__ */ Object.create(null), {
    $: (e) => e,
    $el: (e) => e.vnode.el,
    $data: (e) => e.data,
    $props: (e) => e.props,
    $attrs: (e) => e.attrs,
    $slots: (e) => e.slots,
    $refs: (e) => e.refs,
    $parent: (e) => Zr(e.parent),
    $root: (e) => Zr(e.root),
    $host: (e) => e.ce,
    $emit: (e) => e.emit,
    $options: (e) => e.type,
    $forceUpdate: (e) => e.f || (e.f = () => {
      As(e.update);
    }),
    $nextTick: (e) => e.n || (e.n = Ne.bind(e.proxy)),
    $watch: (e) => Yt
  })
), Or = (e, t) => e !== ae && !e.__isScriptSetup && re(e, t), Yl = {
  get({ _: e }, t) {
    if (t === "__v_skip")
      return !0;
    const { ctx: n, setupState: r, data: s, props: i, accessCache: o, type: a, appContext: l } = e;
    let c;
    if (t[0] !== "$") {
      const p = o[t];
      if (p !== void 0)
        switch (p) {
          case 1:
            return r[t];
          case 2:
            return s[t];
          case 4:
            return n[t];
          case 3:
            return i[t];
        }
      else {
        if (Or(r, t))
          return o[t] = 1, r[t];
        if (s !== ae && re(s, t))
          return o[t] = 2, s[t];
        if (
          // only cache other properties when instance has declared (thus stable)
          // props
          (c = e.propsOptions[0]) && re(c, t)
        )
          return o[t] = 3, i[t];
        if (n !== ae && re(n, t))
          return o[t] = 4, n[t];
        o[t] = 0;
      }
    }
    const u = vn[t];
    let h, g;
    if (u)
      return t === "$attrs" && Ie(e.attrs, "get", ""), u(e);
    if (
      // css module (injected by vue-loader)
      (h = a.__cssModules) && (h = h[t])
    )
      return h;
    if (n !== ae && re(n, t))
      return o[t] = 4, n[t];
    if (
      // global properties
      g = l.config.globalProperties, re(g, t)
    )
      return g[t];
  },
  set({ _: e }, t, n) {
    const { data: r, setupState: s, ctx: i } = e;
    return Or(s, t) ? (s[t] = n, !0) : r !== ae && re(r, t) ? (r[t] = n, !0) : re(e.props, t) || t[0] === "$" && t.slice(1) in e ? !1 : (i[t] = n, !0);
  },
  has({
    _: { data: e, setupState: t, accessCache: n, ctx: r, appContext: s, propsOptions: i, type: o }
  }, a) {
    let l, c;
    return !!(n[a] || e !== ae && a[0] !== "$" && re(e, a) || Or(t, a) || (l = i[0]) && re(l, a) || re(r, a) || re(vn, a) || re(s.config.globalProperties, a) || (c = o.__cssModules) && c[a]);
  },
  defineProperty(e, t, n) {
    return n.get != null ? e._.accessCache[t] = 0 : re(n, "value") && this.set(e, t, n.value, null), Reflect.defineProperty(e, t, n);
  }
};
function Eo() {
  return {
    app: null,
    config: {
      isNativeTag: Ki,
      performance: !1,
      globalProperties: {},
      optionMergeStrategies: {},
      errorHandler: void 0,
      warnHandler: void 0,
      compilerOptions: {}
    },
    mixins: [],
    components: {},
    directives: {},
    provides: /* @__PURE__ */ Object.create(null),
    optionsCache: /* @__PURE__ */ new WeakMap(),
    propsCache: /* @__PURE__ */ new WeakMap(),
    emitsCache: /* @__PURE__ */ new WeakMap()
  };
}
let Tl = 0;
function jl(e, t) {
  return function(r, s = null) {
    le(r) || (r = Pe({}, r)), s != null && !we(s) && (s = null);
    const i = Eo(), o = /* @__PURE__ */ new WeakSet(), a = [];
    let l = !1;
    const c = i.app = {
      _uid: Tl++,
      _component: r,
      _props: s,
      _container: null,
      _context: i,
      _instance: null,
      version: Ac,
      get config() {
        return i.config;
      },
      set config(u) {
      },
      use(u, ...h) {
        return o.has(u) || (u && le(u.install) ? (o.add(u), u.install(c, ...h)) : le(u) && (o.add(u), u(c, ...h))), c;
      },
      mixin(u) {
        return c;
      },
      component(u, h) {
        return h ? (i.components[u] = h, c) : i.components[u];
      },
      directive(u, h) {
        return h ? (i.directives[u] = h, c) : i.directives[u];
      },
      mount(u, h, g) {
        if (!l) {
          const p = c._ceVNode || H(r, s);
          return p.appContext = i, g === !0 ? g = "svg" : g === !1 && (g = void 0), e(p, u, g), l = !0, c._container = u, u.__vue_app__ = c, Cr(p.component);
        }
      },
      onUnmount(u) {
        a.push(u);
      },
      unmount() {
        l && (wt(
          a,
          c._instance,
          16
        ), e(null, c._container), delete c._container.__vue_app__);
      },
      provide(u, h) {
        return i.provides[u] = h, c;
      },
      runWithContext(u) {
        const h = jt;
        jt = c;
        try {
          return u();
        } finally {
          jt = h;
        }
      }
    };
    return c;
  };
}
let jt = null;
function Ar(e, t) {
  if (Ye) {
    let n = Ye.provides;
    const r = Ye.parent && Ye.parent.provides;
    r === n && (n = Ye.provides = Object.create(r)), n[e] = t;
  }
}
function Rt(e, t, n = !1) {
  const r = Ms();
  if (r || jt) {
    let s = jt ? jt._context.provides : r ? r.parent == null || r.ce ? r.vnode.appContext && r.vnode.appContext.provides : r.parent.provides : void 0;
    if (s && e in s)
      return s[e];
    if (arguments.length > 1)
      return n && le(t) ? t.call(r && r.proxy) : t;
  }
}
function Pl() {
  return !!(Ms() || jt);
}
const So = {}, Io = () => Object.create(So), Mo = (e) => Object.getPrototypeOf(e) === So;
function Kl(e, t, n, r = !1) {
  const s = {}, i = Io();
  e.propsDefaults = /* @__PURE__ */ Object.create(null), Bo(e, t, s, i);
  for (const o in e.propsOptions[0])
    o in s || (s[o] = void 0);
  n ? e.props = r ? s : ys(s) : e.type.props ? e.props = s : e.props = i, e.attrs = i;
}
function Nl(e, t, n, r) {
  const {
    props: s,
    attrs: i,
    vnode: { patchFlag: o }
  } = e, a = ne(s), [l] = e.propsOptions;
  let c = !1;
  if (
    // always force full diff in dev
    // - #1942 if hmr is enabled with sfc component
    // - vite#872 non-sfc component used by sfc component
    (r || o > 0) && !(o & 16)
  ) {
    if (o & 8) {
      const u = e.vnode.dynamicProps;
      for (let h = 0; h < u.length; h++) {
        let g = u[h];
        if (xr(e.emitsOptions, g))
          continue;
        const p = t[g];
        if (l)
          if (re(i, g))
            p !== i[g] && (i[g] = p, c = !0);
          else {
            const w = He(g);
            s[w] = Hr(
              l,
              a,
              w,
              p,
              e,
              !1
            );
          }
        else
          p !== i[g] && (i[g] = p, c = !0);
      }
    }
  } else {
    Bo(e, t, s, i) && (c = !0);
    let u;
    for (const h in a)
      (!t || // for camelCase
      !re(t, h) && // it's possible the original props was passed in as kebab-case
      // and converted to camelCase (#955)
      ((u = Ge(h)) === h || !re(t, u))) && (l ? n && // for camelCase
      (n[h] !== void 0 || // for kebab-case
      n[u] !== void 0) && (s[h] = Hr(
        l,
        a,
        h,
        void 0,
        e,
        !0
      )) : delete s[h]);
    if (i !== a)
      for (const h in i)
        (!t || !re(t, h)) && (delete i[h], c = !0);
  }
  c && ft(e.attrs, "set", "");
}
function Bo(e, t, n, r) {
  const [s, i] = e.propsOptions;
  let o = !1, a;
  if (t)
    for (let l in t) {
      if (hn(l))
        continue;
      const c = t[l];
      let u;
      s && re(s, u = He(l)) ? !i || !i.includes(u) ? n[u] = c : (a || (a = {}))[u] = c : xr(e.emitsOptions, l) || (!(l in r) || c !== r[l]) && (r[l] = c, o = !0);
    }
  if (i) {
    const l = ne(n), c = a || ae;
    for (let u = 0; u < i.length; u++) {
      const h = i[u];
      n[h] = Hr(
        s,
        l,
        h,
        c[h],
        e,
        !re(c, h)
      );
    }
  }
  return o;
}
function Hr(e, t, n, r, s, i) {
  const o = e[n];
  if (o != null) {
    const a = re(o, "default");
    if (a && r === void 0) {
      const l = o.default;
      if (o.type !== Function && !o.skipFactory && le(l)) {
        const { propsDefaults: c } = s;
        if (n in c)
          r = c[n];
        else {
          const u = Bs(s);
          r = c[n] = l.call(
            null,
            t
          ), u();
        }
      } else
        r = l;
      s.ce && s.ce._setProp(n, r);
    }
    o[
      0
      /* shouldCast */
    ] && (i && !a ? r = !1 : o[
      1
      /* shouldCastTrue */
    ] && (r === "" || r === Ge(n)) && (r = !0));
  }
  return r;
}
function Ll(e, t, n = !1) {
  const r = t.propsCache, s = r.get(e);
  if (s)
    return s;
  const i = e.props, o = {}, a = [];
  if (!i)
    return we(e) && r.set(e, Ht), Ht;
  if (_(i))
    for (let c = 0; c < i.length; c++) {
      const u = He(i[c]);
      $s(u) && (o[u] = ae);
    }
  else if (i)
    for (const c in i) {
      const u = He(c);
      if ($s(u)) {
        const h = i[c], g = o[u] = _(h) || le(h) ? { type: h } : Pe({}, h), p = g.type;
        let w = !1, m = !0;
        if (_(p))
          for (let Q = 0; Q < p.length; ++Q) {
            const k = p[Q], M = le(k) && k.name;
            if (M === "Boolean") {
              w = !0;
              break;
            } else M === "String" && (m = !1);
          }
        else
          w = le(p) && p.name === "Boolean";
        g[
          0
          /* shouldCast */
        ] = w, g[
          1
          /* shouldCastTrue */
        ] = m, (w || re(g, "default")) && a.push(u);
      }
    }
  const l = [o, a];
  return we(e) && r.set(e, l), l;
}
function $s(e) {
  return e[0] !== "$" && !hn(e);
}
const Es = (e) => e === "_" || e === "_ctx" || e === "$stable", Ss = (e) => _(e) ? e.map(st) : [st(e)], Wl = (e, t, n) => {
  if (t._n)
    return t;
  const r = Wn((...s) => Ss(t(...s)), n);
  return r._c = !1, r;
}, ko = (e, t, n) => {
  const r = e._ctx;
  for (const s in e) {
    if (Es(s)) continue;
    const i = e[s];
    if (le(i))
      t[s] = Wl(s, i, r);
    else if (i != null) {
      const o = Ss(i);
      t[s] = () => o;
    }
  }
}, Ro = (e, t) => {
  const n = Ss(t);
  e.slots.default = () => n;
}, Fo = (e, t, n) => {
  for (const r in t)
    (n || !Es(r)) && (e[r] = t[r]);
}, Jl = (e, t, n) => {
  const r = e.slots = Io();
  if (e.vnode.shapeFlag & 32) {
    const s = t._;
    s ? (Fo(r, t, n), n && Zi(r, "_", s, !0)) : ko(t, r);
  } else t && Ro(e, t);
}, ql = (e, t, n) => {
  const { vnode: r, slots: s } = e;
  let i = !0, o = ae;
  if (r.shapeFlag & 32) {
    const a = t._;
    a ? n && a === 1 ? i = !1 : Fo(s, t, n) : (i = !t.$stable, ko(t, s)), o = t;
  } else t && (Ro(e, t), o = { default: 1 });
  if (i)
    for (const a in s)
      !Es(a) && o[a] == null && delete s[a];
}, Oe = ac;
function Zl(e) {
  return Hl(e);
}
function Hl(e, t) {
  const n = pr();
  n.__VUE__ = !0;
  const {
    insert: r,
    remove: s,
    patchProp: i,
    createElement: o,
    createText: a,
    createComment: l,
    setText: c,
    setElementText: u,
    parentNode: h,
    nextSibling: g,
    setScopeId: p = Yt,
    insertStaticContent: w
  } = e, m = (f, d, y, S = null, A = null, x = null, F = void 0, R = null, B = !!d.dynamicChildren) => {
    if (f === d)
      return;
    f && !cn(f, d) && (S = Dn(f), K(f, A, x, !0), f = null), d.patchFlag === -2 && (B = !1, d.dynamicChildren = null);
    const { type: I, ref: N, shapeFlag: O } = d;
    switch (I) {
      case Vr:
        Q(f, d, y, S);
        break;
      case Bt:
        k(f, d, y, S);
        break;
      case Jn:
        f == null && M(d, y, S, F);
        break;
      case ge:
        at(
          f,
          d,
          y,
          S,
          A,
          x,
          F,
          R,
          B
        );
        break;
      default:
        O & 1 ? v(
          f,
          d,
          y,
          S,
          A,
          x,
          F,
          R,
          B
        ) : O & 6 ? Ve(
          f,
          d,
          y,
          S,
          A,
          x,
          F,
          R,
          B
        ) : (O & 64 || O & 128) && I.process(
          f,
          d,
          y,
          S,
          A,
          x,
          F,
          R,
          B,
          on
        );
    }
    N != null && A ? gn(N, f && f.ref, x, d || f, !d) : N == null && f && f.ref != null && gn(f.ref, null, x, f, !0);
  }, Q = (f, d, y, S) => {
    if (f == null)
      r(
        d.el = a(d.children),
        y,
        S
      );
    else {
      const A = d.el = f.el;
      d.children !== f.children && c(A, d.children);
    }
  }, k = (f, d, y, S) => {
    f == null ? r(
      d.el = l(d.children || ""),
      y,
      S
    ) : d.el = f.el;
  }, M = (f, d, y, S) => {
    [f.el, f.anchor] = w(
      f.children,
      d,
      y,
      S,
      f.el,
      f.anchor
    );
  }, G = ({ el: f, anchor: d }, y, S) => {
    let A;
    for (; f && f !== d; )
      A = g(f), r(f, y, S), f = A;
    r(d, y, S);
  }, V = ({ el: f, anchor: d }) => {
    let y;
    for (; f && f !== d; )
      y = g(f), s(f), f = y;
    s(d);
  }, v = (f, d, y, S, A, x, F, R, B) => {
    d.type === "svg" ? F = "svg" : d.type === "math" && (F = "mathml"), f == null ? C(
      d,
      y,
      S,
      A,
      x,
      F,
      R,
      B
    ) : $(
      f,
      d,
      A,
      x,
      F,
      R,
      B
    );
  }, C = (f, d, y, S, A, x, F, R) => {
    let B, I;
    const { props: N, shapeFlag: O, transition: P, dirs: Z } = f;
    if (B = f.el = o(
      f.type,
      x,
      N && N.is,
      N
    ), O & 8 ? u(B, f.children) : O & 16 && Y(
      f.children,
      B,
      null,
      S,
      A,
      Gr(f, x),
      F,
      R
    ), Z && Ut(f, null, S, "created"), E(B, f, f.scopeId, F, S), N) {
      for (const ue in N)
        ue !== "value" && !hn(ue) && i(B, ue, null, N[ue], x, S);
      "value" in N && i(B, "value", null, N.value, x), (I = N.onVnodeBeforeMount) && nt(I, S, f);
    }
    Z && Ut(f, null, S, "beforeMount");
    const te = zl(A, P);
    te && P.beforeEnter(B), r(B, d, y), ((I = N && N.onVnodeMounted) || te || Z) && Oe(() => {
      I && nt(I, S, f), te && P.enter(B), Z && Ut(f, null, S, "mounted");
    }, A);
  }, E = (f, d, y, S, A) => {
    if (y && p(f, y), S)
      for (let x = 0; x < S.length; x++)
        p(f, S[x]);
    if (A) {
      let x = A.subTree;
      if (d === x || Oo(x.type) && (x.ssContent === d || x.ssFallback === d)) {
        const F = A.vnode;
        E(
          f,
          F,
          F.scopeId,
          F.slotScopeIds,
          A.parent
        );
      }
    }
  }, Y = (f, d, y, S, A, x, F, R, B = 0) => {
    for (let I = B; I < f.length; I++) {
      const N = f[I] = R ? At(f[I]) : st(f[I]);
      m(
        null,
        N,
        d,
        y,
        S,
        A,
        x,
        F,
        R
      );
    }
  }, $ = (f, d, y, S, A, x, F) => {
    const R = d.el = f.el;
    let { patchFlag: B, dynamicChildren: I, dirs: N } = d;
    B |= f.patchFlag & 16;
    const O = f.props || ae, P = d.props || ae;
    let Z;
    if (y && Dt(y, !1), (Z = P.onVnodeBeforeUpdate) && nt(Z, y, d, f), N && Ut(d, f, y, "beforeUpdate"), y && Dt(y, !0), (O.innerHTML && P.innerHTML == null || O.textContent && P.textContent == null) && u(R, ""), I ? z(
      f.dynamicChildren,
      I,
      R,
      y,
      S,
      Gr(d, A),
      x
    ) : F || ie(
      f,
      d,
      R,
      null,
      y,
      S,
      Gr(d, A),
      x,
      !1
    ), B > 0) {
      if (B & 16)
        he(R, O, P, y, A);
      else if (B & 2 && O.class !== P.class && i(R, "class", null, P.class, A), B & 4 && i(R, "style", O.style, P.style, A), B & 8) {
        const te = d.dynamicProps;
        for (let ue = 0; ue < te.length; ue++) {
          const se = te[ue], Fe = O[se], Qe = P[se];
          (Qe !== Fe || se === "value") && i(R, se, Fe, Qe, A, y);
        }
      }
      B & 1 && f.children !== d.children && u(R, d.children);
    } else !F && I == null && he(R, O, P, y, A);
    ((Z = P.onVnodeUpdated) || N) && Oe(() => {
      Z && nt(Z, y, d, f), N && Ut(d, f, y, "updated");
    }, S);
  }, z = (f, d, y, S, A, x, F) => {
    for (let R = 0; R < d.length; R++) {
      const B = f[R], I = d[R], N = (
        // oldVNode may be an errored async setup() component inside Suspense
        // which will not have a mounted element
        B.el && // - In the case of a Fragment, we need to provide the actual parent
        // of the Fragment itself so it can move its children.
        (B.type === ge || // - In the case of different nodes, there is going to be a replacement
        // which also requires the correct parent container
        !cn(B, I) || // - In the case of a component, it could contain anything.
        B.shapeFlag & 198) ? h(B.el) : (
          // In other cases, the parent container is not actually used so we
          // just pass the block element here to avoid a DOM parentNode call.
          y
        )
      );
      m(
        B,
        I,
        N,
        null,
        S,
        A,
        x,
        F,
        !0
      );
    }
  }, he = (f, d, y, S, A) => {
    if (d !== y) {
      if (d !== ae)
        for (const x in d)
          !hn(x) && !(x in y) && i(
            f,
            x,
            d[x],
            null,
            A,
            S
          );
      for (const x in y) {
        if (hn(x)) continue;
        const F = y[x], R = d[x];
        F !== R && x !== "value" && i(f, x, R, F, A, S);
      }
      "value" in y && i(f, "value", d.value, y.value, A);
    }
  }, at = (f, d, y, S, A, x, F, R, B) => {
    const I = d.el = f ? f.el : a(""), N = d.anchor = f ? f.anchor : a("");
    let { patchFlag: O, dynamicChildren: P, slotScopeIds: Z } = d;
    Z && (R = R ? R.concat(Z) : Z), f == null ? (r(I, y, S), r(N, y, S), Y(
      // #10007
      // such fragment like `<></>` will be compiled into
      // a fragment which doesn't have a children.
      // In this case fallback to an empty array
      d.children || [],
      y,
      N,
      A,
      x,
      F,
      R,
      B
    )) : O > 0 && O & 64 && P && // #2715 the previous fragment could've been a BAILed one as a result
    // of renderSlot() with no valid children
    f.dynamicChildren ? (z(
      f.dynamicChildren,
      P,
      y,
      A,
      x,
      F,
      R
    ), // #2080 if the stable fragment has a key, it's a <template v-for> that may
    //  get moved around. Make sure all root level vnodes inherit el.
    // #2134 or if it's a component root, it may also get moved around
    // as the component is being moved.
    (d.key != null || A && d === A.subTree) && Qo(
      f,
      d,
      !0
      /* shallow */
    )) : ie(
      f,
      d,
      y,
      N,
      A,
      x,
      F,
      R,
      B
    );
  }, Ve = (f, d, y, S, A, x, F, R, B) => {
    d.slotScopeIds = R, f == null ? d.shapeFlag & 512 ? A.ctx.activate(
      d,
      y,
      S,
      F,
      B
    ) : pe(
      d,
      y,
      S,
      A,
      x,
      F,
      B
    ) : Be(f, d, B);
  }, pe = (f, d, y, S, A, x, F) => {
    const R = f.component = gc(
      f,
      S,
      A
    );
    if (Ul(f) && (R.ctx.renderer = on), mc(R, !1, F), R.asyncDep) {
      if (A && A.registerDep(R, J, F), !f.el) {
        const B = R.subTree = H(Bt);
        k(null, B, d, y), f.placeholder = B.el;
      }
    } else
      J(
        R,
        f,
        d,
        y,
        A,
        x,
        F
      );
  }, Be = (f, d, y) => {
    const S = d.component = f.component;
    if (ic(f, d, y))
      if (S.asyncDep && !S.asyncResolved) {
        q(S, d, y);
        return;
      } else
        S.next = d, S.update();
    else
      d.el = f.el, S.vnode = d;
  }, J = (f, d, y, S, A, x, F) => {
    const R = () => {
      if (f.isMounted) {
        let { next: O, bu: P, u: Z, parent: te, vnode: ue } = f;
        {
          const et = Uo(f);
          if (et) {
            O && (O.el = ue.el, q(f, O, F)), et.asyncDep.then(() => {
              f.isUnmounted || R();
            });
            return;
          }
        }
        let se = O, Fe;
        Dt(f, !1), O ? (O.el = ue.el, q(f, O, F)) : O = ue, P && Ln(P), (Fe = O.props && O.props.onVnodeBeforeUpdate) && nt(Fe, te, O, ue), Dt(f, !0);
        const Qe = ti(f), $e = f.subTree;
        f.subTree = Qe, m(
          $e,
          Qe,
          // parent may have changed if it's in a teleport
          h($e.el),
          // anchor may have changed if it's in a fragment
          Dn($e),
          f,
          A,
          x
        ), O.el = Qe.el, se === null && oc(f, Qe.el), Z && Oe(Z, A), (Fe = O.props && O.props.onVnodeUpdated) && Oe(
          () => nt(Fe, te, O, ue),
          A
        );
      } else {
        let O;
        const { el: P, props: Z } = d, { bm: te, m: ue, parent: se, root: Fe, type: Qe } = f, $e = mn(d);
        Dt(f, !1), te && Ln(te), !$e && (O = Z && Z.onVnodeBeforeMount) && nt(O, se, d), Dt(f, !0);
        {
          Fe.ce && // @ts-expect-error _def is private
          Fe.ce._def.shadowRoot !== !1 && Fe.ce._injectChildStyle(Qe);
          const et = f.subTree = ti(f);
          m(
            null,
            et,
            y,
            S,
            f,
            A,
            x
          ), d.el = et.el;
        }
        if (ue && Oe(ue, A), !$e && (O = Z && Z.onVnodeMounted)) {
          const et = d;
          Oe(
            () => nt(O, se, et),
            A
          );
        }
        (d.shapeFlag & 256 || se && mn(se.vnode) && se.vnode.shapeFlag & 256) && f.a && Oe(f.a, A), f.isMounted = !0, d = y = S = null;
      }
    };
    f.scope.on();
    const B = f.effect = new $i(R);
    f.scope.off();
    const I = f.update = B.run.bind(B), N = f.job = B.runIfDirty.bind(B);
    N.i = f, N.id = f.uid, B.scheduler = () => As(N), Dt(f, !0), I();
  }, q = (f, d, y) => {
    d.component = f;
    const S = f.vnode.props;
    f.vnode = d, f.next = null, Nl(f, d.props, S, y), ql(f, d.children, y), St(), _s(f), It();
  }, ie = (f, d, y, S, A, x, F, R, B = !1) => {
    const I = f && f.children, N = f ? f.shapeFlag : 0, O = d.children, { patchFlag: P, shapeFlag: Z } = d;
    if (P > 0) {
      if (P & 128) {
        ce(
          I,
          O,
          y,
          S,
          A,
          x,
          F,
          R,
          B
        );
        return;
      } else if (P & 256) {
        de(
          I,
          O,
          y,
          S,
          A,
          x,
          F,
          R,
          B
        );
        return;
      }
    }
    Z & 8 ? (N & 16 && sn(I, A, x), O !== I && u(y, O)) : N & 16 ? Z & 16 ? ce(
      I,
      O,
      y,
      S,
      A,
      x,
      F,
      R,
      B
    ) : sn(I, A, x, !0) : (N & 8 && u(y, ""), Z & 16 && Y(
      O,
      y,
      S,
      A,
      x,
      F,
      R,
      B
    ));
  }, de = (f, d, y, S, A, x, F, R, B) => {
    f = f || Ht, d = d || Ht;
    const I = f.length, N = d.length, O = Math.min(I, N);
    let P;
    for (P = 0; P < O; P++) {
      const Z = d[P] = B ? At(d[P]) : st(d[P]);
      m(
        f[P],
        Z,
        y,
        null,
        A,
        x,
        F,
        R,
        B
      );
    }
    I > N ? sn(
      f,
      A,
      x,
      !0,
      !1,
      O
    ) : Y(
      d,
      y,
      S,
      A,
      x,
      F,
      R,
      B,
      O
    );
  }, ce = (f, d, y, S, A, x, F, R, B) => {
    let I = 0;
    const N = d.length;
    let O = f.length - 1, P = N - 1;
    for (; I <= O && I <= P; ) {
      const Z = f[I], te = d[I] = B ? At(d[I]) : st(d[I]);
      if (cn(Z, te))
        m(
          Z,
          te,
          y,
          null,
          A,
          x,
          F,
          R,
          B
        );
      else
        break;
      I++;
    }
    for (; I <= O && I <= P; ) {
      const Z = f[O], te = d[P] = B ? At(d[P]) : st(d[P]);
      if (cn(Z, te))
        m(
          Z,
          te,
          y,
          null,
          A,
          x,
          F,
          R,
          B
        );
      else
        break;
      O--, P--;
    }
    if (I > O) {
      if (I <= P) {
        const Z = P + 1, te = Z < N ? d[Z].el : S;
        for (; I <= P; )
          m(
            null,
            d[I] = B ? At(d[I]) : st(d[I]),
            y,
            te,
            A,
            x,
            F,
            R,
            B
          ), I++;
      }
    } else if (I > P)
      for (; I <= O; )
        K(f[I], A, x, !0), I++;
    else {
      const Z = I, te = I, ue = /* @__PURE__ */ new Map();
      for (I = te; I <= P; I++) {
        const De = d[I] = B ? At(d[I]) : st(d[I]);
        De.key != null && ue.set(De.key, I);
      }
      let se, Fe = 0;
      const Qe = P - te + 1;
      let $e = !1, et = 0;
      const an = new Array(Qe);
      for (I = 0; I < Qe; I++) an[I] = 0;
      for (I = Z; I <= O; I++) {
        const De = f[I];
        if (Fe >= Qe) {
          K(De, A, x, !0);
          continue;
        }
        let tt;
        if (De.key != null)
          tt = ue.get(De.key);
        else
          for (se = te; se <= P; se++)
            if (an[se - te] === 0 && cn(De, d[se])) {
              tt = se;
              break;
            }
        tt === void 0 ? K(De, A, x, !0) : (an[tt - te] = I + 1, tt >= et ? et = tt : $e = !0, m(
          De,
          d[tt],
          y,
          null,
          A,
          x,
          F,
          R,
          B
        ), Fe++);
      }
      const Ls = $e ? Xl(an) : Ht;
      for (se = Ls.length - 1, I = Qe - 1; I >= 0; I--) {
        const De = te + I, tt = d[De], Ws = d[De + 1], Js = De + 1 < N ? (
          // #13559, fallback to el placeholder for unresolved async component
          Ws.el || Ws.placeholder
        ) : S;
        an[I] === 0 ? m(
          null,
          tt,
          y,
          Js,
          A,
          x,
          F,
          R,
          B
        ) : $e && (se < 0 || I !== Ls[se] ? D(tt, y, Js, 2) : se--);
      }
    }
  }, D = (f, d, y, S, A = null) => {
    const { el: x, type: F, transition: R, children: B, shapeFlag: I } = f;
    if (I & 6) {
      D(f.component.subTree, d, y, S);
      return;
    }
    if (I & 128) {
      f.suspense.move(d, y, S);
      return;
    }
    if (I & 64) {
      F.move(f, d, y, on);
      return;
    }
    if (F === ge) {
      r(x, d, y);
      for (let O = 0; O < B.length; O++)
        D(B[O], d, y, S);
      r(f.anchor, d, y);
      return;
    }
    if (F === Jn) {
      G(f, d, y);
      return;
    }
    if (S !== 2 && I & 1 && R)
      if (S === 0)
        R.beforeEnter(x), r(x, d, y), Oe(() => R.enter(x), A);
      else {
        const { leave: O, delayLeave: P, afterLeave: Z } = R, te = () => {
          f.ctx.isUnmounted ? s(x) : r(x, d, y);
        }, ue = () => {
          x._isLeaving && x[Rl](
            !0
            /* cancelled */
          ), O(x, () => {
            te(), Z && Z();
          });
        };
        P ? P(x, te, ue) : ue();
      }
    else
      r(x, d, y);
  }, K = (f, d, y, S = !1, A = !1) => {
    const {
      type: x,
      props: F,
      ref: R,
      children: B,
      dynamicChildren: I,
      shapeFlag: N,
      patchFlag: O,
      dirs: P,
      cacheIndex: Z
    } = f;
    if (O === -2 && (A = !1), R != null && (St(), gn(R, null, y, f, !0), It()), Z != null && (d.renderCache[Z] = void 0), N & 256) {
      d.ctx.deactivate(f);
      return;
    }
    const te = N & 1 && P, ue = !mn(f);
    let se;
    if (ue && (se = F && F.onVnodeBeforeUnmount) && nt(se, d, f), N & 6)
      Qt(f.component, y, S);
    else {
      if (N & 128) {
        f.suspense.unmount(y, S);
        return;
      }
      te && Ut(f, null, d, "beforeUnmount"), N & 64 ? f.type.remove(
        f,
        d,
        y,
        on,
        S
      ) : I && // #5154
      // when v-once is used inside a block, setBlockTracking(-1) marks the
      // parent block with hasOnce: true
      // so that it doesn't take the fast path during unmount - otherwise
      // components nested in v-once are never unmounted.
      !I.hasOnce && // #1153: fast path should not be taken for non-stable (v-for) fragments
      (x !== ge || O > 0 && O & 64) ? sn(
        I,
        d,
        y,
        !1,
        !0
      ) : (x === ge && O & 384 || !A && N & 16) && sn(B, d, y), S && oe(f);
    }
    (ue && (se = F && F.onVnodeUnmounted) || te) && Oe(() => {
      se && nt(se, d, f), te && Ut(f, null, d, "unmounted");
    }, y);
  }, oe = (f) => {
    const { type: d, el: y, anchor: S, transition: A } = f;
    if (d === ge) {
      lt(y, S);
      return;
    }
    if (d === Jn) {
      V(f);
      return;
    }
    const x = () => {
      s(y), A && !A.persisted && A.afterLeave && A.afterLeave();
    };
    if (f.shapeFlag & 1 && A && !A.persisted) {
      const { leave: F, delayLeave: R } = A, B = () => F(y, x);
      R ? R(f.el, x, B) : B();
    } else
      x();
  }, lt = (f, d) => {
    let y;
    for (; f !== d; )
      y = g(f), s(f), f = y;
    s(d);
  }, Qt = (f, d, y) => {
    const { bum: S, scope: A, job: x, subTree: F, um: R, m: B, a: I } = f;
    ei(B), ei(I), S && Ln(S), A.stop(), x && (x.flags |= 8, K(F, f, d, y)), R && Oe(R, d), Oe(() => {
      f.isUnmounted = !0;
    }, d);
  }, sn = (f, d, y, S = !1, A = !1, x = 0) => {
    for (let F = x; F < f.length; F++)
      K(f[F], d, y, S, A);
  }, Dn = (f) => {
    if (f.shapeFlag & 6)
      return Dn(f.component.subTree);
    if (f.shapeFlag & 128)
      return f.suspense.next();
    const d = g(f.anchor || f.el), y = d && d[Bl];
    return y ? g(y) : d;
  };
  let kr = !1;
  const Ns = (f, d, y) => {
    f == null ? d._vnode && K(d._vnode, null, null, !0) : m(
      d._vnode || null,
      f,
      d,
      null,
      null,
      null,
      y
    ), d._vnode = f, kr || (kr = !0, _s(), xo(), kr = !1);
  }, on = {
    p: m,
    um: K,
    m: D,
    r: oe,
    mt: pe,
    mc: Y,
    pc: ie,
    pbc: z,
    n: Dn,
    o: e
  };
  return {
    render: Ns,
    hydrate: void 0,
    createApp: jl(Ns)
  };
}
function Gr({ type: e, props: t }, n) {
  return n === "svg" && e === "foreignObject" || n === "mathml" && e === "annotation-xml" && t && t.encoding && t.encoding.includes("html") ? void 0 : n;
}
function Dt({ effect: e, job: t }, n) {
  n ? (e.flags |= 32, t.flags |= 4) : (e.flags &= -33, t.flags &= -5);
}
function zl(e, t) {
  return (!e || e && !e.pendingBranch) && t && !t.persisted;
}
function Qo(e, t, n = !1) {
  const r = e.children, s = t.children;
  if (_(r) && _(s))
    for (let i = 0; i < r.length; i++) {
      const o = r[i];
      let a = s[i];
      a.shapeFlag & 1 && !a.dynamicChildren && ((a.patchFlag <= 0 || a.patchFlag === 32) && (a = s[i] = At(s[i]), a.el = o.el), !n && a.patchFlag !== -2 && Qo(o, a)), a.type === Vr && // avoid cached text nodes retaining detached dom nodes
      a.patchFlag !== -1 && (a.el = o.el), a.type === Bt && !a.el && (a.el = o.el);
    }
}
function Xl(e) {
  const t = e.slice(), n = [0];
  let r, s, i, o, a;
  const l = e.length;
  for (r = 0; r < l; r++) {
    const c = e[r];
    if (c !== 0) {
      if (s = n[n.length - 1], e[s] < c) {
        t[r] = s, n.push(r);
        continue;
      }
      for (i = 0, o = n.length - 1; i < o; )
        a = i + o >> 1, e[n[a]] < c ? i = a + 1 : o = a;
      c < e[n[i]] && (i > 0 && (t[r] = n[i - 1]), n[i] = r);
    }
  }
  for (i = n.length, o = n[i - 1]; i-- > 0; )
    n[i] = o, o = t[o];
  return n;
}
function Uo(e) {
  const t = e.subTree.component;
  if (t)
    return t.asyncDep && !t.asyncResolved ? t : Uo(t);
}
function ei(e) {
  if (e)
    for (let t = 0; t < e.length; t++)
      e[t].flags |= 8;
}
const _l = Symbol.for("v-scx"), $l = () => Rt(_l);
function Ft(e, t) {
  return Do(e, null, t);
}
function Ue(e, t, n) {
  return Do(e, t, n);
}
function Do(e, t, n = ae) {
  const { immediate: r, deep: s, flush: i, once: o } = n, a = Pe({}, n), l = t && r || !t && i !== "post";
  let c;
  if (Cn) {
    if (i === "sync") {
      const p = $l();
      c = p.__watcherHandles || (p.__watcherHandles = []);
    } else if (!l) {
      const p = () => {
      };
      return p.stop = Yt, p.resume = Yt, p.pause = Yt, p;
    }
  }
  const u = Ye;
  a.call = (p, w, m) => wt(p, u, w, m);
  let h = !1;
  i === "post" ? a.scheduler = (p) => {
    Oe(p, u && u.suspense);
  } : i !== "sync" && (h = !0, a.scheduler = (p, w) => {
    w ? p() : As(p);
  }), a.augmentJob = (p) => {
    t && (p.flags |= 4), h && (p.flags |= 2, u && (p.id = u.uid, p.i = u));
  };
  const g = El(e, t, a);
  return Cn && (c ? c.push(g) : l && g()), g;
}
const ec = (e, t) => t === "modelValue" || t === "model-value" ? e.modelModifiers : e[`${t}Modifiers`] || e[`${He(t)}Modifiers`] || e[`${Ge(t)}Modifiers`];
function tc(e, t, ...n) {
  if (e.isUnmounted) return;
  const r = e.vnode.props || ae;
  let s = n;
  const i = t.startsWith("update:"), o = i && ec(r, t.slice(7));
  o && (o.trim && (s = n.map((u) => be(u) ? u.trim() : u)), o.number && (s = n.map(Lr)));
  let a, l = r[a = Rr(t)] || // also try camelCase event handler (#2249)
  r[a = Rr(He(t))];
  !l && i && (l = r[a = Rr(Ge(t))]), l && wt(
    l,
    e,
    6,
    s
  );
  const c = r[a + "Once"];
  if (c) {
    if (!e.emitted)
      e.emitted = {};
    else if (e.emitted[a])
      return;
    e.emitted[a] = !0, wt(
      c,
      e,
      6,
      s
    );
  }
}
function nc(e, t, n = !1) {
  const r = t.emitsCache, s = r.get(e);
  if (s !== void 0)
    return s;
  const i = e.emits;
  let o = {};
  return i ? (_(i) ? i.forEach((a) => o[a] = null) : Pe(o, i), we(e) && r.set(e, o), o) : (we(e) && r.set(e, null), null);
}
function xr(e, t) {
  return !e || !ur(t) ? !1 : (t = t.slice(2).replace(/Once$/, ""), re(e, t[0].toLowerCase() + t.slice(1)) || re(e, Ge(t)) || re(e, t));
}
function ti(e) {
  const {
    type: t,
    vnode: n,
    proxy: r,
    withProxy: s,
    propsOptions: [i],
    slots: o,
    attrs: a,
    emit: l,
    render: c,
    renderCache: u,
    props: h,
    data: g,
    setupState: p,
    ctx: w,
    inheritAttrs: m
  } = e, Q = tr(e);
  let k, M;
  try {
    if (n.shapeFlag & 4) {
      const V = s || r, v = V;
      k = st(
        c.call(
          v,
          V,
          u,
          h,
          p,
          g,
          w
        )
      ), M = a;
    } else {
      const V = t;
      k = st(
        V.length > 1 ? V(
          h,
          { attrs: a, slots: o, emit: l }
        ) : V(
          h,
          null
        )
      ), M = t.props ? a : rc(a);
    }
  } catch (V) {
    wn.length = 0, br(V, e, 1), k = H(Bt);
  }
  let G = k;
  if (M && m !== !1) {
    const V = Object.keys(M), { shapeFlag: v } = G;
    V.length && v & 7 && (i && V.some(ds) && (M = sc(
      M,
      i
    )), G = Kt(G, M, !1, !0));
  }
  return n.dirs && (G = Kt(G, null, !1, !0), G.dirs = G.dirs ? G.dirs.concat(n.dirs) : n.dirs), n.transition && Vs(G, n.transition), k = G, tr(Q), k;
}
const rc = (e) => {
  let t;
  for (const n in e)
    (n === "class" || n === "style" || ur(n)) && ((t || (t = {}))[n] = e[n]);
  return t;
}, sc = (e, t) => {
  const n = {};
  for (const r in e)
    (!ds(r) || !(r.slice(9) in t)) && (n[r] = e[r]);
  return n;
};
function ic(e, t, n) {
  const { props: r, children: s, component: i } = e, { props: o, children: a, patchFlag: l } = t, c = i.emitsOptions;
  if (t.dirs || t.transition)
    return !0;
  if (n && l >= 0) {
    if (l & 1024)
      return !0;
    if (l & 16)
      return r ? ni(r, o, c) : !!o;
    if (l & 8) {
      const u = t.dynamicProps;
      for (let h = 0; h < u.length; h++) {
        const g = u[h];
        if (o[g] !== r[g] && !xr(c, g))
          return !0;
      }
    }
  } else
    return (s || a) && (!a || !a.$stable) ? !0 : r === o ? !1 : r ? o ? ni(r, o, c) : !0 : !!o;
  return !1;
}
function ni(e, t, n) {
  const r = Object.keys(t);
  if (r.length !== Object.keys(e).length)
    return !0;
  for (let s = 0; s < r.length; s++) {
    const i = r[s];
    if (t[i] !== e[i] && !xr(n, i))
      return !0;
  }
  return !1;
}
function oc({ vnode: e, parent: t }, n) {
  for (; t; ) {
    const r = t.subTree;
    if (r.suspense && r.suspense.activeBranch === e && (r.el = e.el), r === e)
      (e = t.vnode).el = n, t = t.parent;
    else
      break;
  }
}
const Oo = (e) => e.__isSuspense;
function ac(e, t) {
  t && t.pendingBranch ? _(e) ? t.effects.push(...e) : t.effects.push(e) : Ml(e);
}
const ge = Symbol.for("v-fgt"), Vr = Symbol.for("v-txt"), Bt = Symbol.for("v-cmt"), Jn = Symbol.for("v-stc"), wn = [];
let je = null;
function U(e = !1) {
  wn.push(je = e ? null : []);
}
function lc() {
  wn.pop(), je = wn[wn.length - 1] || null;
}
let Vn = 1;
function ri(e, t = !1) {
  Vn += e, e < 0 && je && t && (je.hasOnce = !0);
}
function Go(e) {
  return e.dynamicChildren = Vn > 0 ? je || Ht : null, lc(), Vn > 0 && je && je.push(e), e;
}
function T(e, t, n, r, s, i) {
  return Go(
    b(
      e,
      t,
      n,
      r,
      s,
      i,
      !0
    )
  );
}
function dt(e, t, n, r, s) {
  return Go(
    H(
      e,
      t,
      n,
      r,
      s,
      !0
    )
  );
}
function nr(e) {
  return e ? e.__v_isVNode === !0 : !1;
}
function cn(e, t) {
  return e.type === t.type && e.key === t.key;
}
const Yo = ({ key: e }) => e ?? null, qn = ({
  ref: e,
  ref_key: t,
  ref_for: n
}) => (typeof e == "number" && (e = "" + e), e != null ? be(e) || xe(e) || le(e) ? { i: We, r: e, k: t, f: !!n } : e : null);
function b(e, t = null, n = null, r = 0, s = null, i = e === ge ? 0 : 1, o = !1, a = !1) {
  const l = {
    __v_isVNode: !0,
    __v_skip: !0,
    type: e,
    props: t,
    key: t && Yo(t),
    ref: t && qn(t),
    scopeId: Co,
    slotScopeIds: null,
    children: n,
    component: null,
    suspense: null,
    ssContent: null,
    ssFallback: null,
    dirs: null,
    transition: null,
    el: null,
    anchor: null,
    target: null,
    targetStart: null,
    targetAnchor: null,
    staticCount: 0,
    shapeFlag: i,
    patchFlag: r,
    dynamicProps: s,
    dynamicChildren: null,
    appContext: null,
    ctx: We
  };
  return a ? (Is(l, n), i & 128 && e.normalize(l)) : n && (l.shapeFlag |= be(n) ? 8 : 16), Vn > 0 && // avoid a block node from tracking itself
  !o && // has current parent block
  je && // presence of a patch flag indicates this node needs patching on updates.
  // component nodes also should always be patched, because even if the
  // component doesn't need to update, it needs to persist the instance on to
  // the next vnode so that it can be properly unmounted later.
  (l.patchFlag > 0 || i & 6) && // the EVENTS flag is only for hydration and if it is the only flag, the
  // vnode should not be considered dynamic due to handler caching.
  l.patchFlag !== 32 && je.push(l), l;
}
const H = cc;
function cc(e, t = null, n = null, r = 0, s = null, i = !1) {
  if ((!e || e === Gl) && (e = Bt), nr(e)) {
    const a = Kt(
      e,
      t,
      !0
      /* mergeRef: true */
    );
    return n && Is(a, n), Vn > 0 && !i && je && (a.shapeFlag & 6 ? je[je.indexOf(e)] = a : je.push(a)), a.patchFlag = -2, a;
  }
  if (bc(e) && (e = e.__vccOpts), t) {
    t = uc(t);
    let { class: a, style: l } = t;
    a && !be(a) && (t.class = me(a)), we(l) && (bs(l) && !_(l) && (l = Pe({}, l)), t.style = Te(l));
  }
  const o = be(e) ? 1 : Oo(e) ? 128 : kl(e) ? 64 : we(e) ? 4 : le(e) ? 2 : 0;
  return b(
    e,
    t,
    n,
    r,
    s,
    o,
    i,
    !0
  );
}
function uc(e) {
  return e ? bs(e) || Mo(e) ? Pe({}, e) : e : null;
}
function Kt(e, t, n = !1, r = !1) {
  const { props: s, ref: i, patchFlag: o, children: a, transition: l } = e, c = t ? To(s || {}, t) : s, u = {
    __v_isVNode: !0,
    __v_skip: !0,
    type: e.type,
    props: c,
    key: c && Yo(c),
    ref: t && t.ref ? (
      // #2078 in the case of <component :is="vnode" ref="extra"/>
      // if the vnode itself already has a ref, cloneVNode will need to merge
      // the refs so the single vnode can be set on multiple refs
      n && i ? _(i) ? i.concat(qn(t)) : [i, qn(t)] : qn(t)
    ) : i,
    scopeId: e.scopeId,
    slotScopeIds: e.slotScopeIds,
    children: a,
    target: e.target,
    targetStart: e.targetStart,
    targetAnchor: e.targetAnchor,
    staticCount: e.staticCount,
    shapeFlag: e.shapeFlag,
    // if the vnode is cloned with extra props, we can no longer assume its
    // existing patch flag to be reliable and need to add the FULL_PROPS flag.
    // note: preserve flag for fragments since they use the flag for children
    // fast paths only.
    patchFlag: t && e.type !== ge ? o === -1 ? 16 : o | 16 : o,
    dynamicProps: e.dynamicProps,
    dynamicChildren: e.dynamicChildren,
    appContext: e.appContext,
    dirs: e.dirs,
    transition: l,
    // These should technically only be non-null on mounted VNodes. However,
    // they *should* be copied for kept-alive vnodes. So we just always copy
    // them since them being non-null during a mount doesn't affect the logic as
    // they will simply be overwritten.
    component: e.component,
    suspense: e.suspense,
    ssContent: e.ssContent && Kt(e.ssContent),
    ssFallback: e.ssFallback && Kt(e.ssFallback),
    placeholder: e.placeholder,
    el: e.el,
    anchor: e.anchor,
    ctx: e.ctx,
    ce: e.ce
  };
  return l && r && Vs(
    u,
    l.clone(u)
  ), u;
}
function fc(e = " ", t = 0) {
  return H(Vr, null, e, t);
}
function hc(e, t) {
  const n = H(Jn, null, e);
  return n.staticCount = t, n;
}
function ye(e = "", t = !1) {
  return t ? (U(), dt(Bt, null, e)) : H(Bt, null, e);
}
function st(e) {
  return e == null || typeof e == "boolean" ? H(Bt) : _(e) ? H(
    ge,
    null,
    // #3666, avoid reference pollution when reusing vnode
    e.slice()
  ) : nr(e) ? At(e) : H(Vr, null, String(e));
}
function At(e) {
  return e.el === null && e.patchFlag !== -1 || e.memo ? e : Kt(e);
}
function Is(e, t) {
  let n = 0;
  const { shapeFlag: r } = e;
  if (t == null)
    t = null;
  else if (_(t))
    n = 16;
  else if (typeof t == "object")
    if (r & 65) {
      const s = t.default;
      s && (s._c && (s._d = !1), Is(e, s()), s._c && (s._d = !0));
      return;
    } else {
      n = 32;
      const s = t._;
      !s && !Mo(t) ? t._ctx = We : s === 3 && We && (We.slots._ === 1 ? t._ = 1 : (t._ = 2, e.patchFlag |= 1024));
    }
  else le(t) ? (t = { default: t, _ctx: We }, n = 32) : (t = String(t), r & 64 ? (n = 16, t = [fc(t)]) : n = 8);
  e.children = t, e.shapeFlag |= n;
}
function To(...e) {
  const t = {};
  for (let n = 0; n < e.length; n++) {
    const r = e[n];
    for (const s in r)
      if (s === "class")
        t.class !== r.class && (t.class = me([t.class, r.class]));
      else if (s === "style")
        t.style = Te([t.style, r.style]);
      else if (ur(s)) {
        const i = t[s], o = r[s];
        o && i !== o && !(_(i) && i.includes(o)) && (t[s] = i ? [].concat(i, o) : o);
      } else s !== "" && (t[s] = r[s]);
  }
  return t;
}
function nt(e, t, n, r = null) {
  wt(e, t, 7, [
    n,
    r
  ]);
}
const dc = Eo();
let pc = 0;
function gc(e, t, n) {
  const r = e.type, s = (t ? t.appContext : e.appContext) || dc, i = {
    uid: pc++,
    vnode: e,
    type: r,
    parent: t,
    appContext: s,
    root: null,
    // to be immediately set
    next: null,
    subTree: null,
    // will be set synchronously right after creation
    effect: null,
    update: null,
    // will be set synchronously right after creation
    job: null,
    scope: new za(
      !0
      /* detached */
    ),
    render: null,
    proxy: null,
    exposed: null,
    exposeProxy: null,
    withProxy: null,
    provides: t ? t.provides : Object.create(s.provides),
    ids: t ? t.ids : ["", 0, 0],
    accessCache: null,
    renderCache: [],
    // local resolved assets
    components: null,
    directives: null,
    // resolved props and emits options
    propsOptions: Ll(r, s),
    emitsOptions: nc(r, s),
    // emit
    emit: null,
    // to be set immediately
    emitted: null,
    // props default value
    propsDefaults: ae,
    // inheritAttrs
    inheritAttrs: r.inheritAttrs,
    // state
    ctx: ae,
    data: ae,
    props: ae,
    attrs: ae,
    slots: ae,
    refs: ae,
    setupState: ae,
    setupContext: null,
    // suspense related
    suspense: n,
    suspenseId: n ? n.pendingId : 0,
    asyncDep: null,
    asyncResolved: !1,
    // lifecycle hooks
    // not using enums here because it results in computed properties
    isMounted: !1,
    isUnmounted: !1,
    isDeactivated: !1,
    bc: null,
    c: null,
    bm: null,
    m: null,
    bu: null,
    u: null,
    um: null,
    bum: null,
    da: null,
    a: null,
    rtg: null,
    rtc: null,
    ec: null,
    sp: null
  };
  return i.ctx = { _: i }, i.root = t ? t.root : i, i.emit = tc.bind(null, i), e.ce && e.ce(i), i;
}
let Ye = null;
const Ms = () => Ye || We;
let rr, zr;
{
  const e = pr(), t = (n, r) => {
    let s;
    return (s = e[n]) || (s = e[n] = []), s.push(r), (i) => {
      s.length > 1 ? s.forEach((o) => o(i)) : s[0](i);
    };
  };
  rr = t(
    "__VUE_INSTANCE_SETTERS__",
    (n) => Ye = n
  ), zr = t(
    "__VUE_SSR_SETTERS__",
    (n) => Cn = n
  );
}
const Bs = (e) => {
  const t = Ye;
  return rr(e), e.scope.on(), () => {
    e.scope.off(), rr(t);
  };
}, si = () => {
  Ye && Ye.scope.off(), rr(null);
};
function jo(e) {
  return e.vnode.shapeFlag & 4;
}
let Cn = !1;
function mc(e, t = !1, n = !1) {
  t && zr(t);
  const { props: r, children: s } = e.vnode, i = jo(e);
  Kl(e, r, i, t), Jl(e, s, n || t);
  const o = i ? vc(e, t) : void 0;
  return t && zr(!1), o;
}
function vc(e, t) {
  const n = e.type;
  e.accessCache = /* @__PURE__ */ Object.create(null), e.proxy = new Proxy(e.ctx, Yl);
  const { setup: r } = n;
  if (r) {
    St();
    const s = e.setupContext = r.length > 1 ? yc(e) : null, i = Bs(e), o = Fn(
      r,
      e,
      0,
      [
        e.props,
        s
      ]
    ), a = Wi(o);
    if (It(), i(), (a || e.sp) && !mn(e) && Ql(e), a) {
      if (o.then(si, si), t)
        return o.then((l) => {
          ii(e, l);
        }).catch((l) => {
          br(l, e, 0);
        });
      e.asyncDep = o;
    } else
      ii(e, o);
  } else
    Po(e);
}
function ii(e, t, n) {
  le(t) ? e.type.__ssrInlineRender ? e.ssrRender = t : e.render = t : we(t) && (e.setupState = wo(t)), Po(e);
}
function Po(e, t, n) {
  const r = e.type;
  e.render || (e.render = r.render || Yt);
}
const wc = {
  get(e, t) {
    return Ie(e, "get", ""), e[t];
  }
};
function yc(e) {
  const t = (n) => {
    e.exposed = n || {};
  };
  return {
    attrs: new Proxy(e.attrs, wc),
    slots: e.slots,
    emit: e.emit,
    expose: t
  };
}
function Cr(e) {
  return e.exposed ? e.exposeProxy || (e.exposeProxy = new Proxy(wo(vl(e.exposed)), {
    get(t, n) {
      if (n in t)
        return t[n];
      if (n in vn)
        return vn[n](e);
    },
    has(t, n) {
      return n in t || n in vn;
    }
  })) : e.proxy;
}
function bc(e) {
  return le(e) && "__vccOpts" in e;
}
const j = (e, t) => Vl(e, t, Cn);
function it(e, t, n) {
  const r = arguments.length;
  return r === 2 ? we(t) && !_(t) ? nr(t) ? H(e, null, [t]) : H(e, t) : H(e, null, t) : (r > 3 ? n = Array.prototype.slice.call(arguments, 2) : r === 3 && nr(n) && (n = [n]), H(e, t, n));
}
const Ac = "3.5.20";
/**
* @vue/runtime-dom v3.5.20
* (c) 2018-present Yuxi (Evan) You and Vue contributors
* @license MIT
**/
let Xr;
const oi = typeof window < "u" && window.trustedTypes;
if (oi)
  try {
    Xr = /* @__PURE__ */ oi.createPolicy("vue", {
      createHTML: (e) => e
    });
  } catch {
  }
const Ko = Xr ? (e) => Xr.createHTML(e) : (e) => e, xc = "http://www.w3.org/2000/svg", Vc = "http://www.w3.org/1998/Math/MathML", ut = typeof document < "u" ? document : null, ai = ut && /* @__PURE__ */ ut.createElement("template"), Cc = {
  insert: (e, t, n) => {
    t.insertBefore(e, n || null);
  },
  remove: (e) => {
    const t = e.parentNode;
    t && t.removeChild(e);
  },
  createElement: (e, t, n, r) => {
    const s = t === "svg" ? ut.createElementNS(xc, e) : t === "mathml" ? ut.createElementNS(Vc, e) : n ? ut.createElement(e, { is: n }) : ut.createElement(e);
    return e === "select" && r && r.multiple != null && s.setAttribute("multiple", r.multiple), s;
  },
  createText: (e) => ut.createTextNode(e),
  createComment: (e) => ut.createComment(e),
  setText: (e, t) => {
    e.nodeValue = t;
  },
  setElementText: (e, t) => {
    e.textContent = t;
  },
  parentNode: (e) => e.parentNode,
  nextSibling: (e) => e.nextSibling,
  querySelector: (e) => ut.querySelector(e),
  setScopeId(e, t) {
    e.setAttribute(t, "");
  },
  // __UNSAFE__
  // Reason: innerHTML.
  // Static content here can only come from compiled templates.
  // As long as the user only uses trusted templates, this is safe.
  insertStaticContent(e, t, n, r, s, i) {
    const o = n ? n.previousSibling : t.lastChild;
    if (s && (s === i || s.nextSibling))
      for (; t.insertBefore(s.cloneNode(!0), n), !(s === i || !(s = s.nextSibling)); )
        ;
    else {
      ai.innerHTML = Ko(
        r === "svg" ? `<svg>${e}</svg>` : r === "mathml" ? `<math>${e}</math>` : e
      );
      const a = ai.content;
      if (r === "svg" || r === "mathml") {
        const l = a.firstChild;
        for (; l.firstChild; )
          a.appendChild(l.firstChild);
        a.removeChild(l);
      }
      t.insertBefore(a, n);
    }
    return [
      // first
      o ? o.nextSibling : t.firstChild,
      // last
      n ? n.previousSibling : t.lastChild
    ];
  }
}, Ec = Symbol("_vtc");
function Sc(e, t, n) {
  const r = e[Ec];
  r && (t = (t ? [t, ...r] : [...r]).join(" ")), t == null ? e.removeAttribute("class") : n ? e.setAttribute("class", t) : e.className = t;
}
const sr = Symbol("_vod"), No = Symbol("_vsh"), Lo = {
  // used for prop mismatch check during hydration
  name: "show",
  beforeMount(e, { value: t }, { transition: n }) {
    e[sr] = e.style.display === "none" ? "" : e.style.display, n && t ? n.beforeEnter(e) : un(e, t);
  },
  mounted(e, { value: t }, { transition: n }) {
    n && t && n.enter(e);
  },
  updated(e, { value: t, oldValue: n }, { transition: r }) {
    !t != !n && (r ? t ? (r.beforeEnter(e), un(e, !0), r.enter(e)) : r.leave(e, () => {
      un(e, !1);
    }) : un(e, t));
  },
  beforeUnmount(e, { value: t }) {
    un(e, t);
  }
};
function un(e, t) {
  e.style.display = t ? e[sr] : "none", e[No] = !t;
}
const Ic = Symbol(""), Mc = /(^|;)\s*display\s*:/;
function Bc(e, t, n) {
  const r = e.style, s = be(n);
  let i = !1;
  if (n && !s) {
    if (t)
      if (be(t))
        for (const o of t.split(";")) {
          const a = o.slice(0, o.indexOf(":")).trim();
          n[a] == null && Zn(r, a, "");
        }
      else
        for (const o in t)
          n[o] == null && Zn(r, o, "");
    for (const o in n)
      o === "display" && (i = !0), Zn(r, o, n[o]);
  } else if (s) {
    if (t !== n) {
      const o = r[Ic];
      o && (n += ";" + o), r.cssText = n, i = Mc.test(n);
    }
  } else t && e.removeAttribute("style");
  sr in e && (e[sr] = i ? r.display : "", e[No] && (r.display = "none"));
}
const li = /\s*!important$/;
function Zn(e, t, n) {
  if (_(n))
    n.forEach((r) => Zn(e, t, r));
  else if (n == null && (n = ""), t.startsWith("--"))
    e.setProperty(t, n);
  else {
    const r = kc(e, t);
    li.test(n) ? e.setProperty(
      Ge(r),
      n.replace(li, ""),
      "important"
    ) : e[r] = n;
  }
}
const ci = ["Webkit", "Moz", "ms"], Yr = {};
function kc(e, t) {
  const n = Yr[t];
  if (n)
    return n;
  let r = He(t);
  if (r !== "filter" && r in e)
    return Yr[t] = r;
  r = qi(r);
  for (let s = 0; s < ci.length; s++) {
    const i = ci[s] + r;
    if (i in e)
      return Yr[t] = i;
  }
  return t;
}
const ui = "http://www.w3.org/1999/xlink";
function fi(e, t, n, r, s, i = Ha(t)) {
  r && t.startsWith("xlink:") ? n == null ? e.removeAttributeNS(ui, t.slice(6, t.length)) : e.setAttributeNS(ui, t, n) : n == null || i && !Hi(n) ? e.removeAttribute(t) : e.setAttribute(
    t,
    i ? "" : kt(n) ? String(n) : n
  );
}
function hi(e, t, n, r, s) {
  if (t === "innerHTML" || t === "textContent") {
    n != null && (e[t] = t === "innerHTML" ? Ko(n) : n);
    return;
  }
  const i = e.tagName;
  if (t === "value" && i !== "PROGRESS" && // custom elements may use _value internally
  !i.includes("-")) {
    const a = i === "OPTION" ? e.getAttribute("value") || "" : e.value, l = n == null ? (
      // #11647: value should be set as empty string for null and undefined,
      // but <input type="checkbox"> should be set as 'on'.
      e.type === "checkbox" ? "on" : ""
    ) : String(n);
    (a !== l || !("_value" in e)) && (e.value = l), n == null && e.removeAttribute(t), e._value = n;
    return;
  }
  let o = !1;
  if (n === "" || n == null) {
    const a = typeof e[t];
    a === "boolean" ? n = Hi(n) : n == null && a === "string" ? (n = "", o = !0) : a === "number" && (n = 0, o = !0);
  }
  try {
    e[t] = n;
  } catch {
  }
  o && e.removeAttribute(s || t);
}
function Zt(e, t, n, r) {
  e.addEventListener(t, n, r);
}
function Rc(e, t, n, r) {
  e.removeEventListener(t, n, r);
}
const di = Symbol("_vei");
function Fc(e, t, n, r, s = null) {
  const i = e[di] || (e[di] = {}), o = i[t];
  if (r && o)
    o.value = r;
  else {
    const [a, l] = Qc(t);
    if (r) {
      const c = i[t] = Oc(
        r,
        s
      );
      Zt(e, a, c, l);
    } else o && (Rc(e, a, o, l), i[t] = void 0);
  }
}
const pi = /(?:Once|Passive|Capture)$/;
function Qc(e) {
  let t;
  if (pi.test(e)) {
    t = {};
    let r;
    for (; r = e.match(pi); )
      e = e.slice(0, e.length - r[0].length), t[r[0].toLowerCase()] = !0;
  }
  return [e[2] === ":" ? e.slice(3) : Ge(e.slice(2)), t];
}
let Tr = 0;
const Uc = /* @__PURE__ */ Promise.resolve(), Dc = () => Tr || (Uc.then(() => Tr = 0), Tr = Date.now());
function Oc(e, t) {
  const n = (r) => {
    if (!r._vts)
      r._vts = Date.now();
    else if (r._vts <= n.attached)
      return;
    wt(
      Gc(r, n.value),
      t,
      5,
      [r]
    );
  };
  return n.value = e, n.attached = Dc(), n;
}
function Gc(e, t) {
  if (_(t)) {
    const n = e.stopImmediatePropagation;
    return e.stopImmediatePropagation = () => {
      n.call(e), e._stopped = !0;
    }, t.map(
      (r) => (s) => !s._stopped && r && r(s)
    );
  } else
    return t;
}
const gi = (e) => e.charCodeAt(0) === 111 && e.charCodeAt(1) === 110 && // lowercase letter
e.charCodeAt(2) > 96 && e.charCodeAt(2) < 123, Yc = (e, t, n, r, s, i) => {
  const o = s === "svg";
  t === "class" ? Sc(e, r, o) : t === "style" ? Bc(e, n, r) : ur(t) ? ds(t) || Fc(e, t, n, r, i) : (t[0] === "." ? (t = t.slice(1), !0) : t[0] === "^" ? (t = t.slice(1), !1) : Tc(e, t, r, o)) ? (hi(e, t, r), !e.tagName.includes("-") && (t === "value" || t === "checked" || t === "selected") && fi(e, t, r, o, i, t !== "value")) : /* #11081 force set props for possible async custom element */ e._isVueCE && (/[A-Z]/.test(t) || !be(r)) ? hi(e, He(t), r, i, t) : (t === "true-value" ? e._trueValue = r : t === "false-value" && (e._falseValue = r), fi(e, t, r, o));
};
function Tc(e, t, n, r) {
  if (r)
    return !!(t === "innerHTML" || t === "textContent" || t in e && gi(t) && le(n));
  if (t === "spellcheck" || t === "draggable" || t === "translate" || t === "autocorrect" || t === "form" || t === "list" && e.tagName === "INPUT" || t === "type" && e.tagName === "TEXTAREA")
    return !1;
  if (t === "width" || t === "height") {
    const s = e.tagName;
    if (s === "IMG" || s === "VIDEO" || s === "CANVAS" || s === "SOURCE")
      return !1;
  }
  return gi(t) && be(n) ? !1 : t in e;
}
const mi = {};
/*! #__NO_SIDE_EFFECTS__ */
// @__NO_SIDE_EFFECTS__
function jc(e, t, n) {
  const r = /* @__PURE__ */ Me(e, t);
  hr(r) && Pe(r, t);
  class s extends ks {
    constructor(o) {
      super(r, o, n);
    }
  }
  return s.def = r, s;
}
const Pc = typeof HTMLElement < "u" ? HTMLElement : class {
};
class ks extends Pc {
  constructor(t, n = {}, r = Ai) {
    super(), this._def = t, this._props = n, this._createApp = r, this._isVueCE = !0, this._instance = null, this._app = null, this._nonce = this._def.nonce, this._connected = !1, this._resolved = !1, this._numberProps = null, this._styleChildren = /* @__PURE__ */ new WeakSet(), this._ob = null, this.shadowRoot && r !== Ai ? this._root = this.shadowRoot : t.shadowRoot !== !1 ? (this.attachShadow({ mode: "open" }), this._root = this.shadowRoot) : this._root = this;
  }
  connectedCallback() {
    if (!this.isConnected) return;
    !this.shadowRoot && !this._resolved && this._parseSlots(), this._connected = !0;
    let t = this;
    for (; t = t && (t.parentNode || t.host); )
      if (t instanceof ks) {
        this._parent = t;
        break;
      }
    this._instance || (this._resolved ? this._mount(this._def) : t && t._pendingResolve ? this._pendingResolve = t._pendingResolve.then(() => {
      this._pendingResolve = void 0, this._resolveDef();
    }) : this._resolveDef());
  }
  _setParent(t = this._parent) {
    t && (this._instance.parent = t._instance, this._inheritParentContext(t));
  }
  _inheritParentContext(t = this._parent) {
    t && this._app && Object.setPrototypeOf(
      this._app._context.provides,
      t._instance.provides
    );
  }
  disconnectedCallback() {
    this._connected = !1, Ne(() => {
      this._connected || (this._ob && (this._ob.disconnect(), this._ob = null), this._app && this._app.unmount(), this._instance && (this._instance.ce = void 0), this._app = this._instance = null);
    });
  }
  /**
   * resolve inner component definition (handle possible async component)
   */
  _resolveDef() {
    if (this._pendingResolve)
      return;
    for (let r = 0; r < this.attributes.length; r++)
      this._setAttr(this.attributes[r].name);
    this._ob = new MutationObserver((r) => {
      for (const s of r)
        this._setAttr(s.attributeName);
    }), this._ob.observe(this, { attributes: !0 });
    const t = (r, s = !1) => {
      this._resolved = !0, this._pendingResolve = void 0;
      const { props: i, styles: o } = r;
      let a;
      if (i && !_(i))
        for (const l in i) {
          const c = i[l];
          (c === Number || c && c.type === Number) && (l in this._props && (this._props[l] = qs(this._props[l])), (a || (a = /* @__PURE__ */ Object.create(null)))[He(l)] = !0);
        }
      this._numberProps = a, this._resolveProps(r), this.shadowRoot && this._applyStyles(o), this._mount(r);
    }, n = this._def.__asyncLoader;
    n ? this._pendingResolve = n().then((r) => {
      r.configureApp = this._def.configureApp, t(this._def = r, !0);
    }) : t(this._def);
  }
  _mount(t) {
    this._app = this._createApp(t), this._inheritParentContext(), t.configureApp && t.configureApp(this._app), this._app._ceVNode = this._createVNode(), this._app.mount(this._root);
    const n = this._instance && this._instance.exposed;
    if (n)
      for (const r in n)
        re(this, r) || Object.defineProperty(this, r, {
          // unwrap ref to be consistent with public instance behavior
          get: () => ee(n[r])
        });
  }
  _resolveProps(t) {
    const { props: n } = t, r = _(n) ? n : Object.keys(n || {});
    for (const s of Object.keys(this))
      s[0] !== "_" && r.includes(s) && this._setProp(s, this[s]);
    for (const s of r.map(He))
      Object.defineProperty(this, s, {
        get() {
          return this._getProp(s);
        },
        set(i) {
          this._setProp(s, i, !0, !0);
        }
      });
  }
  _setAttr(t) {
    if (t.startsWith("data-v-")) return;
    const n = this.hasAttribute(t);
    let r = n ? this.getAttribute(t) : mi;
    const s = He(t);
    n && this._numberProps && this._numberProps[s] && (r = qs(r)), this._setProp(s, r, !1, !0);
  }
  /**
   * @internal
   */
  _getProp(t) {
    return this._props[t];
  }
  /**
   * @internal
   */
  _setProp(t, n, r = !0, s = !1) {
    if (n !== this._props[t] && (n === mi ? delete this._props[t] : (this._props[t] = n, t === "key" && this._app && (this._app._ceVNode.key = n)), s && this._instance && this._update(), r)) {
      const i = this._ob;
      i && i.disconnect(), n === !0 ? this.setAttribute(Ge(t), "") : typeof n == "string" || typeof n == "number" ? this.setAttribute(Ge(t), n + "") : n || this.removeAttribute(Ge(t)), i && i.observe(this, { attributes: !0 });
    }
  }
  _update() {
    const t = this._createVNode();
    this._app && (t.appContext = this._app._context), Zc(t, this._root);
  }
  _createVNode() {
    const t = {};
    this.shadowRoot || (t.onVnodeMounted = t.onVnodeUpdated = this._renderSlots.bind(this));
    const n = H(this._def, Pe(t, this._props));
    return this._instance || (n.ce = (r) => {
      this._instance = r, r.ce = this, r.isCE = !0;
      const s = (i, o) => {
        this.dispatchEvent(
          new CustomEvent(
            i,
            hr(o[0]) ? Pe({ detail: o }, o[0]) : { detail: o }
          )
        );
      };
      r.emit = (i, ...o) => {
        s(i, o), Ge(i) !== i && s(Ge(i), o);
      }, this._setParent();
    }), n;
  }
  _applyStyles(t, n) {
    if (!t) return;
    if (n) {
      if (n === this._def || this._styleChildren.has(n))
        return;
      this._styleChildren.add(n);
    }
    const r = this._nonce;
    for (let s = t.length - 1; s >= 0; s--) {
      const i = document.createElement("style");
      r && i.setAttribute("nonce", r), i.textContent = t[s], this.shadowRoot.prepend(i);
    }
  }
  /**
   * Only called when shadowRoot is false
   */
  _parseSlots() {
    const t = this._slots = {};
    let n;
    for (; n = this.firstChild; ) {
      const r = n.nodeType === 1 && n.getAttribute("slot") || "default";
      (t[r] || (t[r] = [])).push(n), this.removeChild(n);
    }
  }
  /**
   * Only called when shadowRoot is false
   */
  _renderSlots() {
    const t = (this._teleportTarget || this).querySelectorAll("slot"), n = this._instance.type.__scopeId;
    for (let r = 0; r < t.length; r++) {
      const s = t[r], i = s.getAttribute("name") || "default", o = this._slots[i], a = s.parentNode;
      if (o)
        for (const l of o) {
          if (n && l.nodeType === 1) {
            const c = n + "-s", u = document.createTreeWalker(l, 1);
            l.setAttribute(c, "");
            let h;
            for (; h = u.nextNode(); )
              h.setAttribute(c, "");
          }
          a.insertBefore(l, s);
        }
      else
        for (; s.firstChild; ) a.insertBefore(s.firstChild, s);
      a.removeChild(s);
    }
  }
  /**
   * @internal
   */
  _injectChildStyle(t) {
    this._applyStyles(t.styles, t);
  }
  /**
   * @internal
   */
  _removeChildStyle(t) {
  }
}
const vi = (e) => {
  const t = e.props["onUpdate:modelValue"] || !1;
  return _(t) ? (n) => Ln(t, n) : t;
};
function Kc(e) {
  e.target.composing = !0;
}
function wi(e) {
  const t = e.target;
  t.composing && (t.composing = !1, t.dispatchEvent(new Event("input")));
}
const jr = Symbol("_assign"), Nc = {
  created(e, { modifiers: { lazy: t, trim: n, number: r } }, s) {
    e[jr] = vi(s);
    const i = r || s.props && s.props.type === "number";
    Zt(e, t ? "change" : "input", (o) => {
      if (o.target.composing) return;
      let a = e.value;
      n && (a = a.trim()), i && (a = Lr(a)), e[jr](a);
    }), n && Zt(e, "change", () => {
      e.value = e.value.trim();
    }), t || (Zt(e, "compositionstart", Kc), Zt(e, "compositionend", wi), Zt(e, "change", wi));
  },
  // set value on mounted so it's after min/max for type="range"
  mounted(e, { value: t }) {
    e.value = t ?? "";
  },
  beforeUpdate(e, { value: t, oldValue: n, modifiers: { lazy: r, trim: s, number: i } }, o) {
    if (e[jr] = vi(o), e.composing) return;
    const a = (i || e.type === "number") && !/^0\d/.test(e.value) ? Lr(e.value) : e.value, l = t ?? "";
    a !== l && (document.activeElement === e && e.type !== "range" && (r && t === n || s && e.value.trim() === l) || (e.value = l));
  }
}, Lc = ["ctrl", "shift", "alt", "meta"], Wc = {
  stop: (e) => e.stopPropagation(),
  prevent: (e) => e.preventDefault(),
  self: (e) => e.target !== e.currentTarget,
  ctrl: (e) => !e.ctrlKey,
  shift: (e) => !e.shiftKey,
  alt: (e) => !e.altKey,
  meta: (e) => !e.metaKey,
  left: (e) => "button" in e && e.button !== 0,
  middle: (e) => "button" in e && e.button !== 1,
  right: (e) => "button" in e && e.button !== 2,
  exact: (e, t) => Lc.some((n) => e[`${n}Key`] && !t.includes(n))
}, En = (e, t) => {
  const n = e._withMods || (e._withMods = {}), r = t.join(".");
  return n[r] || (n[r] = ((s, ...i) => {
    for (let o = 0; o < t.length; o++) {
      const a = Wc[t[o]];
      if (a && a(s, t)) return;
    }
    return e(s, ...i);
  }));
}, Jc = {
  esc: "escape",
  space: " ",
  up: "arrow-up",
  left: "arrow-left",
  right: "arrow-right",
  down: "arrow-down",
  delete: "backspace"
}, yi = (e, t) => {
  const n = e._withKeys || (e._withKeys = {}), r = t.join(".");
  return n[r] || (n[r] = ((s) => {
    if (!("key" in s))
      return;
    const i = Ge(s.key);
    if (t.some(
      (o) => o === i || Jc[o] === i
    ))
      return e(s);
  }));
}, qc = /* @__PURE__ */ Pe({ patchProp: Yc }, Cc);
let bi;
function Wo() {
  return bi || (bi = Zl(qc));
}
const Zc = ((...e) => {
  Wo().render(...e);
}), Ai = ((...e) => {
  const t = Wo().createApp(...e), { mount: n } = t;
  return t.mount = (r) => {
    const s = zc(r);
    if (!s) return;
    const i = t._component;
    !le(i) && !i.render && !i.template && (i.template = s.innerHTML), s.nodeType === 1 && (s.textContent = "");
    const o = n(s, !1, Hc(s));
    return s instanceof Element && (s.removeAttribute("v-cloak"), s.setAttribute("data-v-app", "")), o;
  }, t;
});
function Hc(e) {
  if (e instanceof SVGElement)
    return "svg";
  if (typeof MathMLElement == "function" && e instanceof MathMLElement)
    return "mathml";
}
function zc(e) {
  return be(e) ? document.querySelector(e) : e;
}
var nn = class {
  constructor() {
    this.listeners = /* @__PURE__ */ new Set(), this.subscribe = this.subscribe.bind(this);
  }
  subscribe(e) {
    return this.listeners.add(e), this.onSubscribe(), () => {
      this.listeners.delete(e), this.onUnsubscribe();
    };
  }
  hasListeners() {
    return this.listeners.size > 0;
  }
  onSubscribe() {
  }
  onUnsubscribe() {
  }
}, Nt = typeof window > "u" || "Deno" in globalThis;
function Ke() {
}
function Xc(e, t) {
  return typeof e == "function" ? e(t) : e;
}
function _r(e) {
  return typeof e == "number" && e >= 0 && e !== 1 / 0;
}
function Jo(e, t) {
  return Math.max(e + (t || 0) - Date.now(), 0);
}
function Et(e, t) {
  return typeof e == "function" ? e(t) : e;
}
function Le(e, t) {
  return typeof e == "function" ? e(t) : e;
}
function xi(e, t) {
  const {
    type: n = "all",
    exact: r,
    fetchStatus: s,
    predicate: i,
    queryKey: o,
    stale: a
  } = e;
  if (o) {
    if (r) {
      if (t.queryHash !== Rs(o, t.options))
        return !1;
    } else if (!Sn(t.queryKey, o))
      return !1;
  }
  if (n !== "all") {
    const l = t.isActive();
    if (n === "active" && !l || n === "inactive" && l)
      return !1;
  }
  return !(typeof a == "boolean" && t.isStale() !== a || s && s !== t.state.fetchStatus || i && !i(t));
}
function Vi(e, t) {
  const { exact: n, status: r, predicate: s, mutationKey: i } = e;
  if (i) {
    if (!t.options.mutationKey)
      return !1;
    if (n) {
      if (Lt(t.options.mutationKey) !== Lt(i))
        return !1;
    } else if (!Sn(t.options.mutationKey, i))
      return !1;
  }
  return !(r && t.state.status !== r || s && !s(t));
}
function Rs(e, t) {
  return (t?.queryKeyHashFn || Lt)(e);
}
function Lt(e) {
  return JSON.stringify(
    e,
    (t, n) => $r(n) ? Object.keys(n).sort().reduce((r, s) => (r[s] = n[s], r), {}) : n
  );
}
function Sn(e, t) {
  return e === t ? !0 : typeof e != typeof t ? !1 : e && t && typeof e == "object" && typeof t == "object" ? Object.keys(t).every((n) => Sn(e[n], t[n])) : !1;
}
var _c = Object.prototype.hasOwnProperty;
function qo(e, t) {
  if (e === t)
    return e;
  const n = Ci(e) && Ci(t);
  if (!n && !($r(e) && $r(t))) return t;
  const s = (n ? e : Object.keys(e)).length, i = n ? t : Object.keys(t), o = i.length, a = n ? new Array(o) : {};
  let l = 0;
  for (let c = 0; c < o; c++) {
    const u = n ? c : i[c], h = e[u], g = t[u];
    if (h === g) {
      a[u] = h, (n ? c < s : _c.call(e, u)) && l++;
      continue;
    }
    if (h === null || g === null || typeof h != "object" || typeof g != "object") {
      a[u] = g;
      continue;
    }
    const p = qo(h, g);
    a[u] = p, p === h && l++;
  }
  return s === o && l === s ? e : a;
}
function ir(e, t) {
  if (!t || Object.keys(e).length !== Object.keys(t).length)
    return !1;
  for (const n in e)
    if (e[n] !== t[n])
      return !1;
  return !0;
}
function Ci(e) {
  return Array.isArray(e) && e.length === Object.keys(e).length;
}
function $r(e) {
  if (!Ei(e))
    return !1;
  const t = e.constructor;
  if (t === void 0)
    return !0;
  const n = t.prototype;
  return !(!Ei(n) || !n.hasOwnProperty("isPrototypeOf") || Object.getPrototypeOf(e) !== Object.prototype);
}
function Ei(e) {
  return Object.prototype.toString.call(e) === "[object Object]";
}
function $c(e) {
  return new Promise((t) => {
    setTimeout(t, e);
  });
}
function es(e, t, n) {
  return typeof n.structuralSharing == "function" ? n.structuralSharing(e, t) : n.structuralSharing !== !1 ? qo(e, t) : t;
}
function eu(e, t, n = 0) {
  const r = [...e, t];
  return n && r.length > n ? r.slice(1) : r;
}
function tu(e, t, n = 0) {
  const r = [t, ...e];
  return n && r.length > n ? r.slice(0, -1) : r;
}
var Fs = Symbol();
function Zo(e, t) {
  return !e.queryFn && t?.initialPromise ? () => t.initialPromise : !e.queryFn || e.queryFn === Fs ? () => Promise.reject(new Error(`Missing queryFn: '${e.queryHash}'`)) : e.queryFn;
}
function ts(e, t) {
  return typeof e == "function" ? e(...t) : !!e;
}
var nu = class extends nn {
  #t;
  #e;
  #n;
  constructor() {
    super(), this.#n = (e) => {
      if (!Nt && window.addEventListener) {
        const t = () => e();
        return window.addEventListener("visibilitychange", t, !1), () => {
          window.removeEventListener("visibilitychange", t);
        };
      }
    };
  }
  onSubscribe() {
    this.#e || this.setEventListener(this.#n);
  }
  onUnsubscribe() {
    this.hasListeners() || (this.#e?.(), this.#e = void 0);
  }
  setEventListener(e) {
    this.#n = e, this.#e?.(), this.#e = e((t) => {
      typeof t == "boolean" ? this.setFocused(t) : this.onFocus();
    });
  }
  setFocused(e) {
    this.#t !== e && (this.#t = e, this.onFocus());
  }
  onFocus() {
    const e = this.isFocused();
    this.listeners.forEach((t) => {
      t(e);
    });
  }
  isFocused() {
    return typeof this.#t == "boolean" ? this.#t : globalThis.document?.visibilityState !== "hidden";
  }
}, Qs = new nu(), ru = class extends nn {
  #t = !0;
  #e;
  #n;
  constructor() {
    super(), this.#n = (e) => {
      if (!Nt && window.addEventListener) {
        const t = () => e(!0), n = () => e(!1);
        return window.addEventListener("online", t, !1), window.addEventListener("offline", n, !1), () => {
          window.removeEventListener("online", t), window.removeEventListener("offline", n);
        };
      }
    };
  }
  onSubscribe() {
    this.#e || this.setEventListener(this.#n);
  }
  onUnsubscribe() {
    this.hasListeners() || (this.#e?.(), this.#e = void 0);
  }
  setEventListener(e) {
    this.#n = e, this.#e?.(), this.#e = e(this.setOnline.bind(this));
  }
  setOnline(e) {
    this.#t !== e && (this.#t = e, this.listeners.forEach((n) => {
      n(e);
    }));
  }
  isOnline() {
    return this.#t;
  }
}, or = new ru();
function ns() {
  let e, t;
  const n = new Promise((s, i) => {
    e = s, t = i;
  });
  n.status = "pending", n.catch(() => {
  });
  function r(s) {
    Object.assign(n, s), delete n.resolve, delete n.reject;
  }
  return n.resolve = (s) => {
    r({
      status: "fulfilled",
      value: s
    }), e(s);
  }, n.reject = (s) => {
    r({
      status: "rejected",
      reason: s
    }), t(s);
  }, n;
}
function su(e) {
  return Math.min(1e3 * 2 ** e, 3e4);
}
function Ho(e) {
  return (e ?? "online") === "online" ? or.isOnline() : !0;
}
var rs = class extends Error {
  constructor(e) {
    super("CancelledError"), this.revert = e?.revert, this.silent = e?.silent;
  }
};
function zo(e) {
  let t = !1, n = 0, r;
  const s = ns(), i = () => s.status !== "pending", o = (m) => {
    if (!i()) {
      const Q = new rs(m);
      g(Q), e.onCancel?.(Q);
    }
  }, a = () => {
    t = !0;
  }, l = () => {
    t = !1;
  }, c = () => Qs.isFocused() && (e.networkMode === "always" || or.isOnline()) && e.canRun(), u = () => Ho(e.networkMode) && e.canRun(), h = (m) => {
    i() || (r?.(), s.resolve(m));
  }, g = (m) => {
    i() || (r?.(), s.reject(m));
  }, p = () => new Promise((m) => {
    r = (Q) => {
      (i() || c()) && m(Q);
    }, e.onPause?.();
  }).then(() => {
    r = void 0, i() || e.onContinue?.();
  }), w = () => {
    if (i())
      return;
    let m;
    const Q = n === 0 ? e.initialPromise : void 0;
    try {
      m = Q ?? e.fn();
    } catch (k) {
      m = Promise.reject(k);
    }
    Promise.resolve(m).then(h).catch((k) => {
      if (i())
        return;
      const M = e.retry ?? (Nt ? 0 : 3), G = e.retryDelay ?? su, V = typeof G == "function" ? G(n, k) : G, v = M === !0 || typeof M == "number" && n < M || typeof M == "function" && M(n, k);
      if (t || !v) {
        g(k);
        return;
      }
      n++, e.onFail?.(n, k), $c(V).then(() => c() ? void 0 : p()).then(() => {
        t ? g(k) : w();
      });
    });
  };
  return {
    promise: s,
    status: () => s.status,
    cancel: o,
    continue: () => (r?.(), s),
    cancelRetry: a,
    continueRetry: l,
    canStart: u,
    start: () => (u() ? w() : p().then(w), s)
  };
}
var iu = (e) => setTimeout(e, 0);
function ou() {
  let e = [], t = 0, n = (a) => {
    a();
  }, r = (a) => {
    a();
  }, s = iu;
  const i = (a) => {
    t ? e.push(a) : s(() => {
      n(a);
    });
  }, o = () => {
    const a = e;
    e = [], a.length && s(() => {
      r(() => {
        a.forEach((l) => {
          n(l);
        });
      });
    });
  };
  return {
    batch: (a) => {
      let l;
      t++;
      try {
        l = a();
      } finally {
        t--, t || o();
      }
      return l;
    },
    /**
     * All calls to the wrapped function will be batched.
     */
    batchCalls: (a) => (...l) => {
      i(() => {
        a(...l);
      });
    },
    schedule: i,
    /**
     * Use this method to set a custom notify function.
     * This can be used to for example wrap notifications with `React.act` while running tests.
     */
    setNotifyFunction: (a) => {
      n = a;
    },
    /**
     * Use this method to set a custom function to batch notifications together into a single tick.
     * By default React Query will use the batch function provided by ReactDOM or React Native.
     */
    setBatchNotifyFunction: (a) => {
      r = a;
    },
    setScheduler: (a) => {
      s = a;
    }
  };
}
var Ee = ou(), Xo = class {
  #t;
  destroy() {
    this.clearGcTimeout();
  }
  scheduleGc() {
    this.clearGcTimeout(), _r(this.gcTime) && (this.#t = setTimeout(() => {
      this.optionalRemove();
    }, this.gcTime));
  }
  updateGcTime(e) {
    this.gcTime = Math.max(
      this.gcTime || 0,
      e ?? (Nt ? 1 / 0 : 300 * 1e3)
    );
  }
  clearGcTimeout() {
    this.#t && (clearTimeout(this.#t), this.#t = void 0);
  }
}, au = class extends Xo {
  #t;
  #e;
  #n;
  #r;
  #s;
  #o;
  #a;
  constructor(e) {
    super(), this.#a = !1, this.#o = e.defaultOptions, this.setOptions(e.options), this.observers = [], this.#r = e.client, this.#n = this.#r.getQueryCache(), this.queryKey = e.queryKey, this.queryHash = e.queryHash, this.#t = lu(this.options), this.state = e.state ?? this.#t, this.scheduleGc();
  }
  get meta() {
    return this.options.meta;
  }
  get promise() {
    return this.#s?.promise;
  }
  setOptions(e) {
    this.options = { ...this.#o, ...e }, this.updateGcTime(this.options.gcTime);
  }
  optionalRemove() {
    !this.observers.length && this.state.fetchStatus === "idle" && this.#n.remove(this);
  }
  setData(e, t) {
    const n = es(this.state.data, e, this.options);
    return this.#i({
      data: n,
      type: "success",
      dataUpdatedAt: t?.updatedAt,
      manual: t?.manual
    }), n;
  }
  setState(e, t) {
    this.#i({ type: "setState", state: e, setStateOptions: t });
  }
  cancel(e) {
    const t = this.#s?.promise;
    return this.#s?.cancel(e), t ? t.then(Ke).catch(Ke) : Promise.resolve();
  }
  destroy() {
    super.destroy(), this.cancel({ silent: !0 });
  }
  reset() {
    this.destroy(), this.setState(this.#t);
  }
  isActive() {
    return this.observers.some(
      (e) => Le(e.options.enabled, this) !== !1
    );
  }
  isDisabled() {
    return this.getObserversCount() > 0 ? !this.isActive() : this.options.queryFn === Fs || this.state.dataUpdateCount + this.state.errorUpdateCount === 0;
  }
  isStatic() {
    return this.getObserversCount() > 0 ? this.observers.some(
      (e) => Et(e.options.staleTime, this) === "static"
    ) : !1;
  }
  isStale() {
    return this.getObserversCount() > 0 ? this.observers.some(
      (e) => e.getCurrentResult().isStale
    ) : this.state.data === void 0 || this.state.isInvalidated;
  }
  isStaleByTime(e = 0) {
    return this.state.data === void 0 ? !0 : e === "static" ? !1 : this.state.isInvalidated ? !0 : !Jo(this.state.dataUpdatedAt, e);
  }
  onFocus() {
    this.observers.find((t) => t.shouldFetchOnWindowFocus())?.refetch({ cancelRefetch: !1 }), this.#s?.continue();
  }
  onOnline() {
    this.observers.find((t) => t.shouldFetchOnReconnect())?.refetch({ cancelRefetch: !1 }), this.#s?.continue();
  }
  addObserver(e) {
    this.observers.includes(e) || (this.observers.push(e), this.clearGcTimeout(), this.#n.notify({ type: "observerAdded", query: this, observer: e }));
  }
  removeObserver(e) {
    this.observers.includes(e) && (this.observers = this.observers.filter((t) => t !== e), this.observers.length || (this.#s && (this.#a ? this.#s.cancel({ revert: !0 }) : this.#s.cancelRetry()), this.scheduleGc()), this.#n.notify({ type: "observerRemoved", query: this, observer: e }));
  }
  getObserversCount() {
    return this.observers.length;
  }
  invalidate() {
    this.state.isInvalidated || this.#i({ type: "invalidate" });
  }
  async fetch(e, t) {
    if (this.state.fetchStatus !== "idle" && // If the promise in the retyer is already rejected, we have to definitely
    // re-start the fetch; there is a chance that the query is still in a
    // pending state when that happens
    this.#s?.status() !== "rejected") {
      if (this.state.data !== void 0 && t?.cancelRefetch)
        this.cancel({ silent: !0 });
      else if (this.#s)
        return this.#s.continueRetry(), this.#s.promise;
    }
    if (e && this.setOptions(e), !this.options.queryFn) {
      const a = this.observers.find((l) => l.options.queryFn);
      a && this.setOptions(a.options);
    }
    const n = new AbortController(), r = (a) => {
      Object.defineProperty(a, "signal", {
        enumerable: !0,
        get: () => (this.#a = !0, n.signal)
      });
    }, s = () => {
      const a = Zo(this.options, t), c = (() => {
        const u = {
          client: this.#r,
          queryKey: this.queryKey,
          meta: this.meta
        };
        return r(u), u;
      })();
      return this.#a = !1, this.options.persister ? this.options.persister(
        a,
        c,
        this
      ) : a(c);
    }, o = (() => {
      const a = {
        fetchOptions: t,
        options: this.options,
        queryKey: this.queryKey,
        client: this.#r,
        state: this.state,
        fetchFn: s
      };
      return r(a), a;
    })();
    this.options.behavior?.onFetch(o, this), this.#e = this.state, (this.state.fetchStatus === "idle" || this.state.fetchMeta !== o.fetchOptions?.meta) && this.#i({ type: "fetch", meta: o.fetchOptions?.meta }), this.#s = zo({
      initialPromise: t?.initialPromise,
      fn: o.fetchFn,
      onCancel: (a) => {
        a instanceof rs && a.revert && this.setState({
          ...this.#e,
          fetchStatus: "idle"
        }), n.abort();
      },
      onFail: (a, l) => {
        this.#i({ type: "failed", failureCount: a, error: l });
      },
      onPause: () => {
        this.#i({ type: "pause" });
      },
      onContinue: () => {
        this.#i({ type: "continue" });
      },
      retry: o.options.retry,
      retryDelay: o.options.retryDelay,
      networkMode: o.options.networkMode,
      canRun: () => !0
    });
    try {
      const a = await this.#s.start();
      if (a === void 0)
        throw new Error(`${this.queryHash} data is undefined`);
      return this.setData(a), this.#n.config.onSuccess?.(a, this), this.#n.config.onSettled?.(
        a,
        this.state.error,
        this
      ), a;
    } catch (a) {
      if (a instanceof rs) {
        if (a.silent)
          return this.#s.promise;
        if (a.revert) {
          if (this.state.data === void 0)
            throw a;
          return this.state.data;
        }
      }
      throw this.#i({
        type: "error",
        error: a
      }), this.#n.config.onError?.(
        a,
        this
      ), this.#n.config.onSettled?.(
        this.state.data,
        a,
        this
      ), a;
    } finally {
      this.scheduleGc();
    }
  }
  #i(e) {
    const t = (n) => {
      switch (e.type) {
        case "failed":
          return {
            ...n,
            fetchFailureCount: e.failureCount,
            fetchFailureReason: e.error
          };
        case "pause":
          return {
            ...n,
            fetchStatus: "paused"
          };
        case "continue":
          return {
            ...n,
            fetchStatus: "fetching"
          };
        case "fetch":
          return {
            ...n,
            ..._o(n.data, this.options),
            fetchMeta: e.meta ?? null
          };
        case "success":
          const r = {
            ...n,
            data: e.data,
            dataUpdateCount: n.dataUpdateCount + 1,
            dataUpdatedAt: e.dataUpdatedAt ?? Date.now(),
            error: null,
            isInvalidated: !1,
            status: "success",
            ...!e.manual && {
              fetchStatus: "idle",
              fetchFailureCount: 0,
              fetchFailureReason: null
            }
          };
          return this.#e = e.manual ? r : void 0, r;
        case "error":
          const s = e.error;
          return {
            ...n,
            error: s,
            errorUpdateCount: n.errorUpdateCount + 1,
            errorUpdatedAt: Date.now(),
            fetchFailureCount: n.fetchFailureCount + 1,
            fetchFailureReason: s,
            fetchStatus: "idle",
            status: "error"
          };
        case "invalidate":
          return {
            ...n,
            isInvalidated: !0
          };
        case "setState":
          return {
            ...n,
            ...e.state
          };
      }
    };
    this.state = t(this.state), Ee.batch(() => {
      this.observers.forEach((n) => {
        n.onQueryUpdate();
      }), this.#n.notify({ query: this, type: "updated", action: e });
    });
  }
};
function _o(e, t) {
  return {
    fetchFailureCount: 0,
    fetchFailureReason: null,
    fetchStatus: Ho(t.networkMode) ? "fetching" : "paused",
    ...e === void 0 && {
      error: null,
      status: "pending"
    }
  };
}
function lu(e) {
  const t = typeof e.initialData == "function" ? e.initialData() : e.initialData, n = t !== void 0, r = n ? typeof e.initialDataUpdatedAt == "function" ? e.initialDataUpdatedAt() : e.initialDataUpdatedAt : 0;
  return {
    data: t,
    dataUpdateCount: 0,
    dataUpdatedAt: n ? r ?? Date.now() : 0,
    error: null,
    errorUpdateCount: 0,
    errorUpdatedAt: 0,
    fetchFailureCount: 0,
    fetchFailureReason: null,
    fetchMeta: null,
    isInvalidated: !1,
    status: n ? "success" : "pending",
    fetchStatus: "idle"
  };
}
var $o = class extends nn {
  constructor(t = {}) {
    super(), this.config = t, this.#t = /* @__PURE__ */ new Map();
  }
  #t;
  build(t, n, r) {
    const s = n.queryKey, i = n.queryHash ?? Rs(s, n);
    let o = this.get(i);
    return o || (o = new au({
      client: t,
      queryKey: s,
      queryHash: i,
      options: t.defaultQueryOptions(n),
      state: r,
      defaultOptions: t.getQueryDefaults(s)
    }), this.add(o)), o;
  }
  add(t) {
    this.#t.has(t.queryHash) || (this.#t.set(t.queryHash, t), this.notify({
      type: "added",
      query: t
    }));
  }
  remove(t) {
    const n = this.#t.get(t.queryHash);
    n && (t.destroy(), n === t && this.#t.delete(t.queryHash), this.notify({ type: "removed", query: t }));
  }
  clear() {
    Ee.batch(() => {
      this.getAll().forEach((t) => {
        this.remove(t);
      });
    });
  }
  get(t) {
    return this.#t.get(t);
  }
  getAll() {
    return [...this.#t.values()];
  }
  find(t) {
    const n = { exact: !0, ...t };
    return this.getAll().find(
      (r) => xi(n, r)
    );
  }
  findAll(t = {}) {
    const n = this.getAll();
    return Object.keys(t).length > 0 ? n.filter((r) => xi(t, r)) : n;
  }
  notify(t) {
    Ee.batch(() => {
      this.listeners.forEach((n) => {
        n(t);
      });
    });
  }
  onFocus() {
    Ee.batch(() => {
      this.getAll().forEach((t) => {
        t.onFocus();
      });
    });
  }
  onOnline() {
    Ee.batch(() => {
      this.getAll().forEach((t) => {
        t.onOnline();
      });
    });
  }
}, cu = class extends Xo {
  #t;
  #e;
  #n;
  constructor(e) {
    super(), this.mutationId = e.mutationId, this.#e = e.mutationCache, this.#t = [], this.state = e.state || ea(), this.setOptions(e.options), this.scheduleGc();
  }
  setOptions(e) {
    this.options = e, this.updateGcTime(this.options.gcTime);
  }
  get meta() {
    return this.options.meta;
  }
  addObserver(e) {
    this.#t.includes(e) || (this.#t.push(e), this.clearGcTimeout(), this.#e.notify({
      type: "observerAdded",
      mutation: this,
      observer: e
    }));
  }
  removeObserver(e) {
    this.#t = this.#t.filter((t) => t !== e), this.scheduleGc(), this.#e.notify({
      type: "observerRemoved",
      mutation: this,
      observer: e
    });
  }
  optionalRemove() {
    this.#t.length || (this.state.status === "pending" ? this.scheduleGc() : this.#e.remove(this));
  }
  continue() {
    return this.#n?.continue() ?? // continuing a mutation assumes that variables are set, mutation must have been dehydrated before
    this.execute(this.state.variables);
  }
  async execute(e) {
    const t = () => {
      this.#r({ type: "continue" });
    };
    this.#n = zo({
      fn: () => this.options.mutationFn ? this.options.mutationFn(e) : Promise.reject(new Error("No mutationFn found")),
      onFail: (s, i) => {
        this.#r({ type: "failed", failureCount: s, error: i });
      },
      onPause: () => {
        this.#r({ type: "pause" });
      },
      onContinue: t,
      retry: this.options.retry ?? 0,
      retryDelay: this.options.retryDelay,
      networkMode: this.options.networkMode,
      canRun: () => this.#e.canRun(this)
    });
    const n = this.state.status === "pending", r = !this.#n.canStart();
    try {
      if (n)
        t();
      else {
        this.#r({ type: "pending", variables: e, isPaused: r }), await this.#e.config.onMutate?.(
          e,
          this
        );
        const i = await this.options.onMutate?.(e);
        i !== this.state.context && this.#r({
          type: "pending",
          context: i,
          variables: e,
          isPaused: r
        });
      }
      const s = await this.#n.start();
      return await this.#e.config.onSuccess?.(
        s,
        e,
        this.state.context,
        this
      ), await this.options.onSuccess?.(s, e, this.state.context), await this.#e.config.onSettled?.(
        s,
        null,
        this.state.variables,
        this.state.context,
        this
      ), await this.options.onSettled?.(s, null, e, this.state.context), this.#r({ type: "success", data: s }), s;
    } catch (s) {
      try {
        throw await this.#e.config.onError?.(
          s,
          e,
          this.state.context,
          this
        ), await this.options.onError?.(
          s,
          e,
          this.state.context
        ), await this.#e.config.onSettled?.(
          void 0,
          s,
          this.state.variables,
          this.state.context,
          this
        ), await this.options.onSettled?.(
          void 0,
          s,
          e,
          this.state.context
        ), s;
      } finally {
        this.#r({ type: "error", error: s });
      }
    } finally {
      this.#e.runNext(this);
    }
  }
  #r(e) {
    const t = (n) => {
      switch (e.type) {
        case "failed":
          return {
            ...n,
            failureCount: e.failureCount,
            failureReason: e.error
          };
        case "pause":
          return {
            ...n,
            isPaused: !0
          };
        case "continue":
          return {
            ...n,
            isPaused: !1
          };
        case "pending":
          return {
            ...n,
            context: e.context,
            data: void 0,
            failureCount: 0,
            failureReason: null,
            error: null,
            isPaused: e.isPaused,
            status: "pending",
            variables: e.variables,
            submittedAt: Date.now()
          };
        case "success":
          return {
            ...n,
            data: e.data,
            failureCount: 0,
            failureReason: null,
            error: null,
            status: "success",
            isPaused: !1
          };
        case "error":
          return {
            ...n,
            data: void 0,
            error: e.error,
            failureCount: n.failureCount + 1,
            failureReason: e.error,
            isPaused: !1,
            status: "error"
          };
      }
    };
    this.state = t(this.state), Ee.batch(() => {
      this.#t.forEach((n) => {
        n.onMutationUpdate(e);
      }), this.#e.notify({
        mutation: this,
        type: "updated",
        action: e
      });
    });
  }
};
function ea() {
  return {
    context: void 0,
    data: void 0,
    error: null,
    failureCount: 0,
    failureReason: null,
    isPaused: !1,
    status: "idle",
    variables: void 0,
    submittedAt: 0
  };
}
var ta = class extends nn {
  constructor(t = {}) {
    super(), this.config = t, this.#t = /* @__PURE__ */ new Set(), this.#e = /* @__PURE__ */ new Map(), this.#n = 0;
  }
  #t;
  #e;
  #n;
  build(t, n, r) {
    const s = new cu({
      mutationCache: this,
      mutationId: ++this.#n,
      options: t.defaultMutationOptions(n),
      state: r
    });
    return this.add(s), s;
  }
  add(t) {
    this.#t.add(t);
    const n = Tn(t);
    if (typeof n == "string") {
      const r = this.#e.get(n);
      r ? r.push(t) : this.#e.set(n, [t]);
    }
    this.notify({ type: "added", mutation: t });
  }
  remove(t) {
    if (this.#t.delete(t)) {
      const n = Tn(t);
      if (typeof n == "string") {
        const r = this.#e.get(n);
        if (r)
          if (r.length > 1) {
            const s = r.indexOf(t);
            s !== -1 && r.splice(s, 1);
          } else r[0] === t && this.#e.delete(n);
      }
    }
    this.notify({ type: "removed", mutation: t });
  }
  canRun(t) {
    const n = Tn(t);
    if (typeof n == "string") {
      const s = this.#e.get(n)?.find(
        (i) => i.state.status === "pending"
      );
      return !s || s === t;
    } else
      return !0;
  }
  runNext(t) {
    const n = Tn(t);
    return typeof n == "string" ? this.#e.get(n)?.find((s) => s !== t && s.state.isPaused)?.continue() ?? Promise.resolve() : Promise.resolve();
  }
  clear() {
    Ee.batch(() => {
      this.#t.forEach((t) => {
        this.notify({ type: "removed", mutation: t });
      }), this.#t.clear(), this.#e.clear();
    });
  }
  getAll() {
    return Array.from(this.#t);
  }
  find(t) {
    const n = { exact: !0, ...t };
    return this.getAll().find(
      (r) => Vi(n, r)
    );
  }
  findAll(t = {}) {
    return this.getAll().filter((n) => Vi(t, n));
  }
  notify(t) {
    Ee.batch(() => {
      this.listeners.forEach((n) => {
        n(t);
      });
    });
  }
  resumePausedMutations() {
    const t = this.getAll().filter((n) => n.state.isPaused);
    return Ee.batch(
      () => Promise.all(
        t.map((n) => n.continue().catch(Ke))
      )
    );
  }
};
function Tn(e) {
  return e.options.scope?.id;
}
function Si(e) {
  return {
    onFetch: (t, n) => {
      const r = t.options, s = t.fetchOptions?.meta?.fetchMore?.direction, i = t.state.data?.pages || [], o = t.state.data?.pageParams || [];
      let a = { pages: [], pageParams: [] }, l = 0;
      const c = async () => {
        let u = !1;
        const h = (w) => {
          Object.defineProperty(w, "signal", {
            enumerable: !0,
            get: () => (t.signal.aborted ? u = !0 : t.signal.addEventListener("abort", () => {
              u = !0;
            }), t.signal)
          });
        }, g = Zo(t.options, t.fetchOptions), p = async (w, m, Q) => {
          if (u)
            return Promise.reject();
          if (m == null && w.pages.length)
            return Promise.resolve(w);
          const M = (() => {
            const C = {
              client: t.client,
              queryKey: t.queryKey,
              pageParam: m,
              direction: Q ? "backward" : "forward",
              meta: t.options.meta
            };
            return h(C), C;
          })(), G = await g(M), { maxPages: V } = t.options, v = Q ? tu : eu;
          return {
            pages: v(w.pages, G, V),
            pageParams: v(w.pageParams, m, V)
          };
        };
        if (s && i.length) {
          const w = s === "backward", m = w ? uu : Ii, Q = {
            pages: i,
            pageParams: o
          }, k = m(r, Q);
          a = await p(Q, k, w);
        } else {
          const w = e ?? i.length;
          do {
            const m = l === 0 ? o[0] ?? r.initialPageParam : Ii(r, a);
            if (l > 0 && m == null)
              break;
            a = await p(a, m), l++;
          } while (l < w);
        }
        return a;
      };
      t.options.persister ? t.fetchFn = () => t.options.persister?.(
        c,
        {
          client: t.client,
          queryKey: t.queryKey,
          meta: t.options.meta,
          signal: t.signal
        },
        n
      ) : t.fetchFn = c;
    }
  };
}
function Ii(e, { pages: t, pageParams: n }) {
  const r = t.length - 1;
  return t.length > 0 ? e.getNextPageParam(
    t[r],
    t,
    n[r],
    n
  ) : void 0;
}
function uu(e, { pages: t, pageParams: n }) {
  return t.length > 0 ? e.getPreviousPageParam?.(t[0], t, n[0], n) : void 0;
}
var fu = class {
  #t;
  #e;
  #n;
  #r;
  #s;
  #o;
  #a;
  #i;
  constructor(t = {}) {
    this.#t = t.queryCache || new $o(), this.#e = t.mutationCache || new ta(), this.#n = t.defaultOptions || {}, this.#r = /* @__PURE__ */ new Map(), this.#s = /* @__PURE__ */ new Map(), this.#o = 0;
  }
  mount() {
    this.#o++, this.#o === 1 && (this.#a = Qs.subscribe(async (t) => {
      t && (await this.resumePausedMutations(), this.#t.onFocus());
    }), this.#i = or.subscribe(async (t) => {
      t && (await this.resumePausedMutations(), this.#t.onOnline());
    }));
  }
  unmount() {
    this.#o--, this.#o === 0 && (this.#a?.(), this.#a = void 0, this.#i?.(), this.#i = void 0);
  }
  isFetching(t) {
    return this.#t.findAll({ ...t, fetchStatus: "fetching" }).length;
  }
  isMutating(t) {
    return this.#e.findAll({ ...t, status: "pending" }).length;
  }
  /**
   * Imperative (non-reactive) way to retrieve data for a QueryKey.
   * Should only be used in callbacks or functions where reading the latest data is necessary, e.g. for optimistic updates.
   *
   * Hint: Do not use this function inside a component, because it won't receive updates.
   * Use `useQuery` to create a `QueryObserver` that subscribes to changes.
   */
  getQueryData(t) {
    const n = this.defaultQueryOptions({ queryKey: t });
    return this.#t.get(n.queryHash)?.state.data;
  }
  ensureQueryData(t) {
    const n = this.defaultQueryOptions(t), r = this.#t.build(this, n), s = r.state.data;
    return s === void 0 ? this.fetchQuery(t) : (t.revalidateIfStale && r.isStaleByTime(Et(n.staleTime, r)) && this.prefetchQuery(n), Promise.resolve(s));
  }
  getQueriesData(t) {
    return this.#t.findAll(t).map(({ queryKey: n, state: r }) => {
      const s = r.data;
      return [n, s];
    });
  }
  setQueryData(t, n, r) {
    const s = this.defaultQueryOptions({ queryKey: t }), o = this.#t.get(
      s.queryHash
    )?.state.data, a = Xc(n, o);
    if (a !== void 0)
      return this.#t.build(this, s).setData(a, { ...r, manual: !0 });
  }
  setQueriesData(t, n, r) {
    return Ee.batch(
      () => this.#t.findAll(t).map(({ queryKey: s }) => [
        s,
        this.setQueryData(s, n, r)
      ])
    );
  }
  getQueryState(t) {
    const n = this.defaultQueryOptions({ queryKey: t });
    return this.#t.get(
      n.queryHash
    )?.state;
  }
  removeQueries(t) {
    const n = this.#t;
    Ee.batch(() => {
      n.findAll(t).forEach((r) => {
        n.remove(r);
      });
    });
  }
  resetQueries(t, n) {
    const r = this.#t;
    return Ee.batch(() => (r.findAll(t).forEach((s) => {
      s.reset();
    }), this.refetchQueries(
      {
        type: "active",
        ...t
      },
      n
    )));
  }
  cancelQueries(t, n = {}) {
    const r = { revert: !0, ...n }, s = Ee.batch(
      () => this.#t.findAll(t).map((i) => i.cancel(r))
    );
    return Promise.all(s).then(Ke).catch(Ke);
  }
  invalidateQueries(t, n = {}) {
    return Ee.batch(() => (this.#t.findAll(t).forEach((r) => {
      r.invalidate();
    }), t?.refetchType === "none" ? Promise.resolve() : this.refetchQueries(
      {
        ...t,
        type: t?.refetchType ?? t?.type ?? "active"
      },
      n
    )));
  }
  refetchQueries(t, n = {}) {
    const r = {
      ...n,
      cancelRefetch: n.cancelRefetch ?? !0
    }, s = Ee.batch(
      () => this.#t.findAll(t).filter((i) => !i.isDisabled() && !i.isStatic()).map((i) => {
        let o = i.fetch(void 0, r);
        return r.throwOnError || (o = o.catch(Ke)), i.state.fetchStatus === "paused" ? Promise.resolve() : o;
      })
    );
    return Promise.all(s).then(Ke);
  }
  fetchQuery(t) {
    const n = this.defaultQueryOptions(t);
    n.retry === void 0 && (n.retry = !1);
    const r = this.#t.build(this, n);
    return r.isStaleByTime(
      Et(n.staleTime, r)
    ) ? r.fetch(n) : Promise.resolve(r.state.data);
  }
  prefetchQuery(t) {
    return this.fetchQuery(t).then(Ke).catch(Ke);
  }
  fetchInfiniteQuery(t) {
    return t.behavior = Si(t.pages), this.fetchQuery(t);
  }
  prefetchInfiniteQuery(t) {
    return this.fetchInfiniteQuery(t).then(Ke).catch(Ke);
  }
  ensureInfiniteQueryData(t) {
    return t.behavior = Si(t.pages), this.ensureQueryData(t);
  }
  resumePausedMutations() {
    return or.isOnline() ? this.#e.resumePausedMutations() : Promise.resolve();
  }
  getQueryCache() {
    return this.#t;
  }
  getMutationCache() {
    return this.#e;
  }
  getDefaultOptions() {
    return this.#n;
  }
  setDefaultOptions(t) {
    this.#n = t;
  }
  setQueryDefaults(t, n) {
    this.#r.set(Lt(t), {
      queryKey: t,
      defaultOptions: n
    });
  }
  getQueryDefaults(t) {
    const n = [...this.#r.values()], r = {};
    return n.forEach((s) => {
      Sn(t, s.queryKey) && Object.assign(r, s.defaultOptions);
    }), r;
  }
  setMutationDefaults(t, n) {
    this.#s.set(Lt(t), {
      mutationKey: t,
      defaultOptions: n
    });
  }
  getMutationDefaults(t) {
    const n = [...this.#s.values()], r = {};
    return n.forEach((s) => {
      Sn(t, s.mutationKey) && Object.assign(r, s.defaultOptions);
    }), r;
  }
  defaultQueryOptions(t) {
    if (t._defaulted)
      return t;
    const n = {
      ...this.#n.queries,
      ...this.getQueryDefaults(t.queryKey),
      ...t,
      _defaulted: !0
    };
    return n.queryHash || (n.queryHash = Rs(
      n.queryKey,
      n
    )), n.refetchOnReconnect === void 0 && (n.refetchOnReconnect = n.networkMode !== "always"), n.throwOnError === void 0 && (n.throwOnError = !!n.suspense), !n.networkMode && n.persister && (n.networkMode = "offlineFirst"), n.queryFn === Fs && (n.enabled = !1), n;
  }
  defaultMutationOptions(t) {
    return t?._defaulted ? t : {
      ...this.#n.mutations,
      ...t?.mutationKey && this.getMutationDefaults(t.mutationKey),
      ...t,
      _defaulted: !0
    };
  }
  clear() {
    this.#t.clear(), this.#e.clear();
  }
}, hu = class extends nn {
  constructor(e, t) {
    super(), this.options = t, this.#t = e, this.#i = null, this.#a = ns(), this.bindMethods(), this.setOptions(t);
  }
  #t;
  #e = void 0;
  #n = void 0;
  #r = void 0;
  #s;
  #o;
  #a;
  #i;
  #g;
  #h;
  // This property keeps track of the last query with defined data.
  // It will be used to pass the previous data and query to the placeholder function between renders.
  #d;
  #c;
  #u;
  #l;
  #p = /* @__PURE__ */ new Set();
  bindMethods() {
    this.refetch = this.refetch.bind(this);
  }
  onSubscribe() {
    this.listeners.size === 1 && (this.#e.addObserver(this), Mi(this.#e, this.options) ? this.#f() : this.updateResult(), this.#y());
  }
  onUnsubscribe() {
    this.hasListeners() || this.destroy();
  }
  shouldFetchOnReconnect() {
    return ss(
      this.#e,
      this.options,
      this.options.refetchOnReconnect
    );
  }
  shouldFetchOnWindowFocus() {
    return ss(
      this.#e,
      this.options,
      this.options.refetchOnWindowFocus
    );
  }
  destroy() {
    this.listeners = /* @__PURE__ */ new Set(), this.#b(), this.#A(), this.#e.removeObserver(this);
  }
  setOptions(e) {
    const t = this.options, n = this.#e;
    if (this.options = this.#t.defaultQueryOptions(e), this.options.enabled !== void 0 && typeof this.options.enabled != "boolean" && typeof this.options.enabled != "function" && typeof Le(this.options.enabled, this.#e) != "boolean")
      throw new Error(
        "Expected enabled to be a boolean or a callback that returns a boolean"
      );
    this.#x(), this.#e.setOptions(this.options), t._defaulted && !ir(this.options, t) && this.#t.getQueryCache().notify({
      type: "observerOptionsUpdated",
      query: this.#e,
      observer: this
    });
    const r = this.hasListeners();
    r && Bi(
      this.#e,
      n,
      this.options,
      t
    ) && this.#f(), this.updateResult(), r && (this.#e !== n || Le(this.options.enabled, this.#e) !== Le(t.enabled, this.#e) || Et(this.options.staleTime, this.#e) !== Et(t.staleTime, this.#e)) && this.#m();
    const s = this.#v();
    r && (this.#e !== n || Le(this.options.enabled, this.#e) !== Le(t.enabled, this.#e) || s !== this.#l) && this.#w(s);
  }
  getOptimisticResult(e) {
    const t = this.#t.getQueryCache().build(this.#t, e), n = this.createResult(t, e);
    return pu(this, n) && (this.#r = n, this.#o = this.options, this.#s = this.#e.state), n;
  }
  getCurrentResult() {
    return this.#r;
  }
  trackResult(e, t) {
    return new Proxy(e, {
      get: (n, r) => (this.trackProp(r), t?.(r), r === "promise" && !this.options.experimental_prefetchInRender && this.#a.status === "pending" && this.#a.reject(
        new Error(
          "experimental_prefetchInRender feature flag is not enabled"
        )
      ), Reflect.get(n, r))
    });
  }
  trackProp(e) {
    this.#p.add(e);
  }
  getCurrentQuery() {
    return this.#e;
  }
  refetch({ ...e } = {}) {
    return this.fetch({
      ...e
    });
  }
  fetchOptimistic(e) {
    const t = this.#t.defaultQueryOptions(e), n = this.#t.getQueryCache().build(this.#t, t);
    return n.fetch().then(() => this.createResult(n, t));
  }
  fetch(e) {
    return this.#f({
      ...e,
      cancelRefetch: e.cancelRefetch ?? !0
    }).then(() => (this.updateResult(), this.#r));
  }
  #f(e) {
    this.#x();
    let t = this.#e.fetch(
      this.options,
      e
    );
    return e?.throwOnError || (t = t.catch(Ke)), t;
  }
  #m() {
    this.#b();
    const e = Et(
      this.options.staleTime,
      this.#e
    );
    if (Nt || this.#r.isStale || !_r(e))
      return;
    const n = Jo(this.#r.dataUpdatedAt, e) + 1;
    this.#c = setTimeout(() => {
      this.#r.isStale || this.updateResult();
    }, n);
  }
  #v() {
    return (typeof this.options.refetchInterval == "function" ? this.options.refetchInterval(this.#e) : this.options.refetchInterval) ?? !1;
  }
  #w(e) {
    this.#A(), this.#l = e, !(Nt || Le(this.options.enabled, this.#e) === !1 || !_r(this.#l) || this.#l === 0) && (this.#u = setInterval(() => {
      (this.options.refetchIntervalInBackground || Qs.isFocused()) && this.#f();
    }, this.#l));
  }
  #y() {
    this.#m(), this.#w(this.#v());
  }
  #b() {
    this.#c && (clearTimeout(this.#c), this.#c = void 0);
  }
  #A() {
    this.#u && (clearInterval(this.#u), this.#u = void 0);
  }
  createResult(e, t) {
    const n = this.#e, r = this.options, s = this.#r, i = this.#s, o = this.#o, l = e !== n ? e.state : this.#n, { state: c } = e;
    let u = { ...c }, h = !1, g;
    if (t._optimisticResults) {
      const Y = this.hasListeners(), $ = !Y && Mi(e, t), z = Y && Bi(e, n, t, r);
      ($ || z) && (u = {
        ...u,
        ..._o(c.data, e.options)
      }), t._optimisticResults === "isRestoring" && (u.fetchStatus = "idle");
    }
    let { error: p, errorUpdatedAt: w, status: m } = u;
    g = u.data;
    let Q = !1;
    if (t.placeholderData !== void 0 && g === void 0 && m === "pending") {
      let Y;
      s?.isPlaceholderData && t.placeholderData === o?.placeholderData ? (Y = s.data, Q = !0) : Y = typeof t.placeholderData == "function" ? t.placeholderData(
        this.#d?.state.data,
        this.#d
      ) : t.placeholderData, Y !== void 0 && (m = "success", g = es(
        s?.data,
        Y,
        t
      ), h = !0);
    }
    if (t.select && g !== void 0 && !Q)
      if (s && g === i?.data && t.select === this.#g)
        g = this.#h;
      else
        try {
          this.#g = t.select, g = t.select(g), g = es(s?.data, g, t), this.#h = g, this.#i = null;
        } catch (Y) {
          this.#i = Y;
        }
    this.#i && (p = this.#i, g = this.#h, w = Date.now(), m = "error");
    const k = u.fetchStatus === "fetching", M = m === "pending", G = m === "error", V = M && k, v = g !== void 0, E = {
      status: m,
      fetchStatus: u.fetchStatus,
      isPending: M,
      isSuccess: m === "success",
      isError: G,
      isInitialLoading: V,
      isLoading: V,
      data: g,
      dataUpdatedAt: u.dataUpdatedAt,
      error: p,
      errorUpdatedAt: w,
      failureCount: u.fetchFailureCount,
      failureReason: u.fetchFailureReason,
      errorUpdateCount: u.errorUpdateCount,
      isFetched: u.dataUpdateCount > 0 || u.errorUpdateCount > 0,
      isFetchedAfterMount: u.dataUpdateCount > l.dataUpdateCount || u.errorUpdateCount > l.errorUpdateCount,
      isFetching: k,
      isRefetching: k && !M,
      isLoadingError: G && !v,
      isPaused: u.fetchStatus === "paused",
      isPlaceholderData: h,
      isRefetchError: G && v,
      isStale: Us(e, t),
      refetch: this.refetch,
      promise: this.#a,
      isEnabled: Le(t.enabled, e) !== !1
    };
    if (this.options.experimental_prefetchInRender) {
      const Y = (he) => {
        E.status === "error" ? he.reject(E.error) : E.data !== void 0 && he.resolve(E.data);
      }, $ = () => {
        const he = this.#a = E.promise = ns();
        Y(he);
      }, z = this.#a;
      switch (z.status) {
        case "pending":
          e.queryHash === n.queryHash && Y(z);
          break;
        case "fulfilled":
          (E.status === "error" || E.data !== z.value) && $();
          break;
        case "rejected":
          (E.status !== "error" || E.error !== z.reason) && $();
          break;
      }
    }
    return E;
  }
  updateResult() {
    const e = this.#r, t = this.createResult(this.#e, this.options);
    if (this.#s = this.#e.state, this.#o = this.options, this.#s.data !== void 0 && (this.#d = this.#e), ir(t, e))
      return;
    this.#r = t;
    const n = () => {
      if (!e)
        return !0;
      const { notifyOnChangeProps: r } = this.options, s = typeof r == "function" ? r() : r;
      if (s === "all" || !s && !this.#p.size)
        return !0;
      const i = new Set(
        s ?? this.#p
      );
      return this.options.throwOnError && i.add("error"), Object.keys(this.#r).some((o) => {
        const a = o;
        return this.#r[a] !== e[a] && i.has(a);
      });
    };
    this.#V({ listeners: n() });
  }
  #x() {
    const e = this.#t.getQueryCache().build(this.#t, this.options);
    if (e === this.#e)
      return;
    const t = this.#e;
    this.#e = e, this.#n = e.state, this.hasListeners() && (t?.removeObserver(this), e.addObserver(this));
  }
  onQueryUpdate() {
    this.updateResult(), this.hasListeners() && this.#y();
  }
  #V(e) {
    Ee.batch(() => {
      e.listeners && this.listeners.forEach((t) => {
        t(this.#r);
      }), this.#t.getQueryCache().notify({
        query: this.#e,
        type: "observerResultsUpdated"
      });
    });
  }
};
function du(e, t) {
  return Le(t.enabled, e) !== !1 && e.state.data === void 0 && !(e.state.status === "error" && t.retryOnMount === !1);
}
function Mi(e, t) {
  return du(e, t) || e.state.data !== void 0 && ss(e, t, t.refetchOnMount);
}
function ss(e, t, n) {
  if (Le(t.enabled, e) !== !1 && Et(t.staleTime, e) !== "static") {
    const r = typeof n == "function" ? n(e) : n;
    return r === "always" || r !== !1 && Us(e, t);
  }
  return !1;
}
function Bi(e, t, n, r) {
  return (e !== t || Le(r.enabled, e) === !1) && (!n.suspense || e.state.status !== "error") && Us(e, n);
}
function Us(e, t) {
  return Le(t.enabled, e) !== !1 && e.isStaleByTime(Et(t.staleTime, e));
}
function pu(e, t) {
  return !ir(e.getCurrentResult(), t);
}
var gu = class extends nn {
  #t;
  #e = void 0;
  #n;
  #r;
  constructor(t, n) {
    super(), this.#t = t, this.setOptions(n), this.bindMethods(), this.#s();
  }
  bindMethods() {
    this.mutate = this.mutate.bind(this), this.reset = this.reset.bind(this);
  }
  setOptions(t) {
    const n = this.options;
    this.options = this.#t.defaultMutationOptions(t), ir(this.options, n) || this.#t.getMutationCache().notify({
      type: "observerOptionsUpdated",
      mutation: this.#n,
      observer: this
    }), n?.mutationKey && this.options.mutationKey && Lt(n.mutationKey) !== Lt(this.options.mutationKey) ? this.reset() : this.#n?.state.status === "pending" && this.#n.setOptions(this.options);
  }
  onUnsubscribe() {
    this.hasListeners() || this.#n?.removeObserver(this);
  }
  onMutationUpdate(t) {
    this.#s(), this.#o(t);
  }
  getCurrentResult() {
    return this.#e;
  }
  reset() {
    this.#n?.removeObserver(this), this.#n = void 0, this.#s(), this.#o();
  }
  mutate(t, n) {
    return this.#r = n, this.#n?.removeObserver(this), this.#n = this.#t.getMutationCache().build(this.#t, this.options), this.#n.addObserver(this), this.#n.execute(t);
  }
  #s() {
    const t = this.#n?.state ?? ea();
    this.#e = {
      ...t,
      isPending: t.status === "pending",
      isSuccess: t.status === "success",
      isError: t.status === "error",
      isIdle: t.status === "idle",
      mutate: this.mutate,
      reset: this.reset
    };
  }
  #o(t) {
    Ee.batch(() => {
      if (this.#r && this.hasListeners()) {
        const n = this.#e.variables, r = this.#e.context;
        t?.type === "success" ? (this.#r.onSuccess?.(t.data, n, r), this.#r.onSettled?.(t.data, null, n, r)) : t?.type === "error" && (this.#r.onError?.(t.error, n, r), this.#r.onSettled?.(
          void 0,
          t.error,
          n,
          r
        ));
      }
      this.listeners.forEach((n) => {
        n(this.#e);
      });
    });
  }
}, mu = "VUE_QUERY_CLIENT";
function na(e) {
  const t = e ? `:${e}` : "";
  return `${mu}${t}`;
}
function is(e, t) {
  Object.keys(e).forEach((n) => {
    e[n] = t[n];
  });
}
function os(e, t, n = "", r = 0) {
  if (t) {
    const s = t(e, n, r);
    if (s === void 0 && xe(e) || s !== void 0)
      return s;
  }
  if (Array.isArray(e))
    return e.map(
      (s, i) => os(s, t, String(i), r + 1)
    );
  if (typeof e == "object" && wu(e)) {
    const s = Object.entries(e).map(([i, o]) => [
      i,
      os(o, t, i, r + 1)
    ]);
    return Object.fromEntries(s);
  }
  return e;
}
function vu(e, t) {
  return os(e, t);
}
function X(e, t = !1) {
  return vu(e, (n, r, s) => {
    if (s === 1 && r === "queryKey")
      return X(n, !0);
    if (t && yu(n))
      return X(n(), t);
    if (xe(n))
      return X(ee(n), t);
  });
}
function wu(e) {
  if (Object.prototype.toString.call(e) !== "[object Object]")
    return !1;
  const t = Object.getPrototypeOf(e);
  return t === null || t === Object.prototype;
}
function yu(e) {
  return typeof e == "function";
}
function Er(e = "") {
  if (!Pl())
    throw new Error(
      "vue-query hooks can only be used inside setup() function or functions that support injection context."
    );
  const t = na(e), n = Rt(t);
  if (!n)
    throw new Error(
      "No 'queryClient' found in Vue context, use 'VueQueryPlugin' to properly initialize the library."
    );
  return n;
}
var bu = class extends $o {
  find(e) {
    return super.find(X(e));
  }
  findAll(e = {}) {
    return super.findAll(X(e));
  }
}, Au = class extends ta {
  find(e) {
    return super.find(X(e));
  }
  findAll(e = {}) {
    return super.findAll(X(e));
  }
}, ra = class extends fu {
  constructor(e = {}) {
    const t = {
      defaultOptions: e.defaultOptions,
      queryCache: e.queryCache || new bu(),
      mutationCache: e.mutationCache || new Au()
    };
    super(t), this.isRestoring = L(!1);
  }
  isFetching(e = {}) {
    return super.isFetching(X(e));
  }
  isMutating(e = {}) {
    return super.isMutating(X(e));
  }
  getQueryData(e) {
    return super.getQueryData(X(e));
  }
  ensureQueryData(e) {
    return super.ensureQueryData(X(e));
  }
  getQueriesData(e) {
    return super.getQueriesData(X(e));
  }
  setQueryData(e, t, n = {}) {
    return super.setQueryData(
      X(e),
      t,
      X(n)
    );
  }
  setQueriesData(e, t, n = {}) {
    return super.setQueriesData(
      X(e),
      t,
      X(n)
    );
  }
  getQueryState(e) {
    return super.getQueryState(X(e));
  }
  removeQueries(e = {}) {
    return super.removeQueries(X(e));
  }
  resetQueries(e = {}, t = {}) {
    return super.resetQueries(X(e), X(t));
  }
  cancelQueries(e = {}, t = {}) {
    return super.cancelQueries(X(e), X(t));
  }
  invalidateQueries(e = {}, t = {}) {
    const n = X(e), r = X(t);
    if (super.invalidateQueries(
      { ...n, refetchType: "none" },
      r
    ), n.refetchType === "none")
      return Promise.resolve();
    const s = {
      ...n,
      type: n.refetchType ?? n.type ?? "active"
    };
    return Ne().then(() => super.refetchQueries(s, r));
  }
  refetchQueries(e = {}, t = {}) {
    return super.refetchQueries(
      X(e),
      X(t)
    );
  }
  fetchQuery(e) {
    return super.fetchQuery(X(e));
  }
  prefetchQuery(e) {
    return super.prefetchQuery(X(e));
  }
  fetchInfiniteQuery(e) {
    return super.fetchInfiniteQuery(X(e));
  }
  prefetchInfiniteQuery(e) {
    return super.prefetchInfiniteQuery(X(e));
  }
  setDefaultOptions(e) {
    super.setDefaultOptions(X(e));
  }
  setQueryDefaults(e, t) {
    super.setQueryDefaults(X(e), X(t));
  }
  getQueryDefaults(e) {
    return super.getQueryDefaults(X(e));
  }
  setMutationDefaults(e, t) {
    super.setMutationDefaults(
      X(e),
      X(t)
    );
  }
  getMutationDefaults(e) {
    return super.getMutationDefaults(X(e));
  }
}, xu = {
  install: (e, t = {}) => {
    const n = na(t.queryClientKey);
    let r;
    if ("queryClient" in t && t.queryClient)
      r = t.queryClient;
    else {
      const o = "queryClientConfig" in t ? t.queryClientConfig : void 0;
      r = new ra(o);
    }
    Nt || r.mount();
    let s = () => {
    };
    if (t.clientPersister) {
      r.isRestoring && (r.isRestoring.value = !0);
      const [o, a] = t.clientPersister(r);
      s = o, a.then(() => {
        r.isRestoring && (r.isRestoring.value = !1), t.clientPersisterOnSuccess?.(r);
      });
    }
    const i = () => {
      r.unmount(), s();
    };
    if (e.onUnmount)
      e.onUnmount(i);
    else {
      const o = e.unmount;
      e.unmount = function() {
        i(), o();
      };
    }
    e.provide(n, r);
  }
};
function Vu(e, t, n) {
  const r = Er(), s = j(() => {
    const p = X(t);
    typeof p.enabled == "function" && (p.enabled = p.enabled());
    const w = r.defaultQueryOptions(p);
    return w._optimisticResults = r.isRestoring?.value ? "isRestoring" : "optimistic", w;
  }), i = new e(r, s.value), o = s.value.shallow ? ys(i.getCurrentResult()) : vr(i.getCurrentResult());
  let a = () => {
  };
  r.isRestoring && Ue(
    r.isRestoring,
    (p) => {
      p || (a(), a = i.subscribe((w) => {
        is(o, w);
      }));
    },
    { immediate: !0 }
  );
  const l = () => {
    i.setOptions(s.value), is(o, i.getCurrentResult());
  };
  Ue(s, l), _i(() => {
    a();
  });
  const c = (...p) => (l(), o.refetch(...p)), u = () => new Promise(
    (p, w) => {
      let m = () => {
      };
      const Q = () => {
        if (s.value.enabled !== !1) {
          i.setOptions(s.value);
          const k = i.getOptimisticResult(
            s.value
          );
          k.isStale ? (m(), i.fetchOptimistic(s.value).then(p, (M) => {
            ts(s.value.throwOnError, [
              M,
              i.getCurrentQuery()
            ]) ? w(M) : p(i.getCurrentResult());
          })) : (m(), p(k));
        }
      };
      Q(), m = Ue(s, Q);
    }
  );
  Ue(
    () => o.error,
    (p) => {
      if (o.isError && !o.isFetching && ts(s.value.throwOnError, [
        p,
        i.getCurrentQuery()
      ]))
        throw p;
    }
  );
  const h = s.value.shallow ? mo(o) : wr(o), g = yo(h);
  for (const p in o)
    typeof o[p] == "function" && (g[p] = o[p]);
  return g.suspense = u, g.refetch = c, g;
}
function Cu(e, t) {
  return Vu(hu, e);
}
function sa(e, t) {
  const n = Er(), r = j(() => n.defaultMutationOptions(X(e))), s = new gu(n, r.value), i = r.value.shallow ? ys(s.getCurrentResult()) : vr(s.getCurrentResult()), o = s.subscribe((u) => {
    is(i, u);
  }), a = (u, h) => {
    s.mutate(u, h).catch(() => {
    });
  };
  Ue(r, () => {
    s.setOptions(r.value);
  }), _i(() => {
    o();
  });
  const l = r.value.shallow ? mo(i) : wr(i), c = yo(l);
  return Ue(
    () => i.error,
    (u) => {
      if (u && ts(r.value.throwOnError, [u]))
        throw u;
    }
  ), {
    ...c,
    mutate: a,
    mutateAsync: i.mutate,
    reset: i.reset
  };
}
const jn = {
  market: {
    buttonColor: "linear-gradient(140deg, #881AFF 12.89%, #5E16D9 50.9%, #3B0C8A 85.42%)",
    gradientColors: ["#00010D", "#8E3AEF", "#5E0056", "#0B0C19", "#AB218D", "#000DFF"],
    gradientLogo: ["#881AFF", "#5E16D9", "#3B0C8A", "#881AFF", "#5E16D9", "#3B0C8A"],
    mainAccent: "#8041ff"
  },
  news: {
    buttonColor: "linear-gradient(218deg, #27D1F9 12.75%, #02B4FE 51.95%, #1854FD 90.38%)",
    gradientColors: ["#00010D", "#3ACBEF", "#00495E", "#000E2F", "#2189AB", "#000DFF"],
    gradientLogo: ["#27D1F9", "#02B4FE", "#1854FD", "#27D1F9", "#02B4FE", "#1854FD"],
    mainAccent: "#0EBFF6"
  },
  streaming: {
    buttonColor: "linear-gradient(160deg, #F40307 5.21%, #9B1010 89.35%)",
    gradientColors: ["#00010D", "#9D0003", "#CD1C1F", "#000000", "#AB2128", "#FF0004"],
    gradientLogo: ["#F40307", "#D60A0D", "#9B1010", "#F40307", "#D60A0D", "#9B1010"],
    mainAccent: "#920616"
  },
  mos: {
    buttonColor: "linear-gradient(145deg, #f0c35a 0%, #d4a84b 52%, #a67c2e 100%)",
    gradientColors: ["#0b0b0c", "#d4a84b", "#1a1a1d", "#0b0b0c", "#f0c35a", "#141416"],
    gradientLogo: ["#f0c35a", "#d4a84b", "#a67c2e", "#f0c35a", "#d4a84b", "#a67c2e"],
    mainAccent: "#d4a84b"
  }
};
function Eu(e) {
  const t = j(() => e.sizeClass || "w-[342px] h-[600px]"), n = j(() => "w-[52px] h-[52px] rounded-full"), r = j(() => jn[e.theme].gradientColors), s = j(() => jn[e.theme].gradientLogo), i = j(() => jn[e.theme].mainAccent), o = j(() => jn[e.theme].buttonColor), a = j(() => `flex w-full ${{
    "bottom-right": "justify-end",
    "bottom-left": "justify-start"
  }[e.position || "bottom-right"]}`), l = j(() => "flex flex-col justify-end self-stretch h-[536px] bg-[#0e0f19] p-4");
  return {
    sizeClasses: t,
    gradients: r,
    logoGradients: s,
    buttonClasses: n,
    mainAccent: i,
    buttonContainerClasses: a,
    chatWindowClasses: l,
    buttonBg: o
  };
}
function Su() {
  const e = L(!1), t = L(!1), n = L(!1), r = L(!1), s = L("portrait"), i = j(() => s.value === "portrait"), o = j(() => s.value === "landscape"), a = () => {
    const c = window.innerWidth, u = window.innerHeight;
    e.value = c <= 480, t.value = c > 480 && c < 1024, n.value = c >= 1024, s.value = c > u ? "landscape" : "portrait";
    const h = navigator.userAgent, g = /Android|webOS|iPhone|iPad|iPod|BlackBerry|IEMobile|Opera Mini/i.test(
      h
    );
    r.value = e.value && g || c <= 480;
  }, l = () => {
    a();
  };
  return tn(() => {
    a(), window.addEventListener("resize", l);
  }), Qn(() => {
    window.removeEventListener("resize", l);
  }), {
    isMobile: e,
    isTablet: t,
    isDesktop: n,
    shouldUseFullscreen: r,
    orientation: s,
    isPortrait: i,
    isLandscape: o
  };
}
const Iu = "http://127.0.0.1:8084", Mu = "ws://127.0.0.1:8084";
let ia = Iu, oa = Mu;
function Bu(e, t) {
  e && (ia = e.replace(/\/$/, "")), t && (oa = t.replace(/\/$/, ""));
}
function ku() {
  return ia;
}
function Ru() {
  return oa;
}
const Fu = "/api/v1";
class Qu extends Error {
  constructor(t, n, r) {
    super(t), this.status = n, this.code = r, this.name = "ApiError";
  }
}
async function Pn(e, t) {
  const n = `${ku()}${Fu}${e}`, r = await fetch(n, {
    credentials: "include",
    // For SSO cookies
    headers: {
      "Content-Type": "application/json",
      ...t?.headers || {}
    },
    ...t
  });
  if (!r.ok) {
    let s = `HTTP ${r.status}`, i = r.status.toString();
    try {
      const o = await r.text();
      try {
        const a = JSON.parse(o);
        s = a.message, i = a.code;
      } catch {
        s = o || s;
      }
    } catch {
      s = `HTTP ${r.status} ${r.statusText}`;
    }
    throw new Qu(s, r.status, i);
  }
  return r.status === 204 ? null : r.json();
}
const pt = {
  get: (e, t) => {
    const n = t ? `${e}?${new URLSearchParams(t).toString()}` : e;
    return Pn(n, { method: "GET" });
  },
  post: (e, t) => Pn(e, {
    method: "POST",
    body: t ? JSON.stringify(t) : void 0
  }),
  patch: (e, t) => Pn(e, {
    method: "PATCH",
    body: t ? JSON.stringify(t) : void 0
  }),
  delete: (e) => Pn(e, { method: "DELETE" })
}, aa = {
  // POST /conversations
  create: (e) => pt.post("/conversations", e),
  // GET /conversations
  list: (e, t) => pt.get("/conversations", {
    user_id: e,
    limit: t?.toString()
  }),
  // PATCH /conversations/{conversationId}
  updateStatus: (e, t, n) => pt.patch(`/conversations/${e}?user_id=${t}`, { status: n }),
  // PATCH /conversations/{conversationId}/rate
  rate: (e, t, n) => pt.patch(`/conversations/${e}/rate?user_id=${t}`, n)
}, Uu = {
  // GET /conversations/{conversationId}/messages - простой запрос всей истории
  getHistory: (e) => pt.get(`/conversations/${e}/messages?limit=100`),
  // POST /conversations/{conversationId}/messages
  send: (e, t) => pt.post(`/conversations/${e}/messages`, t)
}, ki = {
  // POST /attachments/init
  init: (e, t) => pt.post(
    `/attachments/init?conversation_id=${e}`,
    t
  ),
  // POST /attachments/{attachmentId}/complete
  complete: (e, t) => pt.post(
    `/attachments/${e}/complete?conversation_id=${t}`
  ),
  // GET /attachments/{attachmentId}/download
  getDownloadUrl: (e) => pt.get(`/attachments/${e}/download`)
}, Ds = {
  conversations: (e) => ["conversations", e],
  messages: (e) => ["messages", e]
};
function Du() {
  const e = Er();
  return sa({
    mutationFn: (t) => aa.create(t),
    onSuccess: (t, n) => {
      e.setQueryData(Ds.conversations(n.user.id), (r) => r ? {
        ...r,
        items: [t, ...r.items]
      } : { items: [t] });
    }
  });
}
function Ou(e) {
  return Cu({
    queryKey: j(() => Ds.messages(e.value)),
    queryFn: () => Uu.getHistory(e.value),
    enabled: j(() => !!e.value),
    // Сортируем сообщения по seq_no
    select: (t) => ({
      ...t,
      items: t.items.sort((n, r) => n.seq_no - r.seq_no).map((n) => ({ ...n, isReceived: !0 }))
    })
  });
}
function Gu() {
  const e = Er();
  return sa({
    mutationFn: ({
      conversation_id: t,
      user_id: n,
      is_like: r
    }) => aa.rate(t, n, { is_like: r }),
    onSuccess: (t, n) => {
      e.setQueryData(Ds.conversations(n.user_id), (r) => r && {
        ...r,
        items: r.items.map(
          (s) => s.id === t.id ? t : s
        )
      });
    }
  });
}
function Yu({
  userId: e,
  userName: t,
  topic: n,
  needToInitializeChat: r
}) {
  const s = L(""), i = L(!1), o = L(!1), a = L(null), l = Du(), {
    data: c,
    isLoading: u,
    refetch: h
  } = Ou(s), g = async () => {
    if (!e.value || !t.value) {
      console.warn("[Chat] Missing userId or userName"), a.value = "Missing user credentials";
      return;
    }
    if (o.value) {
      console.warn("[Chat] Already initializing");
      return;
    }
    o.value = !0, a.value = null;
    try {
      console.log("[Chat] Creating new support conversation...");
      const m = await l.mutateAsync({
        type: "support",
        user: {
          id: e.value,
          username: t.value
        },
        source: "web",
        page_url: window.location.href,
        locale: navigator.language,
        timezone: -((/* @__PURE__ */ new Date()).getTimezoneOffset() / 60),
        // Часовой пояс в часах
        app_version: "1.0.0",
        topic: n?.value
      });
      s.value = m.id, await h(), i.value = !0;
    } catch (m) {
      a.value = m instanceof Error ? m.message : "Failed to initialize chat";
    } finally {
      o.value = !1;
    }
  };
  Ue(
    [e, t, r],
    ([m, Q, k]) => {
      m && Q && !i.value && k && g();
    },
    { immediate: !0 }
  );
  const p = async () => {
    if (!e.value || !t.value)
      throw new Error("Missing user credentials");
    try {
      const m = await l.mutateAsync({
        type: "support",
        user: {
          id: e.value,
          username: `${t.value}_${Math.random().toString(36).substring(2, 8)}`
        },
        source: "web",
        page_url: window.location.href,
        locale: navigator.language,
        timezone: -((/* @__PURE__ */ new Date()).getTimezoneOffset() / 60),
        app_version: "1.0.0"
      });
      return s.value = m.id, console.log("[Chat] Created new conversation for reconnection"), m.id;
    } catch (m) {
      throw console.error("[Chat] Failed to create new conversation:", m), m;
    }
  }, w = j(() => c.value?.items || []);
  return {
    // State
    conversationId: j(() => s.value),
    messageHistory: w,
    isLoading: j(() => o.value),
    isLoadingHistory: j(() => u.value),
    isInitialized: j(() => i.value),
    error: j(() => a.value),
    // Actions
    initializeChat: g,
    refetchHistory: h,
    createNewConversation: p
  };
}
const Tu = {
  xmlns: "http://www.w3.org/2000/svg",
  width: "24",
  height: "24",
  fill: "none"
};
function ju(e, t) {
  return U(), T("svg", Tu, [...t[0] || (t[0] = [
    b("path", {
      fill: "#FAFAFA",
      "fill-rule": "evenodd",
      d: "M4.804 21.644A6.7 6.7 0 0 0 6 21.75a6.7 6.7 0 0 0 3.583-1.029c.774.182 1.584.279 2.417.279 5.322 0 9.75-3.97 9.75-9S17.322 3 12 3s-9.75 3.97-9.75 9c0 2.409 1.025 4.587 2.674 6.192.232.226.277.428.254.543a3.73 3.73 0 0 1-.814 1.686.75.75 0 0 0 .44 1.223M8.25 10.875a1.125 1.125 0 1 0 0 2.25 1.125 1.125 0 0 0 0-2.25M10.875 12a1.125 1.125 0 1 1 2.25 0 1.125 1.125 0 0 1-2.25 0m4.875-1.125a1.125 1.125 0 1 0 0 2.25 1.125 1.125 0 0 0 0-2.25",
      "clip-rule": "evenodd"
    }, null, -1)
  ])]);
}
const Pu = { render: ju }, Ku = {
  width: "100%",
  height: "100%",
  viewBox: "0 0 342 536",
  fill: "none",
  xmlns: "http://www.w3.org/2000/svg",
  preserveAspectRatio: "none",
  class: "w-full h-full"
}, Nu = {
  opacity: "0.5",
  "clip-path": "url(#clip0_7159_6346)"
}, Lu = ["fill"], Wu = { filter: "url(#filter0_f_7159_6346)" }, Ju = ["fill"], qu = { filter: "url(#filter1_f_7159_6346)" }, Zu = ["fill"], Hu = { filter: "url(#filter2_f_7159_6346)" }, zu = ["fill"], Xu = {
  id: "paint0_linear_7159_6346",
  x1: "-1.48014",
  y1: "315.76",
  x2: "364.847",
  y2: "-209.406",
  gradientUnits: "userSpaceOnUse"
}, _u = ["stop-color"], $u = ["stop-color"], ef = /* @__PURE__ */ Me({
  __name: "ChatBgGradients",
  props: {
    gradients: { default: () => ["#00010D", "#8E3AEF", "#5E0056", "#0B0C19", "#AB218D", "#000DFF"] }
  },
  setup(e) {
    const t = e;
    return (n, r) => (U(), T("svg", Ku, [
      b("g", Nu, [
        b("rect", {
          width: "342",
          height: "536",
          fill: t.gradients[0]
        }, null, 8, Lu),
        b("g", Wu, [
          b("path", {
            "fill-rule": "evenodd",
            "clip-rule": "evenodd",
            d: "M106.519 -239.222C173.512 -234.527 223.196 -196.862 267.401 -159.832C307.71 -126.066 347.223 -87.0191 341.433 -42.4397C335.955 -0.26062 284.506 27.8164 239.547 54.6281C173.769 93.8559 -33.6314 153.788 -104.502 111.537C-161.492 77.5614 -131.362 11.2185 -128.474 -42.4397C-125.765 -92.746 -133.876 -146.819 -88.5489 -184.776C-41.0044 -224.59 34.4785 -244.27 106.519 -239.222Z",
            fill: t.gradients[1]
          }, null, 8, Ju)
        ]),
        b("g", qu, [
          b("path", {
            "fill-rule": "evenodd",
            "clip-rule": "evenodd",
            d: "M20.381 564.307C97.2171 534.816 170.704 442.643 243.613 492.725C317.668 543.595 307.983 664.528 303.395 758.712C299.639 835.798 269.137 901.143 221.177 953.239C168.172 1010.81 106.177 1068.01 30.1391 1055.53C-51.4216 1042.14 -123.827 975.296 -160.742 890.537C-250.647 684.106 -112.651 615.367 20.381 564.307Z",
            fill: t.gradients[2]
          }, null, 8, Zu)
        ]),
        b("g", Hu, [
          b("path", {
            "fill-rule": "evenodd",
            "clip-rule": "evenodd",
            d: "M25.2143 -73.4314C199.759 -97.524 363.631 -69.855 307.933 94.0205C291.29 142.988 207.81 155.789 162.587 194.012C109.401 238.967 106.696 331.553 25.2143 332.983C-55.7983 334.404 -72.8448 247.462 -118.945 198.952C-151.73 164.453 -199.772 135.941 -201.877 94.0205C-206.427 3.40264 -83.9232 -58.367 25.2143 -73.4314Z",
            fill: t.gradients[3]
          }, null, 8, zu)
        ]),
        r[0] || (r[0] = b("g", { filter: "url(#filter3_f_7159_6346)" }, [
          b("path", {
            "fill-rule": "evenodd",
            "clip-rule": "evenodd",
            d: "M298.483 162.2C226.203 213.598 262.562 135.244 195.955 172.524C135.996 206.083 65.6882 299.786 16.6815 319.925C-27.7672 338.192 87.3425 238.901 96.0002 213.602C101.587 197.277 31.1021 234.411 55.0107 207.919C81.5222 178.544 137.067 151.934 179.197 121.418C199.385 106.794 219.469 91.7957 236.816 76.2271C291.232 27.3883 323.831 -20.9323 387.347 -65.3994C441.567 -103.359 503.072 -128.043 502.984 -110.117C502.83 -78.7056 357.439 31.1466 329.091 77.8506C307.487 113.444 364.271 115.418 298.483 162.2Z",
            fill: "url(#paint0_linear_7159_6346)"
          })
        ], -1))
      ]),
      b("defs", null, [
        r[1] || (r[1] = hc('<filter id="filter0_f_7159_6346" x="-389" y="-490" width="981" height="866" filterUnits="userSpaceOnUse" color-interpolation-filters="sRGB"><feFlood flood-opacity="0" result="BackgroundImageFix"></feFlood><feBlend mode="normal" in="SourceGraphic" in2="BackgroundImageFix" result="shape"></feBlend><feGaussianBlur stdDeviation="125" result="effect1_foregroundBlur_7159_6346"></feGaussianBlur></filter><filter id="filter1_f_7159_6346" x="-339.471" y="328.093" width="795.977" height="879.169" filterUnits="userSpaceOnUse" color-interpolation-filters="sRGB"><feFlood flood-opacity="0" result="BackgroundImageFix"></feFlood><feBlend mode="normal" in="SourceGraphic" in2="BackgroundImageFix" result="shape"></feBlend><feGaussianBlur stdDeviation="75" result="effect1_foregroundBlur_7159_6346"></feGaussianBlur></filter><filter id="filter2_f_7159_6346" x="-352" y="-231" width="821" height="714" filterUnits="userSpaceOnUse" color-interpolation-filters="sRGB"><feFlood flood-opacity="0" result="BackgroundImageFix"></feFlood><feBlend mode="normal" in="SourceGraphic" in2="BackgroundImageFix" result="shape"></feBlend><feGaussianBlur stdDeviation="75" result="effect1_foregroundBlur_7159_6346"></feGaussianBlur></filter><filter id="filter3_f_7159_6346" x="-143.49" y="-266.081" width="796.474" height="738.235" filterUnits="userSpaceOnUse" color-interpolation-filters="sRGB"><feFlood flood-opacity="0" result="BackgroundImageFix"></feFlood><feBlend mode="normal" in="SourceGraphic" in2="BackgroundImageFix" result="shape"></feBlend><feGaussianBlur stdDeviation="75" result="effect1_foregroundBlur_7159_6346"></feGaussianBlur></filter>', 4)),
        b("linearGradient", Xu, [
          b("stop", {
            "stop-color": t.gradients[4]
          }, null, 8, _u),
          b("stop", {
            offset: "1",
            "stop-color": t.gradients[5]
          }, null, 8, $u)
        ]),
        r[2] || (r[2] = b("clipPath", { id: "clip0_7159_6346" }, [
          b("rect", {
            width: "342",
            height: "536",
            fill: "white"
          })
        ], -1))
      ])
    ]));
  }
}), tf = {
  id: "header",
  class: "w-full h-[18px] flex justify-center items-center"
}, nf = {
  class: "flex justify-center items-center gap-2.5 px-1 pb-0.5 rounded-[100px]",
  style: { background: "rgba(108, 111, 172, 0.08)" }
}, rf = { class: "font-normal text-xs text-slate-500 font-golos" }, sf = /* @__PURE__ */ Me({
  __name: "ChatHeader",
  props: {
    visibleMessageDate: {}
  },
  setup(e) {
    const t = e, n = (s) => {
      if (!s) return "Сегодня";
      const i = typeof s == "string" ? new Date(s) : s, o = /* @__PURE__ */ new Date(), a = new Date(o.getFullYear(), o.getMonth(), o.getDate()), l = new Date(i.getFullYear(), i.getMonth(), i.getDate()), c = a.getTime() - l.getTime(), u = Math.floor(c / (1e3 * 60 * 60 * 24));
      return u === 0 ? "Сегодня" : u === 1 ? "Вчера" : i.toLocaleDateString("ru-RU", {
        day: "numeric",
        month: "long"
      });
    }, r = j(() => n(t.visibleMessageDate));
    return (s, i) => (U(), T("div", tf, [
      b("div", nf, [
        b("span", rf, Ae(r.value), 1)
      ])
    ]));
  }
}), of = {
  xmlns: "http://www.w3.org/2000/svg",
  width: "24",
  height: "24",
  fill: "none"
};
function af(e, t) {
  return U(), T("svg", of, [...t[0] || (t[0] = [
    b("path", {
      stroke: "currentColor",
      "stroke-linecap": "round",
      "stroke-linejoin": "round",
      "stroke-width": "1.5",
      d: "m14 7.333-5.333 5.334a1.886 1.886 0 0 0 2.667 2.666L16.667 10a3.771 3.771 0 0 0-5.334-5.333L6 10a5.657 5.657 0 0 0 8 8l5.334-5.333"
    }, null, -1)
  ])]);
}
const lf = { render: af }, cf = {
  xmlns: "http://www.w3.org/2000/svg",
  width: "24",
  height: "24",
  fill: "none"
};
function uf(e, t) {
  return U(), T("svg", cf, [...t[0] || (t[0] = [
    b("path", {
      fill: "currentColor",
      d: "m9.51 4.23 8.56 4.28c3.84 1.92 3.84 5.06 0 6.98l-8.56 4.28c-5.76 2.88-8.11.52-5.23-5.23l.87-1.73c.22-.44.22-1.17 0-1.61l-.87-1.74C1.4 3.71 3.76 1.35 9.51 4.23"
    }, null, -1),
    b("path", {
      stroke: "#271436",
      "stroke-linecap": "round",
      "stroke-linejoin": "round",
      "stroke-width": "1.5",
      d: "M5.44 12h5.4"
    }, null, -1)
  ])]);
}
const ff = { render: uf }, hf = {
  xmlns: "http://www.w3.org/2000/svg",
  width: "24",
  height: "24",
  fill: "none"
};
function df(e, t) {
  return U(), T("svg", hf, [...t[0] || (t[0] = [
    b("path", {
      fill: "#FAFAFA",
      "fill-rule": "evenodd",
      d: "M12 22c5.523 0 10-4.477 10-10S17.523 2 12 2 2 6.477 2 12s4.477 10 10 10m-2.505-5.436a.75.75 0 0 1-.993-1.125l.275-.21c.162-.114.395-.26.693-.404A5.8 5.8 0 0 1 12 14.25a5.8 5.8 0 0 1 2.53.575 5 5 0 0 1 .885.545l.083.07a.75.75 0 0 1-.992 1.124l-.14-.105a4 4 0 0 0-.49-.284A4.3 4.3 0 0 0 12 15.75a4.3 4.3 0 0 0-1.876.425c-.218.105-.383.21-.49.284zM17 10a1 1 0 1 1-2 0 1 1 0 0 1 2 0m-9 1a1 1 0 1 0 0-2 1 1 0 0 0 0 2",
      "clip-rule": "evenodd"
    }, null, -1)
  ])]);
}
const pf = { render: df }, gf = (e, t) => {
  const n = e.__vccOpts || e;
  for (const [r, s] of t)
    n[r] = s;
  return n;
}, mf = {}, vf = {
  xmlns: "http://www.w3.org/2000/svg",
  width: "24",
  height: "24",
  viewBox: "0 0 24 24",
  fill: "none"
};
function wf(e, t) {
  return U(), T("svg", vf, [...t[0] || (t[0] = [
    b("path", {
      d: "M12.62 20.81C12.28 20.93 11.72 20.93 11.38 20.81C8.48 19.82 2 15.69 2 8.68998C2 5.59998 4.49 3.09998 7.56 3.09998C9.38 3.09998 10.99 3.97998 12 5.33998C13.01 3.97998 14.63 3.09998 16.44 3.09998C19.51 3.09998 22 5.59998 22 8.68998C22 15.69 15.52 19.82 12.62 20.81Z",
      fill: "url(#paint0_linear_13111_14740)"
    }, null, -1),
    b("defs", null, [
      b("linearGradient", {
        id: "paint0_linear_13111_14740",
        x1: "8.11111",
        y1: "2.35831",
        x2: "14.2043",
        y2: "20.8686",
        gradientUnits: "userSpaceOnUse"
      }, [
        b("stop", { "stop-color": "#F40307" }),
        b("stop", {
          offset: "1",
          "stop-color": "#9B1010"
        })
      ])
    ], -1)
  ])]);
}
const yf = /* @__PURE__ */ gf(mf, [["render", wf]]), bf = {
  xmlns: "http://www.w3.org/2000/svg",
  width: "24",
  height: "24",
  fill: "none"
};
function Af(e, t) {
  return U(), T("svg", bf, [...t[0] || (t[0] = [
    b("path", {
      stroke: "currentColor",
      "stroke-linecap": "round",
      "stroke-linejoin": "round",
      "stroke-width": "1.5",
      d: "M15.182 15.182a4.5 4.5 0 0 1-6.364 0M21 12a9 9 0 1 1-18 0 9 9 0 0 1 18 0M9.75 9.75c0 .414-.168.75-.375.75S9 10.164 9 9.75 9.168 9 9.375 9s.375.336.375.75m-.375 0h.008v.015h-.008zm5.625 0c0 .414-.168.75-.375.75s-.375-.336-.375-.75.168-.75.375-.75.375.336.375.75m-.375 0h.008v.015h-.008z"
    }, null, -1)
  ])]);
}
const xf = { render: Af };
var Ri;
let Vf = Symbol("headlessui.useid"), Cf = 0;
const yn = (Ri = Fl) != null ? Ri : function() {
  return Rt(Vf, () => `${++Cf}`)();
};
function W(e) {
  var t;
  if (e == null || e.value == null) return null;
  let n = (t = e.value.$el) != null ? t : e.value;
  return n instanceof Node ? n : null;
}
function yt(e, t, ...n) {
  if (e in t) {
    let s = t[e];
    return typeof s == "function" ? s(...n) : s;
  }
  let r = new Error(`Tried to handle "${e}" but there is no handler defined. Only defined handlers are: ${Object.keys(t).map((s) => `"${s}"`).join(", ")}.`);
  throw Error.captureStackTrace && Error.captureStackTrace(r, yt), r;
}
var Ef = Object.defineProperty, Sf = (e, t, n) => t in e ? Ef(e, t, { enumerable: !0, configurable: !0, writable: !0, value: n }) : e[t] = n, Fi = (e, t, n) => (Sf(e, typeof t != "symbol" ? t + "" : t, n), n);
let If = class {
  constructor() {
    Fi(this, "current", this.detect()), Fi(this, "currentId", 0);
  }
  set(t) {
    this.current !== t && (this.currentId = 0, this.current = t);
  }
  reset() {
    this.set(this.detect());
  }
  nextId() {
    return ++this.currentId;
  }
  get isServer() {
    return this.current === "server";
  }
  get isClient() {
    return this.current === "client";
  }
  detect() {
    return typeof window > "u" || typeof document > "u" ? "server" : "client";
  }
}, Sr = new If();
function Un(e) {
  if (Sr.isServer) return null;
  if (e instanceof Node) return e.ownerDocument;
  if (e != null && e.hasOwnProperty("value")) {
    let t = W(e);
    if (t) return t.ownerDocument;
  }
  return document;
}
let as = ["[contentEditable=true]", "[tabindex]", "a[href]", "area[href]", "button:not([disabled])", "iframe", "input:not([disabled])", "select:not([disabled])", "textarea:not([disabled])"].map((e) => `${e}:not([tabindex='-1'])`).join(",");
var gt = ((e) => (e[e.First = 1] = "First", e[e.Previous = 2] = "Previous", e[e.Next = 4] = "Next", e[e.Last = 8] = "Last", e[e.WrapAround = 16] = "WrapAround", e[e.NoScroll = 32] = "NoScroll", e))(gt || {}), ar = ((e) => (e[e.Error = 0] = "Error", e[e.Overflow = 1] = "Overflow", e[e.Success = 2] = "Success", e[e.Underflow = 3] = "Underflow", e))(ar || {}), Mf = ((e) => (e[e.Previous = -1] = "Previous", e[e.Next = 1] = "Next", e))(Mf || {});
function Ir(e = document.body) {
  return e == null ? [] : Array.from(e.querySelectorAll(as)).sort((t, n) => Math.sign((t.tabIndex || Number.MAX_SAFE_INTEGER) - (n.tabIndex || Number.MAX_SAFE_INTEGER)));
}
var Os = ((e) => (e[e.Strict = 0] = "Strict", e[e.Loose = 1] = "Loose", e))(Os || {});
function la(e, t = 0) {
  var n;
  return e === ((n = Un(e)) == null ? void 0 : n.body) ? !1 : yt(t, { 0() {
    return e.matches(as);
  }, 1() {
    let r = e;
    for (; r !== null; ) {
      if (r.matches(as)) return !0;
      r = r.parentElement;
    }
    return !1;
  } });
}
var Bf = ((e) => (e[e.Keyboard = 0] = "Keyboard", e[e.Mouse = 1] = "Mouse", e))(Bf || {});
typeof window < "u" && typeof document < "u" && (document.addEventListener("keydown", (e) => {
  e.metaKey || e.altKey || e.ctrlKey || (document.documentElement.dataset.headlessuiFocusVisible = "");
}, !0), document.addEventListener("click", (e) => {
  e.detail === 1 ? delete document.documentElement.dataset.headlessuiFocusVisible : e.detail === 0 && (document.documentElement.dataset.headlessuiFocusVisible = "");
}, !0));
let kf = ["textarea", "input"].join(",");
function Rf(e) {
  var t, n;
  return (n = (t = e?.matches) == null ? void 0 : t.call(e, kf)) != null ? n : !1;
}
function Ff(e, t = (n) => n) {
  return e.slice().sort((n, r) => {
    let s = t(n), i = t(r);
    if (s === null || i === null) return 0;
    let o = s.compareDocumentPosition(i);
    return o & Node.DOCUMENT_POSITION_FOLLOWING ? -1 : o & Node.DOCUMENT_POSITION_PRECEDING ? 1 : 0;
  });
}
function Gt(e, t, { sorted: n = !0, relativeTo: r = null, skipElements: s = [] } = {}) {
  var i;
  let o = (i = Array.isArray(e) ? e.length > 0 ? e[0].ownerDocument : document : e?.ownerDocument) != null ? i : document, a = Array.isArray(e) ? n ? Ff(e) : e : Ir(e);
  s.length > 0 && a.length > 1 && (a = a.filter((w) => !s.includes(w))), r = r ?? o.activeElement;
  let l = (() => {
    if (t & 5) return 1;
    if (t & 10) return -1;
    throw new Error("Missing Focus.First, Focus.Previous, Focus.Next or Focus.Last");
  })(), c = (() => {
    if (t & 1) return 0;
    if (t & 2) return Math.max(0, a.indexOf(r)) - 1;
    if (t & 4) return Math.max(0, a.indexOf(r)) + 1;
    if (t & 8) return a.length - 1;
    throw new Error("Missing Focus.First, Focus.Previous, Focus.Next or Focus.Last");
  })(), u = t & 32 ? { preventScroll: !0 } : {}, h = 0, g = a.length, p;
  do {
    if (h >= g || h + g <= 0) return 0;
    let w = c + h;
    if (t & 16) w = (w + g) % g;
    else {
      if (w < 0) return 3;
      if (w >= g) return 1;
    }
    p = a[w], p?.focus(u), h += l;
  } while (p !== o.activeElement);
  return t & 6 && Rf(p) && p.select(), 2;
}
function Qf() {
  return /iPhone/gi.test(window.navigator.platform) || /Mac/gi.test(window.navigator.platform) && window.navigator.maxTouchPoints > 0;
}
function Uf() {
  return /Android/gi.test(window.navigator.userAgent);
}
function Df() {
  return Qf() || Uf();
}
function Kn(e, t, n) {
  Sr.isServer || Ft((r) => {
    document.addEventListener(e, t, n), r(() => document.removeEventListener(e, t, n));
  });
}
function ca(e, t, n) {
  Sr.isServer || Ft((r) => {
    window.addEventListener(e, t, n), r(() => window.removeEventListener(e, t, n));
  });
}
function Of(e, t, n = j(() => !0)) {
  function r(i, o) {
    if (!n.value || i.defaultPrevented) return;
    let a = o(i);
    if (a === null || !a.getRootNode().contains(a)) return;
    let l = (function c(u) {
      return typeof u == "function" ? c(u()) : Array.isArray(u) || u instanceof Set ? u : [u];
    })(e);
    for (let c of l) {
      if (c === null) continue;
      let u = c instanceof HTMLElement ? c : W(c);
      if (u != null && u.contains(a) || i.composed && i.composedPath().includes(u)) return;
    }
    return !la(a, Os.Loose) && a.tabIndex !== -1 && i.preventDefault(), t(i, a);
  }
  let s = L(null);
  Kn("pointerdown", (i) => {
    var o, a;
    n.value && (s.value = ((a = (o = i.composedPath) == null ? void 0 : o.call(i)) == null ? void 0 : a[0]) || i.target);
  }, !0), Kn("mousedown", (i) => {
    var o, a;
    n.value && (s.value = ((a = (o = i.composedPath) == null ? void 0 : o.call(i)) == null ? void 0 : a[0]) || i.target);
  }, !0), Kn("click", (i) => {
    Df() || s.value && (r(i, () => s.value), s.value = null);
  }, !0), Kn("touchend", (i) => r(i, () => i.target instanceof HTMLElement ? i.target : null), !0), ca("blur", (i) => r(i, () => window.document.activeElement instanceof HTMLIFrameElement ? window.document.activeElement : null), !0);
}
function Qi(e, t) {
  if (e) return e;
  let n = t ?? "button";
  if (typeof n == "string" && n.toLowerCase() === "button") return "button";
}
function Gf(e, t) {
  let n = L(Qi(e.value.type, e.value.as));
  return tn(() => {
    n.value = Qi(e.value.type, e.value.as);
  }), Ft(() => {
    var r;
    n.value || W(t) && W(t) instanceof HTMLButtonElement && !((r = W(t)) != null && r.hasAttribute("type")) && (n.value = "button");
  }), n;
}
var ls = ((e) => (e[e.None = 0] = "None", e[e.RenderStrategy = 1] = "RenderStrategy", e[e.Static = 2] = "Static", e))(ls || {}), Yf = ((e) => (e[e.Unmount = 0] = "Unmount", e[e.Hidden = 1] = "Hidden", e))(Yf || {});
function Mr({ visible: e = !0, features: t = 0, ourProps: n, theirProps: r, ...s }) {
  var i;
  let o = fa(r, n), a = Object.assign(s, { props: o });
  if (e || t & 2 && o.static) return Pr(a);
  if (t & 1) {
    let l = (i = o.unmount) == null || i ? 0 : 1;
    return yt(l, { 0() {
      return null;
    }, 1() {
      return Pr({ ...s, props: { ...o, hidden: !0, style: { display: "none" } } });
    } });
  }
  return Pr(a);
}
function Pr({ props: e, attrs: t, slots: n, slot: r, name: s }) {
  var i, o;
  let { as: a, ...l } = Tf(e, ["unmount", "static"]), c = (i = n.default) == null ? void 0 : i.call(n, r), u = {};
  if (r) {
    let h = !1, g = [];
    for (let [p, w] of Object.entries(r)) typeof w == "boolean" && (h = !0), w === !0 && g.push(p);
    h && (u["data-headlessui-state"] = g.join(" "));
  }
  if (a === "template") {
    if (c = ua(c ?? []), Object.keys(l).length > 0 || Object.keys(t).length > 0) {
      let [h, ...g] = c ?? [];
      if (!jf(h) || g.length > 0) throw new Error(['Passing props on "template"!', "", `The current component <${s} /> is rendering a "template".`, "However we need to passthrough the following props:", Object.keys(l).concat(Object.keys(t)).map((m) => m.trim()).filter((m, Q, k) => k.indexOf(m) === Q).sort((m, Q) => m.localeCompare(Q)).map((m) => `  - ${m}`).join(`
`), "", "You can apply a few solutions:", ['Add an `as="..."` prop, to ensure that we render an actual element instead of a "template".', "Render a single element as the child so that we can forward the props onto that element."].map((m) => `  - ${m}`).join(`
`)].join(`
`));
      let p = fa((o = h.props) != null ? o : {}, l, u), w = Kt(h, p, !0);
      for (let m in p) m.startsWith("on") && (w.props || (w.props = {}), w.props[m] = p[m]);
      return w;
    }
    return Array.isArray(c) && c.length === 1 ? c[0] : c;
  }
  return it(a, Object.assign({}, l, u), { default: () => c });
}
function ua(e) {
  return e.flatMap((t) => t.type === ge ? ua(t.children) : [t]);
}
function fa(...e) {
  if (e.length === 0) return {};
  if (e.length === 1) return e[0];
  let t = {}, n = {};
  for (let r of e) for (let s in r) s.startsWith("on") && typeof r[s] == "function" ? (n[s] != null || (n[s] = []), n[s].push(r[s])) : t[s] = r[s];
  if (t.disabled || t["aria-disabled"]) return Object.assign(t, Object.fromEntries(Object.keys(n).map((r) => [r, void 0])));
  for (let r in n) Object.assign(t, { [r](s, ...i) {
    let o = n[r];
    for (let a of o) {
      if (s instanceof Event && s.defaultPrevented) return;
      a(s, ...i);
    }
  } });
  return t;
}
function Tf(e, t = []) {
  let n = Object.assign({}, e);
  for (let r of t) r in n && delete n[r];
  return n;
}
function jf(e) {
  return e == null ? !1 : typeof e.type == "string" || typeof e.type == "object" || typeof e.type == "function";
}
var In = ((e) => (e[e.None = 1] = "None", e[e.Focusable = 2] = "Focusable", e[e.Hidden = 4] = "Hidden", e))(In || {});
let lr = /* @__PURE__ */ Me({ name: "Hidden", props: { as: { type: [Object, String], default: "div" }, features: { type: Number, default: 1 } }, setup(e, { slots: t, attrs: n }) {
  return () => {
    var r;
    let { features: s, ...i } = e, o = { "aria-hidden": (s & 2) === 2 ? !0 : (r = i["aria-hidden"]) != null ? r : void 0, hidden: (s & 4) === 4 ? !0 : void 0, style: { position: "fixed", top: 1, left: 1, width: 1, height: 0, padding: 0, margin: -1, overflow: "hidden", clip: "rect(0, 0, 0, 0)", whiteSpace: "nowrap", borderWidth: "0", ...(s & 4) === 4 && (s & 2) !== 2 && { display: "none" } } };
    return Mr({ ourProps: o, theirProps: i, slot: {}, attrs: n, slots: t, name: "Hidden" });
  };
} }), ha = Symbol("Context");
var Mn = ((e) => (e[e.Open = 1] = "Open", e[e.Closed = 2] = "Closed", e[e.Closing = 4] = "Closing", e[e.Opening = 8] = "Opening", e))(Mn || {});
function Pf() {
  return Rt(ha, null);
}
function Kf(e) {
  Ar(ha, e);
}
var xt = ((e) => (e.Space = " ", e.Enter = "Enter", e.Escape = "Escape", e.Backspace = "Backspace", e.Delete = "Delete", e.ArrowLeft = "ArrowLeft", e.ArrowUp = "ArrowUp", e.ArrowRight = "ArrowRight", e.ArrowDown = "ArrowDown", e.Home = "Home", e.End = "End", e.PageUp = "PageUp", e.PageDown = "PageDown", e.Tab = "Tab", e))(xt || {});
function Nf(e, t, n, r) {
  Sr.isServer || Ft((s) => {
    e = e ?? window, e.addEventListener(t, n, r), s(() => e.removeEventListener(t, n, r));
  });
}
var mt = ((e) => (e[e.Forwards = 0] = "Forwards", e[e.Backwards = 1] = "Backwards", e))(mt || {});
function da() {
  let e = L(0);
  return ca("keydown", (t) => {
    t.key === "Tab" && (e.value = t.shiftKey ? 1 : 0);
  }), e;
}
function Lf({ defaultContainers: e = [], portals: t, mainTreeNodeRef: n } = {}) {
  let r = L(null), s = Un(r);
  function i() {
    var o, a, l;
    let c = [];
    for (let u of e) u !== null && (u instanceof HTMLElement ? c.push(u) : "value" in u && u.value instanceof HTMLElement && c.push(u.value));
    if (t != null && t.value) for (let u of t.value) c.push(u);
    for (let u of (o = s?.querySelectorAll("html > *, body > *")) != null ? o : []) u !== document.body && u !== document.head && u instanceof HTMLElement && u.id !== "headlessui-portal-root" && (u.contains(W(r)) || u.contains((l = (a = W(r)) == null ? void 0 : a.getRootNode()) == null ? void 0 : l.host) || c.some((h) => u.contains(h)) || c.push(u));
    return c;
  }
  return { resolveContainers: i, contains(o) {
    return i().some((a) => a.contains(o));
  }, mainTreeNodeRef: r, MainTreeNode() {
    return n != null ? null : it(lr, { features: In.Hidden, ref: r });
  } };
}
let Ui = Symbol("PortalParentContext");
function Wf() {
  let e = Rt(Ui, null), t = L([]);
  function n(i) {
    return t.value.push(i), e && e.register(i), () => r(i);
  }
  function r(i) {
    let o = t.value.indexOf(i);
    o !== -1 && t.value.splice(o, 1), e && e.unregister(i);
  }
  let s = { register: n, unregister: r, portals: t };
  return [t, /* @__PURE__ */ Me({ name: "PortalWrapper", setup(i, { slots: o }) {
    return Ar(Ui, s), () => {
      var a;
      return (a = o.default) == null ? void 0 : a.call(o);
    };
  } })];
}
var Jf = ((e) => (e[e.Open = 0] = "Open", e[e.Closed = 1] = "Closed", e))(Jf || {});
let pa = Symbol("PopoverContext");
function Gs(e) {
  let t = Rt(pa, null);
  if (t === null) {
    let n = new Error(`<${e} /> is missing a parent <${va.name} /> component.`);
    throw Error.captureStackTrace && Error.captureStackTrace(n, Gs), n;
  }
  return t;
}
let qf = Symbol("PopoverGroupContext");
function ga() {
  return Rt(qf, null);
}
let ma = Symbol("PopoverPanelContext");
function Zf() {
  return Rt(ma, null);
}
let va = /* @__PURE__ */ Me({ name: "Popover", inheritAttrs: !1, props: { as: { type: [Object, String], default: "div" } }, setup(e, { slots: t, attrs: n, expose: r }) {
  var s;
  let i = L(null);
  r({ el: i, $el: i });
  let o = L(1), a = L(null), l = L(null), c = L(null), u = L(null), h = j(() => Un(i)), g = j(() => {
    var v, C;
    if (!W(a) || !W(u)) return !1;
    for (let Ve of document.querySelectorAll("body > *")) if (Number(Ve?.contains(W(a))) ^ Number(Ve?.contains(W(u)))) return !0;
    let E = Ir(), Y = E.indexOf(W(a)), $ = (Y + E.length - 1) % E.length, z = (Y + 1) % E.length, he = E[$], at = E[z];
    return !((v = W(u)) != null && v.contains(he)) && !((C = W(u)) != null && C.contains(at));
  }), p = { popoverState: o, buttonId: L(null), panelId: L(null), panel: u, button: a, isPortalled: g, beforePanelSentinel: l, afterPanelSentinel: c, togglePopover() {
    o.value = yt(o.value, { 0: 1, 1: 0 });
  }, closePopover() {
    o.value !== 1 && (o.value = 1);
  }, close(v) {
    p.closePopover();
    let C = v ? v instanceof HTMLElement ? v : v.value instanceof HTMLElement ? W(v) : W(p.button) : W(p.button);
    C?.focus();
  } };
  Ar(pa, p), Kf(j(() => yt(o.value, { 0: Mn.Open, 1: Mn.Closed })));
  let w = { buttonId: p.buttonId, panelId: p.panelId, close() {
    p.closePopover();
  } }, m = ga(), Q = m?.registerPopover, [k, M] = Wf(), G = Lf({ mainTreeNodeRef: m?.mainTreeNodeRef, portals: k, defaultContainers: [a, u] });
  function V() {
    var v, C, E, Y;
    return (Y = m?.isFocusWithinPopoverGroup()) != null ? Y : ((v = h.value) == null ? void 0 : v.activeElement) && (((C = W(a)) == null ? void 0 : C.contains(h.value.activeElement)) || ((E = W(u)) == null ? void 0 : E.contains(h.value.activeElement)));
  }
  return Ft(() => Q?.(w)), Nf((s = h.value) == null ? void 0 : s.defaultView, "focus", (v) => {
    var C, E;
    v.target !== window && v.target instanceof HTMLElement && o.value === 0 && (V() || a && u && (G.contains(v.target) || (C = W(p.beforePanelSentinel)) != null && C.contains(v.target) || (E = W(p.afterPanelSentinel)) != null && E.contains(v.target) || p.closePopover()));
  }, !0), Of(G.resolveContainers, (v, C) => {
    var E;
    p.closePopover(), la(C, Os.Loose) || (v.preventDefault(), (E = W(a)) == null || E.focus());
  }, j(() => o.value === 0)), () => {
    let v = { open: o.value === 0, close: p.close };
    return it(ge, [it(M, {}, () => Mr({ theirProps: { ...e, ...n }, ourProps: { ref: i }, slot: v, slots: t, attrs: n, name: "Popover" })), it(G.MainTreeNode)]);
  };
} }), Hf = /* @__PURE__ */ Me({ name: "PopoverButton", props: { as: { type: [Object, String], default: "button" }, disabled: { type: [Boolean], default: !1 }, id: { type: String, default: null } }, inheritAttrs: !1, setup(e, { attrs: t, slots: n, expose: r }) {
  var s;
  let i = (s = e.id) != null ? s : `headlessui-popover-button-${yn()}`, o = Gs("PopoverButton"), a = j(() => Un(o.button));
  r({ el: o.button, $el: o.button }), tn(() => {
    o.buttonId.value = i;
  }), Qn(() => {
    o.buttonId.value = null;
  });
  let l = ga(), c = l?.closeOthers, u = Zf(), h = j(() => u === null ? !1 : u.value === o.panelId.value), g = L(null), p = `headlessui-focus-sentinel-${yn()}`;
  h.value || Ft(() => {
    o.button.value = W(g);
  });
  let w = Gf(j(() => ({ as: e.as, type: t.type })), g);
  function m(v) {
    var C, E, Y, $, z;
    if (h.value) {
      if (o.popoverState.value === 1) return;
      switch (v.key) {
        case xt.Space:
        case xt.Enter:
          v.preventDefault(), (E = (C = v.target).click) == null || E.call(C), o.closePopover(), (Y = W(o.button)) == null || Y.focus();
          break;
      }
    } else switch (v.key) {
      case xt.Space:
      case xt.Enter:
        v.preventDefault(), v.stopPropagation(), o.popoverState.value === 1 && c?.(o.buttonId.value), o.togglePopover();
        break;
      case xt.Escape:
        if (o.popoverState.value !== 0) return c?.(o.buttonId.value);
        if (!W(o.button) || ($ = a.value) != null && $.activeElement && !((z = W(o.button)) != null && z.contains(a.value.activeElement))) return;
        v.preventDefault(), v.stopPropagation(), o.closePopover();
        break;
    }
  }
  function Q(v) {
    h.value || v.key === xt.Space && v.preventDefault();
  }
  function k(v) {
    var C, E;
    e.disabled || (h.value ? (o.closePopover(), (C = W(o.button)) == null || C.focus()) : (v.preventDefault(), v.stopPropagation(), o.popoverState.value === 1 && c?.(o.buttonId.value), o.togglePopover(), (E = W(o.button)) == null || E.focus()));
  }
  function M(v) {
    v.preventDefault(), v.stopPropagation();
  }
  let G = da();
  function V() {
    let v = W(o.panel);
    if (!v) return;
    function C() {
      yt(G.value, { [mt.Forwards]: () => Gt(v, gt.First), [mt.Backwards]: () => Gt(v, gt.Last) }) === ar.Error && Gt(Ir().filter((E) => E.dataset.headlessuiFocusGuard !== "true"), yt(G.value, { [mt.Forwards]: gt.Next, [mt.Backwards]: gt.Previous }), { relativeTo: W(o.button) });
    }
    C();
  }
  return () => {
    let v = o.popoverState.value === 0, C = { open: v }, { ...E } = e, Y = h.value ? { ref: g, type: w.value, onKeydown: m, onClick: k } : { ref: g, id: i, type: w.value, "aria-expanded": o.popoverState.value === 0, "aria-controls": W(o.panel) ? o.panelId.value : void 0, disabled: e.disabled ? !0 : void 0, onKeydown: m, onKeyup: Q, onClick: k, onMousedown: M };
    return it(ge, [Mr({ ourProps: Y, theirProps: { ...t, ...E }, slot: C, attrs: t, slots: n, name: "PopoverButton" }), v && !h.value && o.isPortalled.value && it(lr, { id: p, features: In.Focusable, "data-headlessui-focus-guard": !0, as: "button", type: "button", onFocus: V })]);
  };
} }), zf = /* @__PURE__ */ Me({ name: "PopoverPanel", props: { as: { type: [Object, String], default: "div" }, static: { type: Boolean, default: !1 }, unmount: { type: Boolean, default: !0 }, focus: { type: Boolean, default: !1 }, id: { type: String, default: null } }, inheritAttrs: !1, setup(e, { attrs: t, slots: n, expose: r }) {
  var s;
  let i = (s = e.id) != null ? s : `headlessui-popover-panel-${yn()}`, { focus: o } = e, a = Gs("PopoverPanel"), l = j(() => Un(a.panel)), c = `headlessui-focus-sentinel-before-${yn()}`, u = `headlessui-focus-sentinel-after-${yn()}`;
  r({ el: a.panel, $el: a.panel }), tn(() => {
    a.panelId.value = i;
  }), Qn(() => {
    a.panelId.value = null;
  }), Ar(ma, a.panelId), Ft(() => {
    var M, G;
    if (!o || a.popoverState.value !== 0 || !a.panel) return;
    let V = (M = l.value) == null ? void 0 : M.activeElement;
    (G = W(a.panel)) != null && G.contains(V) || Gt(W(a.panel), gt.First);
  });
  let h = Pf(), g = j(() => h !== null ? (h.value & Mn.Open) === Mn.Open : a.popoverState.value === 0);
  function p(M) {
    var G, V;
    switch (M.key) {
      case xt.Escape:
        if (a.popoverState.value !== 0 || !W(a.panel) || l.value && !((G = W(a.panel)) != null && G.contains(l.value.activeElement))) return;
        M.preventDefault(), M.stopPropagation(), a.closePopover(), (V = W(a.button)) == null || V.focus();
        break;
    }
  }
  function w(M) {
    var G, V, v, C, E;
    let Y = M.relatedTarget;
    Y && W(a.panel) && ((G = W(a.panel)) != null && G.contains(Y) || (a.closePopover(), ((v = (V = W(a.beforePanelSentinel)) == null ? void 0 : V.contains) != null && v.call(V, Y) || (E = (C = W(a.afterPanelSentinel)) == null ? void 0 : C.contains) != null && E.call(C, Y)) && Y.focus({ preventScroll: !0 })));
  }
  let m = da();
  function Q() {
    let M = W(a.panel);
    if (!M) return;
    function G() {
      yt(m.value, { [mt.Forwards]: () => {
        var V;
        Gt(M, gt.First) === ar.Error && ((V = W(a.afterPanelSentinel)) == null || V.focus());
      }, [mt.Backwards]: () => {
        var V;
        (V = W(a.button)) == null || V.focus({ preventScroll: !0 });
      } });
    }
    G();
  }
  function k() {
    let M = W(a.panel);
    if (!M) return;
    function G() {
      yt(m.value, { [mt.Forwards]: () => {
        let V = W(a.button), v = W(a.panel);
        if (!V) return;
        let C = Ir(), E = C.indexOf(V), Y = C.slice(0, E + 1), $ = [...C.slice(E + 1), ...Y];
        for (let z of $.slice()) if (z.dataset.headlessuiFocusGuard === "true" || v != null && v.contains(z)) {
          let he = $.indexOf(z);
          he !== -1 && $.splice(he, 1);
        }
        Gt($, gt.First, { sorted: !1 });
      }, [mt.Backwards]: () => {
        var V;
        Gt(M, gt.Previous) === ar.Error && ((V = W(a.button)) == null || V.focus());
      } });
    }
    G();
  }
  return () => {
    let M = { open: a.popoverState.value === 0, close: a.close }, { focus: G, ...V } = e, v = { ref: a.panel, id: i, onKeydown: p, onFocusout: o && a.popoverState.value === 0 ? w : void 0, tabIndex: -1 };
    return Mr({ ourProps: v, theirProps: { ...t, ...V }, attrs: t, slot: M, slots: { ...n, default: (...C) => {
      var E;
      return [it(ge, [g.value && a.isPortalled.value && it(lr, { id: c, ref: a.beforePanelSentinel, features: In.Focusable, "data-headlessui-focus-guard": !0, as: "button", type: "button", onFocus: Q }), (E = n.default) == null ? void 0 : E.call(n, ...C), g.value && a.isPortalled.value && it(lr, { id: u, ref: a.afterPanelSentinel, features: In.Focusable, "data-headlessui-focus-guard": !0, as: "button", type: "button", onFocus: k })])];
    } }, features: ls.RenderStrategy | ls.Static, visible: g.value, name: "PopoverPanel" });
  };
} });
const Xf = { class: "flex-shrink-0" }, _f = ["onClick"], $f = /* @__PURE__ */ Me({
  __name: "SmileBtn",
  emits: ["emojiSelect"],
  setup(e, { emit: t }) {
    const n = t, r = [
      "😂",
      "🤣",
      "😍",
      "🥰",
      "😘",
      "😎",
      "😊",
      "😁",
      "😀",
      "😉",
      "😅",
      "🙃",
      "🥲",
      "🤔",
      "🥳",
      "🤩",
      "😏",
      "🥺",
      "😢",
      "😭",
      "😤",
      "😡",
      "🤬",
      "🤯",
      "😱",
      "🤗",
      "🙏",
      "👍",
      "👎",
      "👏",
      "🙌",
      "🤝",
      "👌",
      "🤌",
      "✌️",
      "🤞",
      "🤟",
      "🤘",
      "👊",
      "✊",
      "💪",
      "🫶",
      "❤️",
      "🧡",
      "💛",
      "💚",
      "💙",
      "💜",
      "🤎",
      "🖤",
      "🤍",
      "💔",
      "💕",
      "💞",
      "💓",
      "💗",
      "💖",
      "💘",
      "💝",
      "💯",
      "💥",
      "🔥",
      "🌟",
      "✨",
      "🎉",
      "🎊",
      "🥳",
      "🎂",
      "🍰",
      "🍕",
      "🍔",
      "🍟",
      "🍣",
      "🍫",
      "🍭",
      "🍩",
      "🍪",
      "🍷",
      "🍺",
      "🍻",
      "🥂",
      "🍹",
      "☕",
      "🍼",
      "🥤",
      "🚗",
      "✈️",
      "🚀",
      "🏠",
      "🏖",
      "🗽",
      "🌍",
      "🌎",
      "🌏",
      "🌈",
      "⛅",
      "☀️",
      "🌙",
      "⭐",
      "🌌",
      "⛄",
      "❄️",
      "☔",
      "🌊",
      "🌋",
      "🐶",
      "🐱",
      "🐭",
      "🐹",
      "🐰",
      "🐻",
      "🐼",
      "🐨",
      "🐯",
      "🦁",
      "🐷",
      "🐸",
      "🐵",
      "🙈",
      "🙉",
      "🙊",
      "🐒",
      "🐔",
      "🐧",
      "🦉",
      "🦋",
      "🐞",
      "🐝",
      "🐠",
      "🐳",
      "🦈",
      "🐙",
      "🌸",
      "🌹",
      "🌺",
      "🌻",
      "🌷",
      "🌼",
      "🪴",
      "🍀",
      "🎄",
      "🌵",
      "🎁",
      "🛍",
      "💄",
      "👠",
      "👑",
      "👜",
      "👗",
      "🕶",
      "⌚",
      "📱",
      "💻",
      "🖤",
      "🤖",
      "👻",
      "💀",
      "☠️",
      "💩",
      "👽",
      "👾",
      "🎃"
    ], s = (i, o) => {
      n("emojiSelect", i), o();
    };
    return (i, o) => (U(), dt(ee(va), { class: "relative" }, {
      default: Wn(({ open: a, close: l }) => [
        H(ee(Hf), {
          class: me(["w-[36px] h-[36px] rounded-full backdrop-blur-[25px] focus:outline-none focus:ring-0 focus:border-none focus:shadow-none focus-visible:outline-none flex items-center justify-center", [a ? " text-white" : "text-[#64748b]"]]),
          style: Te({
            background: a ? "rgba(250, 250, 250, 0.08)" : "transparent"
          })
        }, {
          default: Wn(() => [
            b("button", Xf, [
              H(ee(xf), { class: "w-6 h-6 cursor-pointer hover:text-neutral-50 transition-colors" })
            ])
          ]),
          _: 2
        }, 1032, ["class", "style"]),
        H(ee(zf), { class: "fixed bottom-[150px] right-[120px] w-[202px] h-[108px] z-50 p-0!" }, {
          default: Wn(() => [
            b("div", {
              class: "p-[12px] pr-[4px] flex flex-wrap w-[225px] h-[108px] overflow-y-auto z-10 backdrop-blur-[25px] rounded-[20px]",
              style: Te([{ background: "rgba(108, 111, 172, 0.08)" }, {
                "scrollbar-width": "thin",
                "scrollbar-color": "rgba(255, 255, 255, 0.2) transparent"
              }])
            }, [
              (U(), T(ge, null, Vt(r, (c) => b("button", {
                key: c,
                type: "button",
                class: "font-normal display-block text-[24px] leading-[28px] flex items-center justify-center cursor-pointer whitespace-nowrap hover:bg-controlsHover",
                onClick: (u) => s(c, l)
              }, Ae(c), 9, _f)), 64))
            ])
          ]),
          _: 2
        }, 1024)
      ]),
      _: 1
    }));
  }
}), eh = {
  xmlns: "http://www.w3.org/2000/svg",
  width: "2",
  height: "24",
  fill: "none"
};
function th(e, t) {
  return U(), T("svg", eh, [...t[0] || (t[0] = [
    b("rect", {
      width: "2",
      height: "24",
      fill: "#FAFAFA",
      opacity: ".5",
      rx: "1"
    }, null, -1)
  ])]);
}
const nh = { render: th }, rh = {
  xmlns: "http://www.w3.org/2000/svg",
  width: "10",
  height: "10",
  fill: "none"
};
function sh(e, t) {
  return U(), T("svg", rh, [...t[0] || (t[0] = [
    b("path", {
      fill: "#FAFAFA",
      "fill-rule": "evenodd",
      d: "M.646.646a.5.5 0 0 1 .708 0L5 4.293 8.646.646a.5.5 0 1 1 .708.708L5.707 5l3.647 3.646a.5.5 0 1 1-.708.708L5 5.707 1.354 9.354a.5.5 0 1 1-.708-.708L4.293 5 .646 1.354a.5.5 0 0 1 0-.708",
      "clip-rule": "evenodd"
    }, null, -1)
  ])]);
}
const ih = { render: sh }, oh = {
  key: 0,
  class: "w-full flex justify-center items-center backdrop-blur-[25px] rounded-[20px] mb-[16px] py-[12px]",
  style: { background: "rgba(108, 111, 172, 0.08)" }
}, ah = { class: "flex flex-col gap-[12px] text-[#64748B] font-golos" }, lh = { class: "flex gap-[8px]" }, ch = {
  key: 1,
  class: "w-full flex justify-center items-center backdrop-blur-[25px] rounded-[20px] mb-[16px] py-[12px]",
  style: { background: "rgba(108, 111, 172, 0.08)" }
}, uh = {
  key: 2,
  class: "p-[12px] flex flex-col gap-[10px] rounded-[20px] mb-[12px]",
  style: { background: "rgba(108, 111, 172, 0.08)" }
}, fh = { class: "flex gap-[10px]" }, hh = { class: "font-normal text-[12px] leading-[10px] text-neutral-50 opacity-50" }, dh = { class: "flex gap-[8px]" }, ph = ["src", "alt"], gh = ["onClick"], mh = {
  key: 0,
  class: "font-normal text-[12px] leading-3 text-neutral-50 opacity-50"
}, vh = { class: "flex gap-[10px] grow w-full min-h-[44px]" }, wh = {
  class: "rounded-[24px] flex-1 flex flex-col overflow-hidden",
  style: { background: "rgba(108, 111, 172, 0.08)" }
}, yh = {
  key: 0,
  class: "px-[14px] mt-[10px] flex gap-[6px] items-center w-full"
}, bh = { class: "flex flex-col w-full min-w-0" }, Ah = { class: "font-normal text-[10px] leading-3 text-neutral-50 opacity-50 truncate" }, xh = { class: "font-normal text-xs text-neutral-50 truncate" }, Vh = { class: "flex items-end flex-1" }, Ch = { class: "flex justify-center items-center h-full pl-[10px]" }, Eh = ["disabled"], Sh = ["onKeydown"], Ih = { class: "py-[4px] pr-[4px]" }, Mh = { class: "flex items-end" }, Bh = ["disabled"], kh = /* @__PURE__ */ Me({
  __name: "ChatFooter",
  props: {
    accentColor: {},
    sendMessage: { type: Function },
    canSendMessages: { type: Boolean },
    replyingMessage: {},
    conversationId: {},
    chatState: {},
    isMobile: { type: Boolean }
  },
  emits: ["sendRateConversation"],
  setup(e, { expose: t }) {
    const n = e, r = L(""), s = L([]), i = L(), o = L(!1), a = L(!1), l = L(), c = () => {
      i.value && i.value.click();
    }, u = () => {
      s.value.forEach((v) => {
        URL.revokeObjectURL(v.download_url || "");
      }), s.value = [], a.value = !1;
    }, h = async (v) => {
      const C = v.target;
      if (C.files) {
        const E = Array.from(C.files), Y = s.value.length + E.length;
        let $ = E;
        if (Y > 6) {
          const z = 6 - s.value.length;
          $ = E.slice(0, z), a.value = !0;
        } else
          a.value = !1;
        o.value = !0;
        for (const z of $)
          if (w(z)) {
            const he = {
              attachment_id: `temp-${Date.now()}-${Math.random().toString(36).substr(2, 9)}`,
              file_name: z.name,
              size_bytes: z.size,
              content_type: z.type,
              download_url: URL.createObjectURL(z)
            };
            s.value.push(he), g(z, he.attachment_id);
          } else
            alert(`Файл ${z.name} не поддерживается. Разрешены только изображения.`);
        C.value = "";
      }
    }, g = async (v, C) => {
      if (!n.conversationId) {
        console.error("No conversationId provided");
        return;
      }
      try {
        const E = await ki.init(n.conversationId, {
          file_name: v.name,
          content_type: v.type,
          size_bytes: v.size
        });
        await fetch(E.upload_url, {
          method: "PUT",
          headers: { "Content-Type": v.type },
          body: v
        }), await ki.complete(E.attachment_id, n.conversationId);
        const Y = s.value.findIndex(($) => $.attachment_id === C);
        Y !== -1 && (s.value[Y].attachment_id = E.attachment_id);
      } catch (E) {
        console.error("Failed to upload attachment:", E);
        const Y = s.value.findIndex(($) => $.attachment_id === C);
        Y !== -1 && (URL.revokeObjectURL(s.value[Y].download_url || ""), s.value.splice(Y, 1));
      } finally {
        s.value.some(
          (Y) => Y.attachment_id.startsWith("temp-")
        ) || (o.value = !1);
      }
    }, p = j(() => {
      const v = r.value.trim().length > 0 && n.canSendMessages, C = s.value.length > 0;
      if (C) {
        const E = s.value.some(
          (Y) => Y.attachment_id.startsWith("temp-")
        );
        return !v && !C || E || o.value || n.chatState !== "active";
      }
      return !v && !C;
    }), w = (v) => [
      "image/png",
      "image/jpg",
      "image/jpeg",
      "image/gif",
      "image/webp",
      "image/bmp",
      "image/svg+xml"
    ].includes(v.type), m = (v) => {
      const C = s.value.findIndex((E) => E.attachment_id === v);
      C !== -1 && (URL.revokeObjectURL(s.value[C].download_url || ""), s.value.splice(C, 1), s.value.length < 6 && (a.value = !1));
    }, Q = async () => {
      if (await Ne(), l.value) {
        l.value.style.height = "auto";
        const v = l.value.scrollHeight, E = Math.min(v, 120);
        l.value.style.height = E + "px";
      }
    }, k = () => {
      (r.value.trim() || s.value.length > 0) && n.canSendMessages && n.sendMessage && (n.sendMessage(r.value.trim(), n.replyingMessage?.message_id, s.value), r.value = "", s.value.forEach((v) => {
        URL.revokeObjectURL(v.download_url || "");
      }), s.value = [], a.value = !1, Q());
    }, M = (v) => {
      if (l.value) {
        const C = l.value.selectionStart, E = l.value.selectionEnd, Y = r.value.substring(0, C), $ = r.value.substring(E);
        r.value = Y + v + $;
        let z = r.value;
        r.value = z, Ne(() => {
          if (l.value) {
            const he = C + v.length;
            l.value.setSelectionRange(he, he), l.value.focus();
          }
        });
      }
    }, G = () => {
      r.value += `
`, Q();
    };
    return t({
      focusInput: () => {
        l.value && l.value.focus();
      }
    }), (v, C) => (U(), T(ge, null, [
      v.chatState === "over" ? (U(), T("div", oh, [
        b("div", ah, [
          C[3] || (C[3] = b("span", { class: "font-normal text-xs text-center" }, "Мы помогли вам?", -1)),
          b("div", lh, [
            b("button", {
              style: { background: "rgba(108, 111, 172, 0.08)" },
              class: "w-[44px] h-[44px] backdrop-blur-[25px] rounded-full flex justify-center items-center",
              onClick: C[0] || (C[0] = (E) => v.$emit("sendRateConversation", !1))
            }, [
              H(ee(pf))
            ]),
            b("button", {
              style: { background: "rgba(108, 111, 172, 0.08)" },
              class: "w-[44px] h-[44px] backdrop-blur-[25px] rounded-full flex justify-center items-center",
              onClick: C[1] || (C[1] = (E) => v.$emit("sendRateConversation", !0))
            }, [
              H(yf)
            ])
          ])
        ])
      ])) : ye("", !0),
      v.chatState === "thankYou" ? (U(), T("div", ch, [...C[4] || (C[4] = [
        b("span", { class: "font-golos font-normal text-xs text-center text-neutral-50" }, "Спасибо за оценку 💜", -1)
      ])])) : ye("", !0),
      s.value.length > 0 ? (U(), T("div", uh, [
        b("div", fh, [
          b("span", hh, Ae(s.value.length) + " фото", 1),
          b("span", {
            style: Te({ color: v.accentColor }),
            class: "font-normal text-[12px] leading-[10px] cursor-pointer",
            onClick: u
          }, "Отменить", 4)
        ]),
        b("div", dh, [
          (U(!0), T(ge, null, Vt(s.value, (E) => (U(), T("div", {
            key: E.attachment_id,
            class: "group relative w-[40px] h-[40px] rounded-[12px] overflow-hidden bg-gray-200 flex justify-center items-center"
          }, [
            b("img", {
              src: E.download_url,
              alt: E.file_name,
              class: "w-full h-full object-cover"
            }, null, 8, ph),
            b("button", {
              style: { background: "rgba(108, 111, 172, 0.08)" },
              onClick: (Y) => m(E.attachment_id),
              class: "absolute top-[8px] left-[8px] backdrop-blur-[25px] w-[23.999998092651367px] h-[23.999998092651367px] rounded-lg flex justify-center items-center opacity-0 group-hover:opacity-100 transition-opacity duration-200"
            }, [
              H(ee(ih))
            ], 8, gh)
          ]))), 128))
        ]),
        a.value ? (U(), T("span", mh, "Превышен максимум")) : ye("", !0)
      ])) : ye("", !0),
      b("div", vh, [
        b("div", wh, [
          v.replyingMessage ? (U(), T("div", yh, [
            H(ee(nh)),
            b("div", bh, [
              b("span", Ah, Ae(v.replyingMessage.sender), 1),
              b("span", xh, Ae(v.replyingMessage.text), 1)
            ])
          ])) : ye("", !0),
          b("div", Vh, [
            b("div", Ch, [
              b("button", {
                disabled: v.chatState !== "active",
                class: me(["mr-[6px] w-[24px] h-[24px] flex-shrink-0 transition-opacity", v.chatState === "active" ? "cursor-pointer" : "cursor-not-allowed opacity-50"]),
                onClick: c
              }, [
                H(ee(lf), {
                  class: me(["w-6 h-6 text-[#64748b] transition-colors", v.chatState === "active" ? "hover:text-neutral-50" : ""])
                }, null, 8, ["class"])
              ], 10, Eh)
            ]),
            b("input", {
              ref_key: "fileInputRef",
              ref: i,
              type: "file",
              multiple: "",
              accept: "image/*,.png,.jpg,.jpeg,.gif,.webp,.bmp,.svg",
              onChange: h,
              style: { display: "none" }
            }, null, 544),
            xs(b("textarea", {
              ref_key: "textareaRef",
              ref: l,
              "onUpdate:modelValue": C[2] || (C[2] = (E) => r.value = E),
              placeholder: "Сообщение...",
              rows: "1",
              onInput: Q,
              onKeydown: [
                yi(En(k, ["exact", "prevent"]), ["enter"]),
                yi(En(G, ["shift", "exact"]), ["enter"])
              ],
              class: me(["flex-1 py-[10px] bg-transparent border-none outline-none text-neutral-50 placeholder-[#64748b] font-golos resize-none max-h-[120px] min-h-[24px] leading-6 hide-scrollbar", [v.isMobile ? "text-md" : "text-sm"]]),
              style: { "word-wrap": "break-word", "white-space": "pre-wrap", "overflow-wrap": "break-word", "overflow-y": "auto", "scrollbar-width": "none", "-ms-overflow-style": "none" }
            }, null, 42, Sh), [
              [Nc, r.value]
            ]),
            b("div", Ih, [
              H($f, { onEmojiSelect: M })
            ])
          ])
        ]),
        b("div", Mh, [
          b("button", {
            onClick: k,
            disabled: p.value,
            class: me(["flex items-center justify-center w-[44px] cursor-pointer h-[44px] rounded-[24px] transition-colors disabled:cursor-not-allowed", [p.value ? "text-[#64748b]" : "text-neutral-50 cursor-not-allowed"]]),
            style: Te({
              background: p.value ? "rgba(108, 111, 172, 0.08)" : n.accentColor
            })
          }, [
            H(ee(ff), { class: "w-6 h-6" })
          ], 14, Bh)
        ])
      ])
    ], 64));
  }
}), Rh = {
  xmlns: "http://www.w3.org/2000/svg",
  width: "16",
  height: "16",
  fill: "none"
};
function Fh(e, t) {
  return U(), T("svg", Rh, [...t[0] || (t[0] = [
    b("path", {
      fill: "#FAFAFA",
      d: "M11.423 3.769a.73.73 0 0 1 .06.96l-.06.068-7.272 7.273a.73.73 0 0 1-.96.06l-.069-.06L.213 9.16a.727.727 0 0 1 .96-1.088l.069.06 2.394 2.395 6.759-6.758a.727.727 0 0 1 1.028 0m4.364 0a.73.73 0 0 1 .06.96l-.06.068-7.273 7.273a.727.727 0 0 1-1.089-.96l.06-.069 7.273-7.272a.727.727 0 0 1 1.029 0"
    }, null, -1)
  ])]);
}
const Qh = { render: Fh }, Uh = {
  xmlns: "http://www.w3.org/2000/svg",
  width: "12",
  height: "10",
  fill: "none"
};
function Dh(e, t) {
  return U(), T("svg", Uh, [...t[0] || (t[0] = [
    b("path", {
      fill: "#FAFAFA",
      d: "M11.646.769a.73.73 0 0 1 .06.96l-.06.068L4.373 9.07a.73.73 0 0 1-.96.06l-.069-.06L.435 6.16a.727.727 0 0 1 .96-1.088l.069.06 2.395 2.394L10.617.77a.727.727 0 0 1 1.028 0"
    }, null, -1)
  ])]);
}
const Oh = { render: Dh }, Gh = {
  xmlns: "http://www.w3.org/2000/svg",
  width: "28",
  height: "28",
  viewBox: "0 0 28 28",
  fill: "none"
}, Yh = {
  id: "paint0_linear_12865_5899",
  x1: "0.35",
  y1: "6.3",
  x2: "18.55",
  y2: "28.35",
  gradientUnits: "userSpaceOnUse"
}, Th = ["stop-color"], jh = ["stop-color"], Ph = ["stop-color"], Kh = {
  id: "paint1_linear_12865_5899",
  x1: "4.54326",
  y1: "8.66542",
  x2: "17.1522",
  y2: "23.9417",
  gradientUnits: "userSpaceOnUse"
}, Nh = ["stop-color"], Lh = ["stop-color"], Wh = ["stop-color"], Jh = /* @__PURE__ */ Me({
  __name: "SupportAvatar",
  props: {
    logoGradients: { default: () => ["#881AFF", "#5E16D9", "#3B0C8A", "#881AFF", "#5E16D9", "#3B0C8A"] }
  },
  setup(e) {
    return (t, n) => (U(), T("svg", Gh, [
      n[0] || (n[0] = b("path", {
        d: "M14 0C11.2311 0 8.52431 0.821086 6.22202 2.35943C3.91973 3.89777 2.12532 6.08427 1.06569 8.64243C0.00606596 11.2006 -0.271181 14.0155 0.269012 16.7313C0.809205 19.447 2.14258 21.9416 4.10051 23.8995C6.05845 25.8574 8.55301 27.1908 11.2687 27.731C13.9845 28.2712 16.7994 27.9939 19.3576 26.9343C21.9157 25.8747 24.1022 24.0803 25.6406 21.778C27.1789 19.4757 28 16.7689 28 14C28 10.287 26.525 6.72601 23.8995 4.1005C21.274 1.475 17.713 0 14 0ZM14 25.0376C11.817 25.0376 9.68297 24.3903 7.86784 23.1774C6.05272 21.9646 4.638 20.2408 3.80259 18.2239C2.96718 16.207 2.7486 13.9878 3.17449 11.8467C3.60038 9.70559 4.65161 7.73887 6.19524 6.19524C7.73888 4.6516 9.70559 3.60037 11.8467 3.17448C13.9878 2.7486 16.2071 2.96718 18.2239 3.80259C20.2408 4.638 21.9646 6.05271 23.1774 7.86784C24.3903 9.68296 25.0376 11.817 25.0376 14C25.0346 16.9264 23.8708 19.7322 21.8015 21.8015C19.7322 23.8708 16.9264 25.0346 14 25.0376Z",
        fill: "url(#paint0_linear_12865_5899)"
      }, null, -1)),
      n[1] || (n[1] = b("path", {
        d: "M14 4.30078C12.0817 4.30078 10.2064 4.86963 8.6114 5.93539C7.01637 7.00115 5.7732 8.51596 5.03909 10.2883C4.30498 12.0606 4.11291 14.0107 4.48715 15.8922C4.8614 17.7737 5.78516 19.5019 7.14162 20.8584C8.49807 22.2148 10.2263 23.1386 12.1078 23.5128C13.9892 23.8871 15.9394 23.695 17.7117 22.9609C19.484 22.2268 20.9988 20.9836 22.0646 19.3886C23.1303 17.7935 23.6992 15.9183 23.6992 14C23.6962 11.4285 22.6734 8.9632 20.8551 7.14489C19.0368 5.32657 16.5715 4.30375 14 4.30078ZM14 21.5544C12.0692 21.5544 10.2175 20.7874 8.85225 19.4221C7.48698 18.0569 6.71999 16.2052 6.71999 14.2744H21.28C21.28 15.2304 21.0917 16.1771 20.7258 17.0603C20.36 17.9436 19.8237 18.7461 19.1477 19.4221C18.4717 20.0981 17.6692 20.6344 16.7859 21.0002C15.9027 21.3661 14.956 21.5544 14 21.5544Z",
        fill: "url(#paint1_linear_12865_5899)"
      }, null, -1)),
      b("defs", null, [
        b("linearGradient", Yh, [
          b("stop", {
            "stop-color": t.logoGradients[0]
          }, null, 8, Th),
          b("stop", {
            offset: "0.52399",
            "stop-color": t.logoGradients[1]
          }, null, 8, jh),
          b("stop", {
            offset: "1",
            "stop-color": t.logoGradients[2]
          }, null, 8, Ph)
        ]),
        b("linearGradient", Kh, [
          b("stop", {
            "stop-color": t.logoGradients[3]
          }, null, 8, Nh),
          b("stop", {
            offset: "0.52399",
            "stop-color": t.logoGradients[4]
          }, null, 8, Lh),
          b("stop", {
            offset: "1",
            "stop-color": t.logoGradients[5]
          }, null, 8, Wh)
        ])
      ])
    ]));
  }
}), qh = { class: "w-full h-full overflow-hidden bg-transparent" }, Zh = ["src", "alt"], Hh = {
  key: 1,
  class: "w-full h-full flex flex-col items-center justify-center bg-gray-50 border border-gray-200 p-2 text-center"
}, zh = { class: "flex-1 flex flex-col justify-center min-h-0" }, Xh = { class: "text-xs font-medium text-gray-800 mb-0.5 break-words leading-tight" }, _h = { class: "text-[10px] text-gray-600" }, Di = "data:image/svg+xml;base64,", Ze = /* @__PURE__ */ Me({
  __name: "AttachmentImage",
  props: {
    attachment: {}
  },
  setup(e) {
    const t = e, n = j(() => t.attachment.content_type.startsWith("image/")), r = j(() => {
      const o = t.attachment.file_name;
      return o.length > 20 ? o.substring(0, 17) + "..." : o;
    });
    function s(o) {
      const a = o.target;
      a.src = Di;
    }
    function i(o) {
      if (o === 0) return "0 B";
      const a = 1024, l = ["B", "KB", "MB", "GB"], c = Math.floor(Math.log(o) / Math.log(a));
      return parseFloat((o / Math.pow(a, c)).toFixed(1)) + " " + l[c];
    }
    return (o, a) => (U(), T("div", qh, [
      n.value ? (U(), T("img", {
        key: 0,
        src: o.attachment.download_url || Di,
        alt: o.attachment.file_name,
        class: "w-full h-full object-cover transition-transform duration-200 hover:scale-105 cursor-pointer",
        onError: s
      }, null, 40, Zh)) : (U(), T("div", Hh, [
        a[0] || (a[0] = b("div", { class: "text-2xl mb-1" }, "📎", -1)),
        b("div", zh, [
          b("div", Xh, Ae(r.value), 1),
          b("div", _h, Ae(i(o.attachment.size_bytes)), 1)
        ])
      ]))
    ]));
  }
}), $h = {
  key: 0,
  class: "w-full aspect-[3/4] min-w-[183px] min-h-[243px]"
}, ed = {
  key: 1,
  class: "grid grid-cols-2 gap-0.5 w-full min-w-[214px]"
}, td = {
  key: 2,
  class: "grid grid-cols-2 gap-0.5 w-full min-w-[214px] aspect-[214/208]"
}, nd = {
  key: 3,
  class: "grid grid-cols-2 grid-rows-2 gap-0.5 w-full min-w-[214px] aspect-[214/208]"
}, rd = {
  key: 4,
  class: "grid grid-cols-2 gap-0.5 w-full min-w-[162px] aspect-[162/240]"
}, sd = {
  key: 5,
  class: "grid grid-cols-2 grid-rows-3 gap-0.5 w-full min-w-[162px] aspect-[162/240]"
}, id = {
  key: 1,
  class: "relative aspect-square min-w-[80px] min-h-[80px]"
}, od = {
  key: 0,
  class: "absolute inset-0 bg-black/60 text-white flex items-center justify-center font-semibold text-sm"
}, Oi = /* @__PURE__ */ Me({
  __name: "MsgAttachments",
  props: {
    attachments: {},
    hasText: { type: Boolean },
    isSupportMsg: { type: Boolean }
  },
  setup(e) {
    return (t, n) => t.attachments.length > 0 ? (U(), T("div", {
      key: 0,
      class: me(["overflow-hidden w-full max-w-sm", t.isSupportMsg ? "" : t.hasText ? "rounded-t-[20px]" : "rounded-[20px]"])
    }, [
      t.attachments.length === 1 ? (U(), T("div", $h, [
        H(Ze, {
          attachment: t.attachments[0]
        }, null, 8, ["attachment"])
      ])) : t.attachments.length === 2 ? (U(), T("div", ed, [
        (U(!0), T(ge, null, Vt(t.attachments, (r) => (U(), dt(Ze, {
          key: r.attachment_id,
          attachment: r,
          class: "aspect-[106/137] min-w-[106px] min-h-[137px]"
        }, null, 8, ["attachment"]))), 128))
      ])) : t.attachments.length === 3 ? (U(), T("div", td, [
        H(Ze, {
          attachment: t.attachments[0],
          class: "row-span-2 aspect-[106/208] min-w-[106px] min-h-[208px]"
        }, null, 8, ["attachment"]),
        H(Ze, {
          attachment: t.attachments[1],
          class: "aspect-[106/103] min-w-[106px] min-h-[103px]"
        }, null, 8, ["attachment"]),
        H(Ze, {
          attachment: t.attachments[2],
          class: "aspect-[106/103] min-w-[106px] min-h-[103px]"
        }, null, 8, ["attachment"])
      ])) : t.attachments.length === 4 ? (U(), T("div", nd, [
        (U(!0), T(ge, null, Vt(t.attachments, (r) => (U(), dt(Ze, {
          key: r.attachment_id,
          attachment: r,
          class: "aspect-[106/103] min-w-[106px] min-h-[103px]"
        }, null, 8, ["attachment"]))), 128))
      ])) : t.attachments.length === 5 ? (U(), T("div", rd, [
        (U(!0), T(ge, null, Vt(t.attachments.slice(0, 2), (r) => (U(), dt(Ze, {
          key: r.attachment_id,
          attachment: r,
          class: "aspect-square min-w-[80px] min-h-[80px]"
        }, null, 8, ["attachment"]))), 128)),
        H(Ze, {
          attachment: t.attachments[2],
          class: "col-span-2 aspect-[162/80] min-w-[162px] min-h-[80px]"
        }, null, 8, ["attachment"]),
        (U(!0), T(ge, null, Vt(t.attachments.slice(3, 5), (r) => (U(), dt(Ze, {
          key: r.attachment_id,
          attachment: r,
          class: "aspect-square min-w-[80px] min-h-[80px]"
        }, null, 8, ["attachment"]))), 128))
      ])) : (U(), T("div", sd, [
        (U(!0), T(ge, null, Vt(t.attachments.slice(0, 6), (r, s) => (U(), T(ge, {
          key: r.attachment_id
        }, [
          s < 5 ? (U(), dt(Ze, {
            key: 0,
            attachment: r,
            class: "aspect-square min-w-[80px] min-h-[80px]"
          }, null, 8, ["attachment"])) : (U(), T("div", id, [
            H(Ze, { attachment: r }, null, 8, ["attachment"]),
            t.attachments.length > 6 ? (U(), T("div", od, " +" + Ae(t.attachments.length - 6), 1)) : ye("", !0)
          ]))
        ], 64))), 128))
      ]))
    ], 2)) : ye("", !0);
  }
}), ad = {
  xmlns: "http://www.w3.org/2000/svg",
  width: "3",
  height: "32",
  fill: "none"
};
function ld(e, t) {
  return U(), T("svg", ad, [...t[0] || (t[0] = [
    b("rect", {
      width: "3",
      height: "32",
      fill: "#FAFAFA",
      rx: "1.5"
    }, null, -1)
  ])]);
}
const cd = { render: ld }, ud = {
  key: 0,
  class: "relative"
}, fd = { class: "w-full flex gap-[10px]" }, hd = {
  id: "avatar",
  class: "h-full"
}, dd = {
  id: "message",
  class: "px-[12px] py-[8px] flex flex-col gap-0.5 rounded-tr-[20px] rounded-br-[20px] rounded-bl-[20px] min-w-[120px]",
  style: { background: "rgba(108, 111, 172, 0.08)" }
}, pd = {
  key: 0,
  id: "attachment-msg",
  class: "w-full"
}, gd = {
  id: "msg",
  class: "min-w-0"
}, md = { class: "text-neutral-50 font-normal text-xs whitespace-nowrap" }, vd = {
  key: 2,
  id: "time-only",
  class: "flex justify-end"
}, wd = { class: "bg-black/20 backdrop-blur-sm rounded px-2 py-1" }, yd = { class: "text-neutral-50 font-normal text-xs" }, bd = {
  key: 1,
  class: "relative"
}, Ad = { class: "flex flex-col max-w-[85%]" }, xd = { id: "attachment" }, Vd = {
  key: 0,
  class: "flex gap-[6px] items-center rounded-[2px]",
  style: { background: "rgba(255, 255, 255, 0.08)" }
}, Cd = { class: "flex flex-col min-w-0" }, Ed = { class: "font-normal text-[10px] leading-3 text-neutral-50 opacity-50 truncate" }, Sd = { class: "font-normal text-xs text-neutral-50 truncate break-words" }, Id = {
  key: 0,
  class: "min-w-0"
}, Md = { class: "font-normal text-xs whitespace-nowrap" }, Bd = { key: 0 }, kd = { key: 1 }, Rd = /* @__PURE__ */ Me({
  __name: "ChatMsg",
  props: {
    message_id: {},
    conversation_id: {},
    seq_no: {},
    sender: {},
    source: {},
    content_type: {},
    text: {},
    attachments: {},
    reply_to_message_id: {},
    created_at: {},
    isReceived: { type: Boolean },
    replyingMessage: {},
    msgColor: {},
    logoGradients: {},
    isMenuOpen: { type: Boolean },
    username: {},
    isTyping: { type: Boolean },
    isMobile: { type: Boolean }
  },
  emits: ["openMenu", "closeMenu"],
  setup(e, { emit: t }) {
    const n = e, r = t, s = j(() => n.sender === "support" || n.sender === "system");
    function i(l) {
      r("openMenu", l, n);
    }
    const o = j(() => n.replyingMessage?.sender === "user" ? n.username || "Вы" : "Йеннифер"), a = (l) => {
      const c = new Date(l), u = c.getHours().toString().padStart(2, "0"), h = c.getMinutes().toString().padStart(2, "0");
      return `${u}:${h}`;
    };
    return (l, c) => s.value ? (U(), T("div", ud, [
      b("div", {
        onContextmenu: En(i, ["prevent"]),
        class: "inline-block text-left w-full cursor-default"
      }, [
        b("div", fd, [
          b("div", hd, [
            H(Jh, { "logo-gradients": l.logoGradients }, null, 8, ["logo-gradients"])
          ]),
          b("div", dd, [
            c[0] || (c[0] = b("div", { class: "w-full" }, [
              b("span", { class: "font-normal text-[12px] leading-[20px] font-unbounded text-neutral-50" }, Ae("Йеннифер"))
            ], -1)),
            l.attachments && l.attachments.length > 0 ? (U(), T("div", pd, [
              H(Oi, {
                "is-support-msg": s.value,
                "has-text": !!l.text,
                attachments: l.attachments
              }, null, 8, ["is-support-msg", "has-text", "attachments"])
            ])) : ye("", !0),
            !l.isTyping && l.text ? (U(), T("div", {
              key: 1,
              id: "message-content",
              class: me(
                l.text && l.text.length > 30 ? "flex flex-col gap-[4px] font-normal text-sm font-golos" : "grid grid-cols-[1fr_auto] gap-[15px] font-normal text-sm font-golos items-end"
              )
            }, [
              b("div", gd, [
                b("span", {
                  class: me(["text-neutral-50 break-words word-break-break-word whitespace-pre-wrap", [l.isMobile ? "text-[16px]" : "text-sm"]])
                }, Ae(l.text), 3)
              ]),
              b("div", {
                id: "time",
                class: me(l.text && l.text.length > 30 ? "flex items-end justify-end" : "flex items-end")
              }, [
                b("span", md, Ae(a(l.created_at)), 1)
              ], 2)
            ], 2)) : !l.isTyping && !l.text && l.attachments && l.attachments.length > 0 ? (U(), T("div", vd, [
              b("div", wd, [
                b("span", yd, Ae(a(l.created_at)), 1)
              ])
            ])) : ye("", !0)
          ])
        ])
      ], 32)
    ])) : (U(), T("div", bd, [
      b("div", {
        onContextmenu: En(i, ["prevent"]),
        class: "flex justify-end w-full cursor-default"
      }, [
        b("div", Ad, [
          b("div", xd, [
            l.attachments ? (U(), dt(Oi, {
              key: 0,
              "has-text": !!l.text,
              attachments: l.attachments
            }, null, 8, ["has-text", "attachments"])) : ye("", !0)
          ]),
          b("div", {
            id: "message_content",
            style: Te({
              background: l.text ? l.msgColor : "rgba(108, 111, 172, 0.08)"
            }),
            class: me(["flex flex-col gap-[4px] text-neutral-50", [
              l.attachments && l.attachments.length > 0 ? "" : "rounded-tl-[20px]",
              l.text ? "rounded-br-[20px] rounded-bl-[20px] px-3 py-2 min-w-[80px]" : "absolute bottom-[8px] right-[8px] p-[4px] rounded-[8px] backdrop-blur-[20px]"
            ]])
          }, [
            l.replyingMessage ? (U(), T("div", Vd, [
              H(ee(cd), { class: "text-white" }),
              b("div", Cd, [
                b("span", Ed, Ae(o.value), 1),
                b("span", Sd, Ae(l.replyingMessage.text), 1)
              ])
            ])) : ye("", !0),
            b("div", {
              class: me(
                l.text && l.text.length > 30 ? "flex flex-col gap-[4px] w-full" : "grid grid-cols-[1fr_auto] gap-[4px] items-end w-full"
              )
            }, [
              l.text?.length ? (U(), T("div", Id, [
                b("span", {
                  class: me(["font-normal text-start text-sm break-words word-break-break-word whitespace-pre-wrap", [l.isMobile ? "text-[16px]" : "text-sm"]])
                }, Ae(l.text), 3)
              ])) : ye("", !0),
              b("div", {
                class: me(
                  l.text && l.text.length > 30 ? "flex gap-[4px] items-end justify-end" : "flex gap-[4px] items-end"
                )
              }, [
                b("span", Md, Ae(a(l.created_at)), 1),
                l.isReceived ? (U(), T("span", Bd, [
                  H(ee(Qh))
                ])) : (U(), T("span", kd, [
                  H(ee(Oh))
                ]))
              ], 2)
            ], 2)
          ], 6)
        ])
      ], 32)
    ]));
  }
}), Fd = {
  xmlns: "http://www.w3.org/2000/svg",
  width: "24",
  height: "24",
  fill: "none"
};
function Qd(e, t) {
  return U(), T("svg", Fd, [...t[0] || (t[0] = [
    b("path", {
      stroke: "currentColor",
      "stroke-linecap": "round",
      "stroke-linejoin": "round",
      "stroke-width": "1.5",
      d: "M9 22h6c5 0 7-2 7-7V9c0-5-2-7-7-7H9C4 2 2 4 2 9v6c0 5 2 7 7 7"
    }, null, -1),
    b("path", {
      stroke: "currentColor",
      "stroke-linecap": "round",
      "stroke-linejoin": "round",
      "stroke-miterlimit": "10",
      "stroke-width": "1.5",
      d: "M15 15.38h-4.92C8.38 15.38 7 14 7 12.3s1.38-3.08 3.08-3.08h6.77"
    }, null, -1),
    b("path", {
      stroke: "currentColor",
      "stroke-linecap": "round",
      "stroke-linejoin": "round",
      "stroke-width": "1.5",
      d: "M15.43 10.77 17 9.19l-1.57-1.57"
    }, null, -1)
  ])]);
}
const Ud = { render: Qd }, Dd = {
  xmlns: "http://www.w3.org/2000/svg",
  width: "24",
  height: "24",
  fill: "none"
};
function Od(e, t) {
  return U(), T("svg", Dd, [...t[0] || (t[0] = [
    b("path", {
      stroke: "currentColor",
      "stroke-linecap": "round",
      "stroke-linejoin": "round",
      "stroke-width": "1.5",
      d: "M16 12.9v4.2c0 3.5-1.4 4.9-4.9 4.9H6.9C3.4 22 2 20.6 2 17.1v-4.2C2 9.4 3.4 8 6.9 8h4.2c3.5 0 4.9 1.4 4.9 4.9"
    }, null, -1),
    b("path", {
      stroke: "currentColor",
      "stroke-linecap": "round",
      "stroke-linejoin": "round",
      "stroke-width": "1.5",
      d: "M22 6.9v4.2c0 3.5-1.4 4.9-4.9 4.9H16v-3.1C16 9.4 14.6 8 11.1 8H8V6.9C8 3.4 9.4 2 12.9 2h4.2C20.6 2 22 3.4 22 6.9"
    }, null, -1)
  ])]);
}
const Gd = { render: Od };
var cs = function(e, t) {
  return cs = Object.setPrototypeOf || { __proto__: [] } instanceof Array && function(n, r) {
    n.__proto__ = r;
  } || function(n, r) {
    for (var s in r) Object.prototype.hasOwnProperty.call(r, s) && (n[s] = r[s]);
  }, cs(e, t);
};
function ot(e, t) {
  if (typeof t != "function" && t !== null)
    throw new TypeError("Class extends value " + String(t) + " is not a constructor or null");
  cs(e, t);
  function n() {
    this.constructor = e;
  }
  e.prototype = t === null ? Object.create(t) : (n.prototype = t.prototype, new n());
}
var us = function() {
  return us = Object.assign || function(t) {
    for (var n, r = 1, s = arguments.length; r < s; r++) {
      n = arguments[r];
      for (var i in n) Object.prototype.hasOwnProperty.call(n, i) && (t[i] = n[i]);
    }
    return t;
  }, us.apply(this, arguments);
};
function Yd(e, t, n, r) {
  function s(i) {
    return i instanceof n ? i : new n(function(o) {
      o(i);
    });
  }
  return new (n || (n = Promise))(function(i, o) {
    function a(u) {
      try {
        c(r.next(u));
      } catch (h) {
        o(h);
      }
    }
    function l(u) {
      try {
        c(r.throw(u));
      } catch (h) {
        o(h);
      }
    }
    function c(u) {
      u.done ? i(u.value) : s(u.value).then(a, l);
    }
    c((r = r.apply(e, t || [])).next());
  });
}
function wa(e, t) {
  var n = { label: 0, sent: function() {
    if (i[0] & 1) throw i[1];
    return i[1];
  }, trys: [], ops: [] }, r, s, i, o = Object.create((typeof Iterator == "function" ? Iterator : Object).prototype);
  return o.next = a(0), o.throw = a(1), o.return = a(2), typeof Symbol == "function" && (o[Symbol.iterator] = function() {
    return this;
  }), o;
  function a(c) {
    return function(u) {
      return l([c, u]);
    };
  }
  function l(c) {
    if (r) throw new TypeError("Generator is already executing.");
    for (; o && (o = 0, c[0] && (n = 0)), n; ) try {
      if (r = 1, s && (i = c[0] & 2 ? s.return : c[0] ? s.throw || ((i = s.return) && i.call(s), 0) : s.next) && !(i = i.call(s, c[1])).done) return i;
      switch (s = 0, i && (c = [c[0] & 2, i.value]), c[0]) {
        case 0:
        case 1:
          i = c;
          break;
        case 4:
          return n.label++, { value: c[1], done: !1 };
        case 5:
          n.label++, s = c[1], c = [0];
          continue;
        case 7:
          c = n.ops.pop(), n.trys.pop();
          continue;
        default:
          if (i = n.trys, !(i = i.length > 0 && i[i.length - 1]) && (c[0] === 6 || c[0] === 2)) {
            n = 0;
            continue;
          }
          if (c[0] === 3 && (!i || c[1] > i[0] && c[1] < i[3])) {
            n.label = c[1];
            break;
          }
          if (c[0] === 6 && n.label < i[1]) {
            n.label = i[1], i = c;
            break;
          }
          if (i && n.label < i[2]) {
            n.label = i[2], n.ops.push(c);
            break;
          }
          i[2] && n.ops.pop(), n.trys.pop();
          continue;
      }
      c = t.call(e, n);
    } catch (u) {
      c = [6, u], s = 0;
    } finally {
      r = i = 0;
    }
    if (c[0] & 5) throw c[1];
    return { value: c[0] ? c[1] : void 0, done: !0 };
  }
}
function en(e) {
  var t = typeof Symbol == "function" && Symbol.iterator, n = t && e[t], r = 0;
  if (n) return n.call(e);
  if (e && typeof e.length == "number") return {
    next: function() {
      return e && r >= e.length && (e = void 0), { value: e && e[r++], done: !e };
    }
  };
  throw new TypeError(t ? "Object is not iterable." : "Symbol.iterator is not defined.");
}
function Bn(e, t) {
  var n = typeof Symbol == "function" && e[Symbol.iterator];
  if (!n) return e;
  var r = n.call(e), s, i = [], o;
  try {
    for (; (t === void 0 || t-- > 0) && !(s = r.next()).done; ) i.push(s.value);
  } catch (a) {
    o = { error: a };
  } finally {
    try {
      s && !s.done && (n = r.return) && n.call(r);
    } finally {
      if (o) throw o.error;
    }
  }
  return i;
}
function kn(e, t, n) {
  if (n || arguments.length === 2) for (var r = 0, s = t.length, i; r < s; r++)
    (i || !(r in t)) && (i || (i = Array.prototype.slice.call(t, 0, r)), i[r] = t[r]);
  return e.concat(i || Array.prototype.slice.call(t));
}
function $t(e) {
  return this instanceof $t ? (this.v = e, this) : new $t(e);
}
function Td(e, t, n) {
  if (!Symbol.asyncIterator) throw new TypeError("Symbol.asyncIterator is not defined.");
  var r = n.apply(e, t || []), s, i = [];
  return s = Object.create((typeof AsyncIterator == "function" ? AsyncIterator : Object).prototype), a("next"), a("throw"), a("return", o), s[Symbol.asyncIterator] = function() {
    return this;
  }, s;
  function o(p) {
    return function(w) {
      return Promise.resolve(w).then(p, h);
    };
  }
  function a(p, w) {
    r[p] && (s[p] = function(m) {
      return new Promise(function(Q, k) {
        i.push([p, m, Q, k]) > 1 || l(p, m);
      });
    }, w && (s[p] = w(s[p])));
  }
  function l(p, w) {
    try {
      c(r[p](w));
    } catch (m) {
      g(i[0][3], m);
    }
  }
  function c(p) {
    p.value instanceof $t ? Promise.resolve(p.value.v).then(u, h) : g(i[0][2], p);
  }
  function u(p) {
    l("next", p);
  }
  function h(p) {
    l("throw", p);
  }
  function g(p, w) {
    p(w), i.shift(), i.length && l(i[0][0], i[0][1]);
  }
}
function jd(e) {
  if (!Symbol.asyncIterator) throw new TypeError("Symbol.asyncIterator is not defined.");
  var t = e[Symbol.asyncIterator], n;
  return t ? t.call(e) : (e = typeof en == "function" ? en(e) : e[Symbol.iterator](), n = {}, r("next"), r("throw"), r("return"), n[Symbol.asyncIterator] = function() {
    return this;
  }, n);
  function r(i) {
    n[i] = e[i] && function(o) {
      return new Promise(function(a, l) {
        o = e[i](o), s(a, l, o.done, o.value);
      });
    };
  }
  function s(i, o, a, l) {
    Promise.resolve(l).then(function(c) {
      i({ value: c, done: a });
    }, o);
  }
}
function ve(e) {
  return typeof e == "function";
}
function ya(e) {
  var t = function(r) {
    Error.call(r), r.stack = new Error().stack;
  }, n = e(t);
  return n.prototype = Object.create(Error.prototype), n.prototype.constructor = n, n;
}
var Kr = ya(function(e) {
  return function(n) {
    e(this), this.message = n ? n.length + ` errors occurred during unsubscription:
` + n.map(function(r, s) {
      return s + 1 + ") " + r.toString();
    }).join(`
  `) : "", this.name = "UnsubscriptionError", this.errors = n;
  };
});
function cr(e, t) {
  if (e) {
    var n = e.indexOf(t);
    0 <= n && e.splice(n, 1);
  }
}
var rn = (function() {
  function e(t) {
    this.initialTeardown = t, this.closed = !1, this._parentage = null, this._finalizers = null;
  }
  return e.prototype.unsubscribe = function() {
    var t, n, r, s, i;
    if (!this.closed) {
      this.closed = !0;
      var o = this._parentage;
      if (o)
        if (this._parentage = null, Array.isArray(o))
          try {
            for (var a = en(o), l = a.next(); !l.done; l = a.next()) {
              var c = l.value;
              c.remove(this);
            }
          } catch (m) {
            t = { error: m };
          } finally {
            try {
              l && !l.done && (n = a.return) && n.call(a);
            } finally {
              if (t) throw t.error;
            }
          }
        else
          o.remove(this);
      var u = this.initialTeardown;
      if (ve(u))
        try {
          u();
        } catch (m) {
          i = m instanceof Kr ? m.errors : [m];
        }
      var h = this._finalizers;
      if (h) {
        this._finalizers = null;
        try {
          for (var g = en(h), p = g.next(); !p.done; p = g.next()) {
            var w = p.value;
            try {
              Gi(w);
            } catch (m) {
              i = i ?? [], m instanceof Kr ? i = kn(kn([], Bn(i)), Bn(m.errors)) : i.push(m);
            }
          }
        } catch (m) {
          r = { error: m };
        } finally {
          try {
            p && !p.done && (s = g.return) && s.call(g);
          } finally {
            if (r) throw r.error;
          }
        }
      }
      if (i)
        throw new Kr(i);
    }
  }, e.prototype.add = function(t) {
    var n;
    if (t && t !== this)
      if (this.closed)
        Gi(t);
      else {
        if (t instanceof e) {
          if (t.closed || t._hasParent(this))
            return;
          t._addParent(this);
        }
        (this._finalizers = (n = this._finalizers) !== null && n !== void 0 ? n : []).push(t);
      }
  }, e.prototype._hasParent = function(t) {
    var n = this._parentage;
    return n === t || Array.isArray(n) && n.includes(t);
  }, e.prototype._addParent = function(t) {
    var n = this._parentage;
    this._parentage = Array.isArray(n) ? (n.push(t), n) : n ? [n, t] : t;
  }, e.prototype._removeParent = function(t) {
    var n = this._parentage;
    n === t ? this._parentage = null : Array.isArray(n) && cr(n, t);
  }, e.prototype.remove = function(t) {
    var n = this._finalizers;
    n && cr(n, t), t instanceof e && t._removeParent(this);
  }, e.EMPTY = (function() {
    var t = new e();
    return t.closed = !0, t;
  })(), e;
})(), ba = rn.EMPTY;
function Aa(e) {
  return e instanceof rn || e && "closed" in e && ve(e.remove) && ve(e.add) && ve(e.unsubscribe);
}
function Gi(e) {
  ve(e) ? e() : e.unsubscribe();
}
var Pd = {
  Promise: void 0
}, Kd = {
  setTimeout: function(e, t) {
    for (var n = [], r = 2; r < arguments.length; r++)
      n[r - 2] = arguments[r];
    return setTimeout.apply(void 0, kn([e, t], Bn(n)));
  },
  clearTimeout: function(e) {
    return clearTimeout(e);
  },
  delegate: void 0
};
function xa(e) {
  Kd.setTimeout(function() {
    throw e;
  });
}
function Yi() {
}
function Hn(e) {
  e();
}
var Br = (function(e) {
  ot(t, e);
  function t(n) {
    var r = e.call(this) || this;
    return r.isStopped = !1, n ? (r.destination = n, Aa(n) && n.add(r)) : r.destination = Wd, r;
  }
  return t.create = function(n, r, s) {
    return new Rn(n, r, s);
  }, t.prototype.next = function(n) {
    this.isStopped || this._next(n);
  }, t.prototype.error = function(n) {
    this.isStopped || (this.isStopped = !0, this._error(n));
  }, t.prototype.complete = function() {
    this.isStopped || (this.isStopped = !0, this._complete());
  }, t.prototype.unsubscribe = function() {
    this.closed || (this.isStopped = !0, e.prototype.unsubscribe.call(this), this.destination = null);
  }, t.prototype._next = function(n) {
    this.destination.next(n);
  }, t.prototype._error = function(n) {
    try {
      this.destination.error(n);
    } finally {
      this.unsubscribe();
    }
  }, t.prototype._complete = function() {
    try {
      this.destination.complete();
    } finally {
      this.unsubscribe();
    }
  }, t;
})(rn), Nd = (function() {
  function e(t) {
    this.partialObserver = t;
  }
  return e.prototype.next = function(t) {
    var n = this.partialObserver;
    if (n.next)
      try {
        n.next(t);
      } catch (r) {
        Nn(r);
      }
  }, e.prototype.error = function(t) {
    var n = this.partialObserver;
    if (n.error)
      try {
        n.error(t);
      } catch (r) {
        Nn(r);
      }
    else
      Nn(t);
  }, e.prototype.complete = function() {
    var t = this.partialObserver;
    if (t.complete)
      try {
        t.complete();
      } catch (n) {
        Nn(n);
      }
  }, e;
})(), Rn = (function(e) {
  ot(t, e);
  function t(n, r, s) {
    var i = e.call(this) || this, o;
    return ve(n) || !n ? o = {
      next: n ?? void 0,
      error: r ?? void 0,
      complete: s ?? void 0
    } : o = n, i.destination = new Nd(o), i;
  }
  return t;
})(Br);
function Nn(e) {
  xa(e);
}
function Ld(e) {
  throw e;
}
var Wd = {
  closed: !0,
  next: Yi,
  error: Ld,
  complete: Yi
}, Ys = (function() {
  return typeof Symbol == "function" && Symbol.observable || "@@observable";
})();
function Ts(e) {
  return e;
}
function Jd(e) {
  return e.length === 0 ? Ts : e.length === 1 ? e[0] : function(n) {
    return e.reduce(function(r, s) {
      return s(r);
    }, n);
  };
}
var Re = (function() {
  function e(t) {
    t && (this._subscribe = t);
  }
  return e.prototype.lift = function(t) {
    var n = new e();
    return n.source = this, n.operator = t, n;
  }, e.prototype.subscribe = function(t, n, r) {
    var s = this, i = Zd(t) ? t : new Rn(t, n, r);
    return Hn(function() {
      var o = s, a = o.operator, l = o.source;
      i.add(a ? a.call(i, l) : l ? s._subscribe(i) : s._trySubscribe(i));
    }), i;
  }, e.prototype._trySubscribe = function(t) {
    try {
      return this._subscribe(t);
    } catch (n) {
      t.error(n);
    }
  }, e.prototype.forEach = function(t, n) {
    var r = this;
    return n = Ti(n), new n(function(s, i) {
      var o = new Rn({
        next: function(a) {
          try {
            t(a);
          } catch (l) {
            i(l), o.unsubscribe();
          }
        },
        error: i,
        complete: s
      });
      r.subscribe(o);
    });
  }, e.prototype._subscribe = function(t) {
    var n;
    return (n = this.source) === null || n === void 0 ? void 0 : n.subscribe(t);
  }, e.prototype[Ys] = function() {
    return this;
  }, e.prototype.pipe = function() {
    for (var t = [], n = 0; n < arguments.length; n++)
      t[n] = arguments[n];
    return Jd(t)(this);
  }, e.prototype.toPromise = function(t) {
    var n = this;
    return t = Ti(t), new t(function(r, s) {
      var i;
      n.subscribe(function(o) {
        return i = o;
      }, function(o) {
        return s(o);
      }, function() {
        return r(i);
      });
    });
  }, e.create = function(t) {
    return new e(t);
  }, e;
})();
function Ti(e) {
  var t;
  return (t = e ?? Pd.Promise) !== null && t !== void 0 ? t : Promise;
}
function qd(e) {
  return e && ve(e.next) && ve(e.error) && ve(e.complete);
}
function Zd(e) {
  return e && e instanceof Br || qd(e) && Aa(e);
}
var Hd = ya(function(e) {
  return function() {
    e(this), this.name = "ObjectUnsubscribedError", this.message = "object unsubscribed";
  };
}), vt = (function(e) {
  ot(t, e);
  function t() {
    var n = e.call(this) || this;
    return n.closed = !1, n.currentObservers = null, n.observers = [], n.isStopped = !1, n.hasError = !1, n.thrownError = null, n;
  }
  return t.prototype.lift = function(n) {
    var r = new fs(this, this);
    return r.operator = n, r;
  }, t.prototype._throwIfClosed = function() {
    if (this.closed)
      throw new Hd();
  }, t.prototype.next = function(n) {
    var r = this;
    Hn(function() {
      var s, i;
      if (r._throwIfClosed(), !r.isStopped) {
        r.currentObservers || (r.currentObservers = Array.from(r.observers));
        try {
          for (var o = en(r.currentObservers), a = o.next(); !a.done; a = o.next()) {
            var l = a.value;
            l.next(n);
          }
        } catch (c) {
          s = { error: c };
        } finally {
          try {
            a && !a.done && (i = o.return) && i.call(o);
          } finally {
            if (s) throw s.error;
          }
        }
      }
    });
  }, t.prototype.error = function(n) {
    var r = this;
    Hn(function() {
      if (r._throwIfClosed(), !r.isStopped) {
        r.hasError = r.isStopped = !0, r.thrownError = n;
        for (var s = r.observers; s.length; )
          s.shift().error(n);
      }
    });
  }, t.prototype.complete = function() {
    var n = this;
    Hn(function() {
      if (n._throwIfClosed(), !n.isStopped) {
        n.isStopped = !0;
        for (var r = n.observers; r.length; )
          r.shift().complete();
      }
    });
  }, t.prototype.unsubscribe = function() {
    this.isStopped = this.closed = !0, this.observers = this.currentObservers = null;
  }, Object.defineProperty(t.prototype, "observed", {
    get: function() {
      var n;
      return ((n = this.observers) === null || n === void 0 ? void 0 : n.length) > 0;
    },
    enumerable: !1,
    configurable: !0
  }), t.prototype._trySubscribe = function(n) {
    return this._throwIfClosed(), e.prototype._trySubscribe.call(this, n);
  }, t.prototype._subscribe = function(n) {
    return this._throwIfClosed(), this._checkFinalizedStatuses(n), this._innerSubscribe(n);
  }, t.prototype._innerSubscribe = function(n) {
    var r = this, s = this, i = s.hasError, o = s.isStopped, a = s.observers;
    return i || o ? ba : (this.currentObservers = null, a.push(n), new rn(function() {
      r.currentObservers = null, cr(a, n);
    }));
  }, t.prototype._checkFinalizedStatuses = function(n) {
    var r = this, s = r.hasError, i = r.thrownError, o = r.isStopped;
    s ? n.error(i) : o && n.complete();
  }, t.prototype.asObservable = function() {
    var n = new Re();
    return n.source = this, n;
  }, t.create = function(n, r) {
    return new fs(n, r);
  }, t;
})(Re), fs = (function(e) {
  ot(t, e);
  function t(n, r) {
    var s = e.call(this) || this;
    return s.destination = n, s.source = r, s;
  }
  return t.prototype.next = function(n) {
    var r, s;
    (s = (r = this.destination) === null || r === void 0 ? void 0 : r.next) === null || s === void 0 || s.call(r, n);
  }, t.prototype.error = function(n) {
    var r, s;
    (s = (r = this.destination) === null || r === void 0 ? void 0 : r.error) === null || s === void 0 || s.call(r, n);
  }, t.prototype.complete = function() {
    var n, r;
    (r = (n = this.destination) === null || n === void 0 ? void 0 : n.complete) === null || r === void 0 || r.call(n);
  }, t.prototype._subscribe = function(n) {
    var r, s;
    return (s = (r = this.source) === null || r === void 0 ? void 0 : r.subscribe(n)) !== null && s !== void 0 ? s : ba;
  }, t;
})(vt), js = {
  now: function() {
    return (js.delegate || Date).now();
  },
  delegate: void 0
}, zn = (function(e) {
  ot(t, e);
  function t(n, r, s) {
    n === void 0 && (n = 1 / 0), r === void 0 && (r = 1 / 0), s === void 0 && (s = js);
    var i = e.call(this) || this;
    return i._bufferSize = n, i._windowTime = r, i._timestampProvider = s, i._buffer = [], i._infiniteTimeWindow = !0, i._infiniteTimeWindow = r === 1 / 0, i._bufferSize = Math.max(1, n), i._windowTime = Math.max(1, r), i;
  }
  return t.prototype.next = function(n) {
    var r = this, s = r.isStopped, i = r._buffer, o = r._infiniteTimeWindow, a = r._timestampProvider, l = r._windowTime;
    s || (i.push(n), !o && i.push(a.now() + l)), this._trimBuffer(), e.prototype.next.call(this, n);
  }, t.prototype._subscribe = function(n) {
    this._throwIfClosed(), this._trimBuffer();
    for (var r = this._innerSubscribe(n), s = this, i = s._infiniteTimeWindow, o = s._buffer, a = o.slice(), l = 0; l < a.length && !n.closed; l += i ? 1 : 2)
      n.next(a[l]);
    return this._checkFinalizedStatuses(n), r;
  }, t.prototype._trimBuffer = function() {
    var n = this, r = n._bufferSize, s = n._timestampProvider, i = n._buffer, o = n._infiniteTimeWindow, a = (o ? 1 : 2) * r;
    if (r < 1 / 0 && a < i.length && i.splice(0, i.length - a), !o) {
      for (var l = s.now(), c = 0, u = 1; u < i.length && i[u] <= l; u += 2)
        c = u;
      c && i.splice(0, c + 1);
    }
  }, t;
})(vt), zd = {
  url: "",
  deserializer: function(e) {
    return JSON.parse(e.data);
  },
  serializer: function(e) {
    return JSON.stringify(e);
  }
}, Xd = "WebSocketSubject.error must be called with an object with an error code, and an optional reason: { code: number, reason: string }", _d = (function(e) {
  ot(t, e);
  function t(n, r) {
    var s = e.call(this) || this;
    if (s._socket = null, n instanceof Re)
      s.destination = r, s.source = n;
    else {
      var i = s._config = us({}, zd);
      if (s._output = new vt(), typeof n == "string")
        i.url = n;
      else
        for (var o in n)
          n.hasOwnProperty(o) && (i[o] = n[o]);
      if (!i.WebSocketCtor && WebSocket)
        i.WebSocketCtor = WebSocket;
      else if (!i.WebSocketCtor)
        throw new Error("no WebSocket constructor can be found");
      s.destination = new zn();
    }
    return s;
  }
  return t.prototype.lift = function(n) {
    var r = new t(this._config, this.destination);
    return r.operator = n, r.source = this, r;
  }, t.prototype._resetState = function() {
    this._socket = null, this.source || (this.destination = new zn()), this._output = new vt();
  }, t.prototype.multiplex = function(n, r, s) {
    var i = this;
    return new Re(function(o) {
      try {
        i.next(n());
      } catch (l) {
        o.error(l);
      }
      var a = i.subscribe({
        next: function(l) {
          try {
            s(l) && o.next(l);
          } catch (c) {
            o.error(c);
          }
        },
        error: function(l) {
          return o.error(l);
        },
        complete: function() {
          return o.complete();
        }
      });
      return function() {
        try {
          i.next(r());
        } catch (l) {
          o.error(l);
        }
        a.unsubscribe();
      };
    });
  }, t.prototype._connectSocket = function() {
    var n = this, r = this._config, s = r.WebSocketCtor, i = r.protocol, o = r.url, a = r.binaryType, l = this._output, c = null;
    try {
      c = i ? new s(o, i) : new s(o), this._socket = c, a && (this._socket.binaryType = a);
    } catch (h) {
      l.error(h);
      return;
    }
    var u = new rn(function() {
      n._socket = null, c && c.readyState === 1 && c.close();
    });
    c.onopen = function(h) {
      var g = n._socket;
      if (!g) {
        c.close(), n._resetState();
        return;
      }
      var p = n._config.openObserver;
      p && p.next(h);
      var w = n.destination;
      n.destination = Br.create(function(m) {
        if (c.readyState === 1)
          try {
            var Q = n._config.serializer;
            c.send(Q(m));
          } catch (k) {
            n.destination.error(k);
          }
      }, function(m) {
        var Q = n._config.closingObserver;
        Q && Q.next(void 0), m && m.code ? c.close(m.code, m.reason) : l.error(new TypeError(Xd)), n._resetState();
      }, function() {
        var m = n._config.closingObserver;
        m && m.next(void 0), c.close(), n._resetState();
      }), w && w instanceof zn && u.add(w.subscribe(n.destination));
    }, c.onerror = function(h) {
      n._resetState(), l.error(h);
    }, c.onclose = function(h) {
      c === n._socket && n._resetState();
      var g = n._config.closeObserver;
      g && g.next(h), h.wasClean ? l.complete() : l.error(h);
    }, c.onmessage = function(h) {
      try {
        var g = n._config.deserializer;
        l.next(g(h));
      } catch (p) {
        l.error(p);
      }
    };
  }, t.prototype._subscribe = function(n) {
    var r = this, s = this.source;
    return s ? s.subscribe(n) : (this._socket || this._connectSocket(), this._output.subscribe(n), n.add(function() {
      var i = r._socket;
      r._output.observers.length === 0 && (i && (i.readyState === 1 || i.readyState === 0) && i.close(), r._resetState());
    }), n);
  }, t.prototype.unsubscribe = function() {
    var n = this._socket;
    n && (n.readyState === 1 || n.readyState === 0) && n.close(), this._resetState(), e.prototype.unsubscribe.call(this);
  }, t;
})(fs);
function $d(e) {
  return new _d(e);
}
function ep(e) {
  return ve(e?.lift);
}
function Xe(e) {
  return function(t) {
    if (ep(t))
      return t.lift(function(n) {
        try {
          return e(n, this);
        } catch (r) {
          this.error(r);
        }
      });
    throw new TypeError("Unable to lift unknown Observable type");
  };
}
function qe(e, t, n, r, s) {
  return new tp(e, t, n, r, s);
}
var tp = (function(e) {
  ot(t, e);
  function t(n, r, s, i, o, a) {
    var l = e.call(this, n) || this;
    return l.onFinalize = o, l.shouldUnsubscribe = a, l._next = r ? function(c) {
      try {
        r(c);
      } catch (u) {
        n.error(u);
      }
    } : e.prototype._next, l._error = i ? function(c) {
      try {
        i(c);
      } catch (u) {
        n.error(u);
      } finally {
        this.unsubscribe();
      }
    } : e.prototype._error, l._complete = s ? function() {
      try {
        s();
      } catch (c) {
        n.error(c);
      } finally {
        this.unsubscribe();
      }
    } : e.prototype._complete, l;
  }
  return t.prototype.unsubscribe = function() {
    var n;
    if (!this.shouldUnsubscribe || this.shouldUnsubscribe()) {
      var r = this.closed;
      e.prototype.unsubscribe.call(this), !r && ((n = this.onFinalize) === null || n === void 0 || n.call(this));
    }
  }, t;
})(Br), np = (function(e) {
  ot(t, e);
  function t(n, r) {
    return e.call(this) || this;
  }
  return t.prototype.schedule = function(n, r) {
    return this;
  }, t;
})(rn), ji = {
  setInterval: function(e, t) {
    for (var n = [], r = 2; r < arguments.length; r++)
      n[r - 2] = arguments[r];
    return setInterval.apply(void 0, kn([e, t], Bn(n)));
  },
  clearInterval: function(e) {
    return clearInterval(e);
  },
  delegate: void 0
}, rp = (function(e) {
  ot(t, e);
  function t(n, r) {
    var s = e.call(this, n, r) || this;
    return s.scheduler = n, s.work = r, s.pending = !1, s;
  }
  return t.prototype.schedule = function(n, r) {
    var s;
    if (r === void 0 && (r = 0), this.closed)
      return this;
    this.state = n;
    var i = this.id, o = this.scheduler;
    return i != null && (this.id = this.recycleAsyncId(o, i, r)), this.pending = !0, this.delay = r, this.id = (s = this.id) !== null && s !== void 0 ? s : this.requestAsyncId(o, this.id, r), this;
  }, t.prototype.requestAsyncId = function(n, r, s) {
    return s === void 0 && (s = 0), ji.setInterval(n.flush.bind(n, this), s);
  }, t.prototype.recycleAsyncId = function(n, r, s) {
    if (s === void 0 && (s = 0), s != null && this.delay === s && this.pending === !1)
      return r;
    r != null && ji.clearInterval(r);
  }, t.prototype.execute = function(n, r) {
    if (this.closed)
      return new Error("executing a cancelled action");
    this.pending = !1;
    var s = this._execute(n, r);
    if (s)
      return s;
    this.pending === !1 && this.id != null && (this.id = this.recycleAsyncId(this.scheduler, this.id, null));
  }, t.prototype._execute = function(n, r) {
    var s = !1, i;
    try {
      this.work(n);
    } catch (o) {
      s = !0, i = o || new Error("Scheduled action threw falsy error");
    }
    if (s)
      return this.unsubscribe(), i;
  }, t.prototype.unsubscribe = function() {
    if (!this.closed) {
      var n = this, r = n.id, s = n.scheduler, i = s.actions;
      this.work = this.state = this.scheduler = null, this.pending = !1, cr(i, this), r != null && (this.id = this.recycleAsyncId(s, r, null)), this.delay = null, e.prototype.unsubscribe.call(this);
    }
  }, t;
})(np), Pi = (function() {
  function e(t, n) {
    n === void 0 && (n = e.now), this.schedulerActionCtor = t, this.now = n;
  }
  return e.prototype.schedule = function(t, n, r) {
    return n === void 0 && (n = 0), new this.schedulerActionCtor(this, t).schedule(r, n);
  }, e.now = js.now, e;
})(), sp = (function(e) {
  ot(t, e);
  function t(n, r) {
    r === void 0 && (r = Pi.now);
    var s = e.call(this, n, r) || this;
    return s.actions = [], s._active = !1, s;
  }
  return t.prototype.flush = function(n) {
    var r = this.actions;
    if (this._active) {
      r.push(n);
      return;
    }
    var s;
    this._active = !0;
    do
      if (s = n.execute(n.state, n.delay))
        break;
    while (n = r.shift());
    if (this._active = !1, s) {
      for (; n = r.shift(); )
        n.unsubscribe();
      throw s;
    }
  }, t;
})(Pi), ip = new sp(rp), op = ip, Ps = new Re(function(e) {
  return e.complete();
});
function ap(e) {
  return e && ve(e.schedule);
}
function Va(e) {
  return e[e.length - 1];
}
function Ca(e) {
  return ap(Va(e)) ? e.pop() : void 0;
}
function lp(e, t) {
  return typeof Va(e) == "number" ? e.pop() : t;
}
var Ea = (function(e) {
  return e && typeof e.length == "number" && typeof e != "function";
});
function Sa(e) {
  return ve(e?.then);
}
function Ia(e) {
  return ve(e[Ys]);
}
function Ma(e) {
  return Symbol.asyncIterator && ve(e?.[Symbol.asyncIterator]);
}
function Ba(e) {
  return new TypeError("You provided " + (e !== null && typeof e == "object" ? "an invalid object" : "'" + e + "'") + " where a stream was expected. You can provide an Observable, Promise, ReadableStream, Array, AsyncIterable, or Iterable.");
}
function cp() {
  return typeof Symbol != "function" || !Symbol.iterator ? "@@iterator" : Symbol.iterator;
}
var ka = cp();
function Ra(e) {
  return ve(e?.[ka]);
}
function Fa(e) {
  return Td(this, arguments, function() {
    var n, r, s, i;
    return wa(this, function(o) {
      switch (o.label) {
        case 0:
          n = e.getReader(), o.label = 1;
        case 1:
          o.trys.push([1, , 9, 10]), o.label = 2;
        case 2:
          return [4, $t(n.read())];
        case 3:
          return r = o.sent(), s = r.value, i = r.done, i ? [4, $t(void 0)] : [3, 5];
        case 4:
          return [2, o.sent()];
        case 5:
          return [4, $t(s)];
        case 6:
          return [4, o.sent()];
        case 7:
          return o.sent(), [3, 2];
        case 8:
          return [3, 10];
        case 9:
          return n.releaseLock(), [7];
        case 10:
          return [2];
      }
    });
  });
}
function Qa(e) {
  return ve(e?.getReader);
}
function _e(e) {
  if (e instanceof Re)
    return e;
  if (e != null) {
    if (Ia(e))
      return up(e);
    if (Ea(e))
      return fp(e);
    if (Sa(e))
      return hp(e);
    if (Ma(e))
      return Ua(e);
    if (Ra(e))
      return dp(e);
    if (Qa(e))
      return pp(e);
  }
  throw Ba(e);
}
function up(e) {
  return new Re(function(t) {
    var n = e[Ys]();
    if (ve(n.subscribe))
      return n.subscribe(t);
    throw new TypeError("Provided object does not correctly implement Symbol.observable");
  });
}
function fp(e) {
  return new Re(function(t) {
    for (var n = 0; n < e.length && !t.closed; n++)
      t.next(e[n]);
    t.complete();
  });
}
function hp(e) {
  return new Re(function(t) {
    e.then(function(n) {
      t.closed || (t.next(n), t.complete());
    }, function(n) {
      return t.error(n);
    }).then(null, xa);
  });
}
function dp(e) {
  return new Re(function(t) {
    var n, r;
    try {
      for (var s = en(e), i = s.next(); !i.done; i = s.next()) {
        var o = i.value;
        if (t.next(o), t.closed)
          return;
      }
    } catch (a) {
      n = { error: a };
    } finally {
      try {
        i && !i.done && (r = s.return) && r.call(s);
      } finally {
        if (n) throw n.error;
      }
    }
    t.complete();
  });
}
function Ua(e) {
  return new Re(function(t) {
    gp(e, t).catch(function(n) {
      return t.error(n);
    });
  });
}
function pp(e) {
  return Ua(Fa(e));
}
function gp(e, t) {
  var n, r, s, i;
  return Yd(this, void 0, void 0, function() {
    var o, a;
    return wa(this, function(l) {
      switch (l.label) {
        case 0:
          l.trys.push([0, 5, 6, 11]), n = jd(e), l.label = 1;
        case 1:
          return [4, n.next()];
        case 2:
          if (r = l.sent(), !!r.done) return [3, 4];
          if (o = r.value, t.next(o), t.closed)
            return [2];
          l.label = 3;
        case 3:
          return [3, 1];
        case 4:
          return [3, 11];
        case 5:
          return a = l.sent(), s = { error: a }, [3, 11];
        case 6:
          return l.trys.push([6, , 9, 10]), r && !r.done && (i = n.return) ? [4, i.call(n)] : [3, 8];
        case 7:
          l.sent(), l.label = 8;
        case 8:
          return [3, 10];
        case 9:
          if (s) throw s.error;
          return [7];
        case 10:
          return [7];
        case 11:
          return t.complete(), [2];
      }
    });
  });
}
function Pt(e, t, n, r, s) {
  r === void 0 && (r = 0), s === void 0 && (s = !1);
  var i = t.schedule(function() {
    n(), s ? e.add(this.schedule(null, r)) : this.unsubscribe();
  }, r);
  if (e.add(i), !s)
    return i;
}
function Da(e, t) {
  return t === void 0 && (t = 0), Xe(function(n, r) {
    n.subscribe(qe(r, function(s) {
      return Pt(r, e, function() {
        return r.next(s);
      }, t);
    }, function() {
      return Pt(r, e, function() {
        return r.complete();
      }, t);
    }, function(s) {
      return Pt(r, e, function() {
        return r.error(s);
      }, t);
    }));
  });
}
function Oa(e, t) {
  return t === void 0 && (t = 0), Xe(function(n, r) {
    r.add(e.schedule(function() {
      return n.subscribe(r);
    }, t));
  });
}
function mp(e, t) {
  return _e(e).pipe(Oa(t), Da(t));
}
function vp(e, t) {
  return _e(e).pipe(Oa(t), Da(t));
}
function wp(e, t) {
  return new Re(function(n) {
    var r = 0;
    return t.schedule(function() {
      r === e.length ? n.complete() : (n.next(e[r++]), n.closed || this.schedule());
    });
  });
}
function yp(e, t) {
  return new Re(function(n) {
    var r;
    return Pt(n, t, function() {
      r = e[ka](), Pt(n, t, function() {
        var s, i, o;
        try {
          s = r.next(), i = s.value, o = s.done;
        } catch (a) {
          n.error(a);
          return;
        }
        o ? n.complete() : n.next(i);
      }, 0, !0);
    }), function() {
      return ve(r?.return) && r.return();
    };
  });
}
function Ga(e, t) {
  if (!e)
    throw new Error("Iterable cannot be null");
  return new Re(function(n) {
    Pt(n, t, function() {
      var r = e[Symbol.asyncIterator]();
      Pt(n, t, function() {
        r.next().then(function(s) {
          s.done ? n.complete() : n.next(s.value);
        });
      }, 0, !0);
    });
  });
}
function bp(e, t) {
  return Ga(Fa(e), t);
}
function Ap(e, t) {
  if (e != null) {
    if (Ia(e))
      return mp(e, t);
    if (Ea(e))
      return wp(e, t);
    if (Sa(e))
      return vp(e, t);
    if (Ma(e))
      return Ga(e, t);
    if (Ra(e))
      return yp(e, t);
    if (Qa(e))
      return bp(e, t);
  }
  throw Ba(e);
}
function Ya(e, t) {
  return t ? Ap(e, t) : _e(e);
}
function xp() {
  for (var e = [], t = 0; t < arguments.length; t++)
    e[t] = arguments[t];
  var n = Ca(e);
  return Ya(e, n);
}
function Vp(e) {
  return e instanceof Date && !isNaN(e);
}
function Ta(e, t) {
  return Xe(function(n, r) {
    var s = 0;
    n.subscribe(qe(r, function(i) {
      r.next(e.call(t, i, s++));
    }));
  });
}
function Cp(e, t, n, r, s, i, o, a) {
  var l = [], c = 0, u = 0, h = !1, g = function() {
    h && !l.length && !c && t.complete();
  }, p = function(m) {
    return c < r ? w(m) : l.push(m);
  }, w = function(m) {
    c++;
    var Q = !1;
    _e(n(m, u++)).subscribe(qe(t, function(k) {
      t.next(k);
    }, function() {
      Q = !0;
    }, void 0, function() {
      if (Q)
        try {
          c--;
          for (var k = function() {
            var M = l.shift();
            o || w(M);
          }; l.length && c < r; )
            k();
          g();
        } catch (M) {
          t.error(M);
        }
    }));
  };
  return e.subscribe(qe(t, p, function() {
    h = !0, g();
  })), function() {
  };
}
function Ks(e, t, n) {
  return n === void 0 && (n = 1 / 0), ve(t) ? Ks(function(r, s) {
    return Ta(function(i, o) {
      return t(r, i, s, o);
    })(_e(e(r, s)));
  }, n) : (typeof t == "number" && (n = t), Xe(function(r, s) {
    return Cp(r, s, e, n);
  }));
}
function Ep(e) {
  return e === void 0 && (e = 1 / 0), Ks(Ts, e);
}
function Sp(e, t, n) {
  return e === void 0 && (e = 0), n === void 0 && (n = op), new Re(function(r) {
    var s = Vp(e) ? +e - n.now() : e;
    s < 0 && (s = 0);
    var i = 0;
    return n.schedule(function() {
      r.closed || (r.next(i++), r.complete());
    }, s);
  });
}
function Ip() {
  for (var e = [], t = 0; t < arguments.length; t++)
    e[t] = arguments[t];
  var n = Ca(e), r = lp(e, 1 / 0), s = e;
  return s.length ? s.length === 1 ? _e(s[0]) : Ep(r)(Ya(s, n)) : Ps;
}
function Jt(e, t) {
  return Xe(function(n, r) {
    var s = 0;
    n.subscribe(qe(r, function(i) {
      return e.call(t, i, s++) && r.next(i);
    }));
  });
}
function Mp(e, t, n, r, s) {
  return function(i, o) {
    var a = n, l = t, c = 0;
    i.subscribe(qe(o, function(u) {
      var h = c++;
      l = a ? e(l, u, h) : (a = !0, u), o.next(l);
    }, s));
  };
}
function Bp(e) {
  return e <= 0 ? function() {
    return Ps;
  } : Xe(function(t, n) {
    var r = 0;
    t.subscribe(qe(n, function(s) {
      ++r <= e && (n.next(s), e <= r && n.complete());
    }));
  });
}
function kp(e) {
  return Ta(function() {
    return e;
  });
}
function Rp(e, t) {
  return Ks(function(n, r) {
    return _e(e(n, r)).pipe(Bp(1), kp(n));
  });
}
function Fp(e) {
  return Xe(function(t, n) {
    var r, s = !1, i, o = function() {
      r = t.subscribe(qe(n, void 0, void 0, function(a) {
        i || (i = new vt(), _e(e(i)).subscribe(qe(n, function() {
          return r ? o() : s = !0;
        }))), i && i.next(a);
      })), s && (r.unsubscribe(), r = null, s = !1, o());
    };
    o();
  });
}
function Qp(e, t) {
  return Xe(Mp(e, t, arguments.length >= 2, !0));
}
function Up(e) {
  e === void 0 && (e = {});
  var t = e.connector, n = t === void 0 ? function() {
    return new vt();
  } : t, r = e.resetOnError, s = r === void 0 ? !0 : r, i = e.resetOnComplete, o = i === void 0 ? !0 : i, a = e.resetOnRefCountZero, l = a === void 0 ? !0 : a;
  return function(c) {
    var u, h, g, p = 0, w = !1, m = !1, Q = function() {
      h?.unsubscribe(), h = void 0;
    }, k = function() {
      Q(), u = g = void 0, w = m = !1;
    }, M = function() {
      var G = u;
      k(), G?.unsubscribe();
    };
    return Xe(function(G, V) {
      p++, !m && !w && Q();
      var v = g = g ?? n();
      V.add(function() {
        p--, p === 0 && !m && !w && (h = Nr(M, l));
      }), v.subscribe(V), !u && p > 0 && (u = new Rn({
        next: function(C) {
          return v.next(C);
        },
        error: function(C) {
          m = !0, Q(), h = Nr(k, s, C), v.error(C);
        },
        complete: function() {
          w = !0, Q(), h = Nr(k, o), v.complete();
        }
      }), _e(G).subscribe(u));
    })(c);
  };
}
function Nr(e, t) {
  for (var n = [], r = 2; r < arguments.length; r++)
    n[r - 2] = arguments[r];
  if (t === !0) {
    e();
    return;
  }
  if (t !== !1) {
    var s = new Rn({
      next: function() {
        s.unsubscribe(), e();
      }
    });
    return _e(t.apply(void 0, kn([], Bn(n)))).subscribe(s);
  }
}
function Dp(e, t, n) {
  var r, s, i, o, a = !1;
  return e && typeof e == "object" ? (r = e.bufferSize, o = r === void 0 ? 1 / 0 : r, s = e.windowTime, t = s === void 0 ? 1 / 0 : s, i = e.refCount, a = i === void 0 ? !1 : i, n = e.scheduler) : o = e ?? 1 / 0, Up({
    connector: function() {
      return new zn(o, t, n);
    },
    resetOnError: !0,
    resetOnComplete: !1,
    resetOnRefCountZero: a
  });
}
function Op(e, t) {
  return Xe(function(n, r) {
    var s = null, i = 0, o = !1, a = function() {
      return o && !s && r.complete();
    };
    n.subscribe(qe(r, function(l) {
      s?.unsubscribe();
      var c = 0, u = i++;
      _e(e(l, u)).subscribe(s = qe(r, function(h) {
        return r.next(t ? t(l, h, u, c++) : h);
      }, function() {
        s = null, a();
      }));
    }, function() {
      o = !0, a();
    }));
  });
}
function fn(e, t, n) {
  var r = ve(e) || t || n ? { next: e, error: t, complete: n } : e;
  return r ? Xe(function(s, i) {
    var o;
    (o = r.subscribe) === null || o === void 0 || o.call(r);
    var a = !0;
    s.subscribe(qe(i, function(l) {
      var c;
      (c = r.next) === null || c === void 0 || c.call(r, l), i.next(l);
    }, function() {
      var l;
      a = !1, (l = r.complete) === null || l === void 0 || l.call(r), i.complete();
    }, function(l) {
      var c;
      a = !1, (c = r.error) === null || c === void 0 || c.call(r, l), i.error(l);
    }, function() {
      var l, c;
      a && ((l = r.unsubscribe) === null || l === void 0 || l.call(r)), (c = r.finalize) === null || c === void 0 || c.call(r);
    }));
  }) : Ts;
}
function Gp(e) {
  const {
    conversationId: t,
    afterSeq: n = L(0),
    baseUrl: r = Ru(),
    maxBackoffMs: s = 15e3,
    shouldReconnect: i = L(!0),
    // По умолчанию переподключения включены
    onCreateNewConversation: o,
    onReconnected: a
  } = e, l = L(!1), c = Xs(null), u = L([]), h = new vt(), g = new vt(), p = new vt(), w = Xs(null), m = /* @__PURE__ */ new Map(), Q = 2e4, k = L(null), M = L(Date.now()), G = 15e3, V = 1e4;
  function v() {
    w.value && (console.log("[WS] Cleaning up current socket connection"), w.value.complete(), w.value = null), l.value = !1;
  }
  function C() {
    E(), k.value = setInterval(() => {
      const D = w.value;
      l.value && D && (D.next({ type: "ping", data: {} }), setTimeout(() => {
        Date.now() - M.value > V && (console.warn("[WS] Pong timeout - triggering reconnection"), w.value && w.value.complete(), l.value = !1, c.value = new Error("Pong timeout"), E(), setTimeout(() => {
          g.next();
        }, 1e3));
      }, V));
    }, G);
  }
  function E() {
    k.value && (clearInterval(k.value), k.value = null);
  }
  function Y() {
    if (!$.value)
      throw new Error("No conversation ID provided");
    const D = $d({
      url: $.value,
      serializer: (K) => JSON.stringify(K),
      deserializer: (K) => {
        try {
          return JSON.parse(K.data);
        } catch (oe) {
          throw console.warn("Failed to parse WebSocket message:", K.data), oe;
        }
      },
      openObserver: {
        next: () => {
          for (l.value = !0, c.value = null, M.value = Date.now(), C(), a && a(); u.value.length > 0; ) {
            const K = u.value.shift();
            if (K)
              try {
                D.next(K), console.log("[WS] Sent queued message:", K);
              } catch (oe) {
                console.warn("[WS] Failed to send queued message:", oe), u.value.unshift(K);
                break;
              }
          }
        }
      },
      closeObserver: {
        next: (K) => {
          l.value = !1, E(), console.log("[WS] Disconnected:", K.code, K.reason);
        }
      }
    });
    return w.value = D, D;
  }
  const $ = j(() => {
    if (!t.value) return "";
    const D = new URLSearchParams();
    return D.set("conv_id", t.value), n.value > 0 && D.set("after_seq", n.value.toString()), `${r}/api/v1/messages/ws?${D.toString()}`;
  }), z = Ip(
    xp(null),
    // Запускаем сразу
    g.pipe(
      fn(() => {
        console.log("[WS] Force reconnection triggered"), v();
      })
    ),
    p.pipe(
      fn(() => {
        console.log("[WS] Conversation changed, reconnecting"), v();
      })
    )
  ).pipe(
    Op(() => t.value ? (console.log("[WS] Creating new socket connection"), Y().pipe(
      Fp(
        (D) => D.pipe(
          fn((K) => {
            c.value = K, console.error("[WS] Error:", K);
          }),
          Jt(() => {
            const K = i.value;
            return K || console.log("[WS] Reconnection disabled, stopping retries"), K;
          }),
          Qp((K) => Math.min(K * 2 || 1e3, s), 0),
          Rp((K) => (console.log(`[WS] Retrying in ${K}ms...`), Sp(K)))
        )
      )
    )) : (console.log("[WS] Waiting for conversation ID"), Ps)),
    Dp({ bufferSize: 1, refCount: !1 })
  );
  function he(D) {
    const K = w.value;
    if (l.value && K)
      try {
        if (K.next(D), console.log("[WS] Sent message:", D), D.type === "message.send" && D.data?.client_id) {
          const oe = D.data.client_id, lt = setTimeout(() => {
            console.warn(`[WS] Message ${oe} timeout - connection may be dead`), l.value = !1, c.value = new Error("Message delivery timeout"), m.delete(oe);
          }, Q);
          m.set(oe, { timestamp: Date.now(), timeout: lt }), console.log(`[WS] Tracking message ${oe} for delivery`);
        }
      } catch (oe) {
        console.warn("[WS] Failed to send, adding to outbox:", oe), u.value.push(D);
      }
    else
      u.value.push(D);
  }
  function at(D, K, oe, lt) {
    if (!i.value)
      if (console.log("[WS] Re-enabling reconnections for new message"), i.value = !0, o)
        console.log("[WS] Creating new conversation for new message"), o().then(() => {
          console.log("[WS] New conversation created, creating socket connection");
          try {
            v(), Y().subscribe();
          } catch (Qt) {
            console.error("[WS] Failed to create new connection:", Qt);
          }
        }).catch((Qt) => {
          console.error("[WS] Failed to create new conversation:", Qt);
        });
      else {
        console.log("[WS] Creating new connection for new message (no conversation callback)");
        try {
          v(), Y().subscribe();
        } catch (Qt) {
          console.error("[WS] Failed to create new connection:", Qt);
        }
      }
    he({
      type: "message.send",
      data: {
        text: D,
        client_id: K,
        reply_to_message_id: oe,
        attachments: lt
      }
    }), console.log("[WS] Queued message:", D);
  }
  const Ve = z.pipe(
    Jt((D) => !!D && typeof D == "object" && "type" in D),
    fn((D) => {
      if (c.value && (console.log("[WS] Received message, clearing connection error"), c.value = null), D.type === "pong" && (M.value = Date.now(), console.log("[WS] Received pong from server")), D.type === "message.created" && D.data?.client_id) {
        const K = D.data.client_id, oe = m.get(K);
        oe && (clearTimeout(oe.timeout), m.delete(K));
      }
    })
  ), pe = Ve.pipe(
    Jt((D) => D.type === "welcome")
  ), Be = Ve.pipe(
    Jt(
      (D) => D.type === "message.created" || D.type === "conversation.closed"
    )
  ), J = Ve.pipe(Jt((D) => D.type === "conversation.closed")), q = Ve.pipe(Jt((D) => D.type === "error")), ie = j(() => l.value ? "CONNECTED" : c.value ? "ERROR" : "DISCONNECTED"), de = j(() => u.value.length > 0);
  Ue(t, (D, K) => {
    D && D !== K && p.next();
  });
  const ce = z.subscribe({
    error: (D) => {
      c.value = D, console.error("[WS] Socket stream error:", D);
    }
  });
  return Qn(() => {
    console.log("[WS] Cleaning up..."), ce.unsubscribe();
    for (const [, D] of m)
      clearTimeout(D.timeout);
    m.clear(), E(), h.next(), h.complete(), g.complete(), p.complete(), w.value?.complete();
  }), {
    // State
    connected: j(() => l.value),
    statusText: ie,
    lastError: j(() => c.value),
    hasQueuedMessages: de,
    // RxJS Streams
    welcomeMessages$: pe,
    // Welcome сообщения при подключении
    newMessages$: Be,
    // Новые сообщения чата
    errorMessages$: q,
    // Ошибки от сервера
    closeConversation$: J,
    // Сообщения о закрытии беседы
    // Actions
    send: he,
    // Отправка произвольного сообщения
    sendMessage: at,
    // Отправка текстового сообщения
    // Control methods
    disableReconnection: () => {
      i.value = !1, console.log("[WS] Reconnection disabled");
    },
    enableReconnection: () => {
      i.value = !0, console.log("[WS] Reconnection enabled");
    },
    disconnect: () => {
      i.value = !1, v(), console.log("[WS] Manually disconnecting...");
    }
  };
}
function Yp() {
  const e = typeof globalThis < "u" ? globalThis.crypto : void 0;
  if (e && typeof e.randomUUID == "function")
    return e.randomUUID();
  if (e && typeof e.getRandomValues == "function") {
    const n = new Uint8Array(16);
    e.getRandomValues(n), n[6] = n[6] & 15 | 64, n[8] = n[8] & 63 | 128;
    const r = Array.from(n).map((s) => s.toString(16).padStart(2, "0"));
    return `${r.slice(0, 4).join("")}-${r.slice(4, 6).join("")}-${r.slice(6, 8).join("")}-${r.slice(8, 10).join("")}-${r.slice(10, 16).join("")}`;
  }
  const t = () => Math.floor((1 + Math.random()) * 65536).toString(16).substring(1);
  return `${t()}${t()}-${t()}-${t()}-${t()}${t()}${t()}`;
}
const Tp = "data:audio/mpeg;base64,", jp = {
  key: 0,
  class: "absolute inset-0 z-50 flex items-center justify-center backdrop-blur-[25px] -m-4",
  style: { background: "rgba(108, 111, 172, 0.08)" }
}, Pp = { class: "text-center text-neutral-50 font-golos" }, Kp = { class: "font-normal text-[12px] leading-[20px] text-neutral-50 font-unbounded" }, Np = {
  key: 0,
  class: "flex justify-center items-center gap-[3px] mt-[9px]"
}, Lp = {
  id: "footer",
  class: "flex-shrink-0 w-full mt-[12px]"
}, Wp = /* @__PURE__ */ Me({
  __name: "ChatContent",
  props: {
    msgColor: {},
    logoGradients: {},
    messageHistory: {},
    conversationId: {},
    isLoadingHistory: { type: Boolean },
    clientId: {},
    username: {},
    isOpen: { type: Boolean },
    onCreateNewConversation: { type: Function },
    chatInitError: {},
    isMobile: { type: Boolean }
  },
  emits: ["toggleUnreadMessage", "updateVisibleDate"],
  setup(e, { emit: t }) {
    const n = L("loading"), r = e, s = t, i = L(), o = L(), a = L(null);
    let l = null;
    const c = () => {
      l && l.disconnect(), l = new IntersectionObserver(
        (J) => {
          J.forEach((q) => {
            const ie = q.target.getAttribute("data-message-id");
            ie && (q.isIntersecting ? G.value.add(ie) : G.value.delete(ie));
          }), G.value = new Set(G.value);
        },
        {
          root: i.value,
          threshold: 0.1
          // Сообщение считается видимым если видно 10% от него
        }
      ), i.value && i.value.querySelectorAll("[data-message-id]").forEach((q) => {
        l?.observe(q);
      });
    }, { newMessages$: u, sendMessage: h, connected: g, disableReconnection: p } = Gp({
      conversationId: j(() => r.conversationId),
      onCreateNewConversation: r.onCreateNewConversation,
      onReconnected: () => {
        Ne(() => {
          Be();
        });
      }
    }), w = Gu(), m = j(() => r.chatInitError ? n.value !== "over" && n.value !== "thankYou" : !g.value && n.value !== "over" && n.value !== "thankYou"), Q = j(() => r.chatInitError ? "Чат недоступен. Попробуйте позже" : g.value ? "Подключено" : "Устанавливаем связь"), k = () => {
      try {
        const J = new Audio(Tp);
        J.volume = 0.5, J.play().catch((q) => {
          console.warn("[ChatContent] Failed to play notification sound:", q);
        });
      } catch (J) {
        console.warn("[ChatContent] Error creating audio:", J);
      }
    }, M = L([]), G = L(/* @__PURE__ */ new Set());
    Ue(
      [M, G],
      ([J, q]) => {
        if (J.length > 0 && q.size > 0) {
          const ie = J.filter((de) => q.has(de.message_id));
          if (ie.length > 0) {
            const de = ie.reduce(
              (ce, D) => ce.seq_no > D.seq_no ? ce : D
            );
            s("updateVisibleDate", de.created_at);
          }
        } else if (J.length > 0) {
          const ie = J[J.length - 1];
          s("updateVisibleDate", ie.created_at);
        }
      },
      { deep: !0 }
    ), Ue(
      () => g.value,
      (J, q) => {
        J && (n.value = "active", q === !1 && Ne(() => Be()));
      }
    ), Ft(() => {
      if (r.conversationId && u) {
        const J = r.messageHistory.map((de) => {
          let ce = null;
          return de.reply_to_message_id && (ce = r.messageHistory.find((D) => D.message_id === de.reply_to_message_id) || null), {
            ...de,
            replyingMessage: ce
          };
        });
        M.value = J;
        const ie = u.pipe(
          fn((de) => {
            if (de.type === "conversation.closed") {
              n.value = "over", p();
              return;
            }
            if (de.type === "message.created") {
              const ce = de.data;
              let D = null;
              if (ce.reply_to_message_id && (D = M.value.find((oe) => oe.message_id === ce.reply_to_message_id) || null), ce.sender === "user") {
                const oe = M.value.findIndex(
                  (lt) => lt.isReceived === !1 && lt.sender === "user" && lt.text === ce.text
                );
                if (oe !== -1) {
                  M.value[oe] = {
                    ...ce,
                    replyingMessage: D,
                    isReceived: !0
                  };
                  return;
                }
              }
              if (!M.value.some((oe) => oe.message_id === ce.message_id)) {
                const oe = {
                  ...ce,
                  replyingMessage: D
                };
                M.value.push(oe), r.isOpen || (s("toggleUnreadMessage"), (ce.sender === "support" || ce.sender === "system") && k()), Ne(() => Be());
              }
            }
          })
        ).subscribe();
        return () => {
          ie.unsubscribe();
        };
      }
    });
    const V = (J, q, ie) => {
      let de = null;
      q && (de = M.value.find((D) => D.message_id === q) || null);
      const ce = {
        message_id: `local-${Date.now()}-${Math.random().toString(36).substr(2, 9)}`,
        conversation_id: r.conversationId,
        seq_no: M.value.length,
        // временный seq_no
        sender: "user",
        source: "web",
        content_type: "text",
        attachments: ie || [],
        text: J,
        reply_to_message_id: q,
        replyingMessage: de,
        created_at: (/* @__PURE__ */ new Date()).toISOString(),
        isReceived: !1
        // флаг для отслеживания локальных сообщений
      };
      return M.value.push(ce), Ne(() => Be()), a.value && (a.value = null), ce;
    }, v = (J, q, ie) => {
      r.conversationId && (V(J, q, ie), h(
        J,
        Yp(),
        q,
        ie?.map((de) => de.attachment_id) || []
      ));
    }, C = L(null), E = L({ x: 0, y: 0 }), Y = (J, q) => {
      if (C.value = q, !i.value?.getBoundingClientRect()) return;
      const de = 180, ce = 140;
      let D = J.clientX, K = J.clientY;
      D + de > window.innerWidth && (D = window.innerWidth - de - 10), K + ce > window.innerHeight && (K = K - ce), D < 10 && (D = 10), K < 10 && (K = 10), E.value = { x: D, y: K };
    }, $ = () => {
      C.value = null;
    }, z = () => {
      C.value?.text && navigator.clipboard.writeText(C.value.text), $();
    }, he = () => {
      a.value = C.value, $(), Ne(() => {
        o.value?.focusInput();
      });
    }, at = async (J) => {
      try {
        await w.mutateAsync({
          conversation_id: r.conversationId,
          user_id: r.clientId || "",
          is_like: J
        }), n.value = "thankYou";
      } catch (q) {
        console.error("Failed to rate conversation:", q);
      }
    }, Ve = (J) => {
      const q = J.target;
      q.style.color = r.msgColor;
    }, pe = (J) => {
      const q = J.target;
      q.style.color = "white";
    }, Be = () => {
      i.value && (i.value.scrollTop = i.value.scrollHeight);
    };
    return tn(async () => {
      await Ne(), Be(), c();
    }), Ue(
      M,
      async () => {
        await Ne(), c();
      },
      { flush: "post" }
    ), Qn(() => {
      l && l.disconnect();
    }), (J, q) => (U(), T("div", {
      class: "w-full h-full flex flex-col min-h-0",
      onClick: $
    }, [
      m.value ? (U(), T("div", jp, [
        b("div", Pp, [
          b("div", Kp, Ae(Q.value), 1),
          !ee(g) && !J.chatInitError ? (U(), T("div", Np, [...q[1] || (q[1] = [
            b("div", {
              class: "w-[4.8px] h-[4.8px] bg-white rounded-[100px]",
              style: { animation: "dotFade1 1.5s infinite", opacity: "0" }
            }, null, -1),
            b("div", {
              class: "w-[6.4px] h-[6.4px] bg-white rounded-[100px]",
              style: { animation: "dotFade2 1.5s infinite", opacity: "0" }
            }, null, -1),
            b("div", {
              class: "w-[8px] h-[8px] bg-white rounded-[100px]",
              style: { animation: "dotFade3 1.5s infinite", opacity: "0" }
            }, null, -1)
          ])])) : ye("", !0)
        ])
      ])) : ye("", !0),
      b("div", {
        ref_key: "chatContainer",
        ref: i,
        id: "chat",
        class: "w-full flex-1 overflow-auto space-y-3 custom-scrollbar min-h-0",
        style: {
          "scrollbar-width": "thin",
          "scrollbar-color": "rgba(255, 255, 255, 0.2) transparent"
        }
      }, [
        (U(!0), T(ge, null, Vt(M.value, (ie) => (U(), dt(Rd, To({
          key: ie.seq_no,
          "data-message-id": ie.message_id
        }, { ref_for: !0 }, ie, {
          "is-mobile": J.isMobile,
          msgColor: r.msgColor,
          logoGradients: r.logoGradients,
          isMenuOpen: C.value?.message_id === ie.message_id,
          isRead: !0,
          onOpenMenu: (de) => Y(de, ie),
          username: J.username,
          onCloseMenu: $
        }), null, 16, ["data-message-id", "is-mobile", "msgColor", "logoGradients", "isMenuOpen", "onOpenMenu", "username"]))), 128))
      ], 512),
      b("div", Lp, [
        H(kh, {
          ref_key: "chatFooterRef",
          ref: o,
          accentColor: J.msgColor,
          sendMessage: v,
          canSendMessages: !!J.conversationId,
          replyingMessage: a.value,
          conversationId: J.conversationId,
          chatState: n.value,
          "is-mobile": J.isMobile,
          onSendRateConversation: at
        }, null, 8, ["accentColor", "canSendMessages", "replyingMessage", "conversationId", "chatState", "is-mobile"])
      ]),
      xs(b("div", {
        class: "fixed z-50 flex flex-col gap-3 bg-[#131525] p-4 rounded-[20px] shadow-lg min-w-[180px]",
        style: Te({ left: E.value.x + "px", top: E.value.y + "px" }),
        onClick: q[0] || (q[0] = En(() => {
        }, ["stop"]))
      }, [
        b("button", {
          onClick: he,
          class: "flex items-center gap-2 self-stretch text-white transition-colors",
          onMouseenter: Ve,
          onMouseleave: pe
        }, [
          H(ee(Ud), { class: "w-6 h-6" }),
          q[2] || (q[2] = b("span", { class: "font-medium text-sm text-center text-nowrap" }, "Ответить", -1))
        ], 32),
        C.value?.text?.length ? (U(), T("button", {
          key: 0,
          onClick: z,
          class: "flex items-center gap-2 self-stretch text-white transition-colors",
          onMouseenter: Ve,
          onMouseleave: pe
        }, [
          H(ee(Gd), { class: "w-6 h-6" }),
          q[3] || (q[3] = b("span", { class: "font-medium text-sm text-center text-nowrap" }, "Скопировать текст", -1))
        ], 32)) : ye("", !0)
      ], 4), [
        [Lo, C.value]
      ])
    ]));
  }
}), Jp = {
  xmlns: "http://www.w3.org/2000/svg",
  width: "6",
  height: "12",
  fill: "none"
};
function qp(e, t) {
  return U(), T("svg", Jp, [...t[0] || (t[0] = [
    b("path", {
      fill: "#F8FAFC",
      "fill-rule": "evenodd",
      d: "M5.468.414a.75.75 0 0 1 .118 1.055L1.96 6l3.626 4.531a.75.75 0 1 1-1.172.938l-4-5a.75.75 0 0 1 0-.937l4-5A.75.75 0 0 1 5.468.414",
      "clip-rule": "evenodd"
    }, null, -1)
  ])]);
}
const Zp = { render: qp }, Hp = {
  key: 0,
  class: "fixed inset-0 w-full h-full backdrop-blur-sm bg-black/20 z-20 pointer-events-auto"
}, zp = { class: "absolute inset-0 w-full h-full" }, Xp = { class: "relative z-10 w-full h-full flex flex-col gap-[12px]" }, _p = /* @__PURE__ */ Me({
  __name: "OgLiveChat.ce",
  props: {
    position: { default: "bottom-right", type: String },
    sizeClass: { default: "w-[342px] h-[600px]", type: String },
    theme: { default: "mos", type: String },
    userId: { default: void 0, type: String },
    userName: { default: void 0, type: String },
    topic: { default: void 0, type: String },
    apiBaseUrl: { default: void 0, type: String },
    wsBaseUrl: { default: void 0, type: String }
  },
  setup(e) {
    const t = L(!1), n = L(!1), r = L(!1), s = e;
    Ue(
      () => [s.apiBaseUrl, s.wsBaseUrl],
      ([pe, Be]) => {
        Bu(pe, Be);
      },
      { immediate: !0 }
    );
    const {
      conversationId: i,
      messageHistory: o,
      isLoadingHistory: a,
      createNewConversation: l,
      error: c
    } = Yu({
      userId: j(() => s.userId || ""),
      userName: j(() => s.userName || ""),
      topic: j(() => s.topic || ""),
      needToInitializeChat: j(() => n.value)
      // Инициализируем только когда открыт чат
    }), u = L(/* @__PURE__ */ new Date()), h = (pe) => {
      u.value = pe;
    }, g = (pe) => {
      if (!t.value) return;
      (typeof pe.composedPath == "function" ? pe.composedPath() : []).some(
        (q) => q instanceof HTMLElement && q.tagName === "OG-CHAT"
      ) || (t.value = !1);
    };
    Ue(t, (pe) => {
      pe && !n.value && (n.value = !0), t.value && (r.value = !1);
    });
    const p = () => {
      t.value || (r.value = !0);
    }, { shouldUseFullscreen: w } = Su(), m = j(() => w.value && t.value), Q = j(() => m.value ? [
      "fixed inset-0 z-40",
      "flex flex-col justify-end items-center",
      // Прижимаем к низу
      "w-full h-full"
    ].join(" ") : t.value ? [v.value, "fixed flex flex-col justify-end"].join(" ") : ["fixed pointer-events-none"].join(" ")), k = j(() => m.value ? [
      Y.value,
      "relative overflow-hidden",
      "w-full",
      "z-30",
      // Чат поверх подложки (такой же как кнопка)
      "rounded-t-[32px]"
    ].join(" ") : Y.value + " relative overflow-hidden rounded-[32px]"), M = j(() => m.value ? {
      height: "calc(var(--app-vh, 100vh) * 0.8)",
      maxHeight: "calc(var(--app-vh, 100vh) * 0.8)"
    } : {}), G = j(() => m.value ? {
      bottom: "calc(var(--app-vh, 100vh) * 0.8 + 62px)",
      // Высота чата + отступ
      left: "16px"
    } : {});
    function V() {
      const pe = window.visualViewport ? window.visualViewport.height : window.innerHeight;
      document.documentElement.style.setProperty("--app-vh", `${pe}px`);
    }
    tn(() => {
      V(), window.visualViewport?.addEventListener("resize", V), window.addEventListener("resize", V), document.addEventListener("pointerdown", g, { capture: !0 }), document.addEventListener("touchstart", g, { capture: !0 });
    }), Ol(() => {
      window.visualViewport?.removeEventListener("resize", V), window.removeEventListener("resize", V), document.documentElement.style.removeProperty("--app-vh"), document.removeEventListener("pointerdown", g, { capture: !0 }), document.removeEventListener("touchstart", g, { capture: !0 });
    }), Ue(m, (pe) => {
      pe ? document.body.style.overflow = "hidden" : document.body.style.overflow = "";
    });
    const {
      sizeClasses: v,
      buttonClasses: C,
      buttonContainerClasses: E,
      chatWindowClasses: Y,
      gradients: $,
      mainAccent: z,
      logoGradients: he,
      buttonBg: at
    } = Eu(s), Ve = () => {
      t.value = !t.value;
    };
    return (pe, Be) => (U(), T("div", {
      id: "chatContainer",
      class: me(Q.value)
    }, [
      ee(w) && t.value ? (U(), T("div", Hp, [
        b("div", {
          class: "absolute z-40",
          style: Te(G.value)
        }, [
          b("button", {
            class: "h-[36px] w-[83px] flex items-center px-1 py-2 rounded-xl backdrop-blur-[25px]",
            style: { background: "rgba(108, 111, 172, 0.08)" },
            onClick: Ve
          }, [
            H(ee(Zp), { class: "mx-[10px]" }),
            Be[0] || (Be[0] = b("span", { class: "font-medium text-sm text-center text-neutral-50" }, "Назад", -1))
          ])
        ], 4)
      ])) : ye("", !0),
      n.value ? xs((U(), T("div", {
        key: 1,
        class: me([k.value, "relative pointer-events-auto"]),
        style: Te(M.value)
      }, [
        b("div", zp, [
          H(ef, {
            gradients: ee($),
            class: "w-full h-full"
          }, null, 8, ["gradients"])
        ]),
        b("div", Xp, [
          H(sf, { "visible-message-date": u.value }, null, 8, ["visible-message-date"]),
          H(Wp, {
            "msg-color": ee(z),
            "logo-gradients": ee(he),
            "message-history": ee(o),
            "conversation-id": ee(i),
            "is-loading-history": ee(a),
            "client-id": pe.userId,
            username: pe.userName,
            "on-create-new-conversation": ee(l),
            "chat-init-error": ee(c),
            isOpen: t.value,
            "is-mobile": ee(w),
            onToggleUnreadMessage: p,
            onUpdateVisibleDate: h
          }, null, 8, ["msg-color", "logo-gradients", "message-history", "conversation-id", "is-loading-history", "client-id", "username", "on-create-new-conversation", "chat-init-error", "isOpen", "is-mobile"])
        ])
      ], 6)), [
        [Lo, t.value]
      ]) : ye("", !0),
      t.value && ee(w) ? ye("", !0) : (U(), T("div", {
        key: 2,
        class: me([ee(E), "pointer-events-auto"])
      }, [
        b("button", {
          onClick: Ve,
          class: me([
            ee(C),
            "flex mt-[16px] items-center justify-center active:scale-95 relative"
          ]),
          style: Te({
            outline: "none",
            background: ee(at)
          }),
          "aria-label": "Открыть чат"
        }, [
          H(ee(Pu)),
          r.value ? (U(), T("div", {
            key: 0,
            class: "absolute top-0 right-0 w-2.5 h-2.5 rounded-[100px] outline-4 outline-t-4 outline-l-4 outline-r-4 outline-b-4 outline outline-[#0b0b0c]",
            style: Te({ backgroundColor: ee(z) })
          }, null, 4)) : ye("", !0)
        ], 6)
      ], 2))
    ], 2));
  }
}), $p = '*,:before,:after{--tw-border-spacing-x: 0;--tw-border-spacing-y: 0;--tw-translate-x: 0;--tw-translate-y: 0;--tw-rotate: 0;--tw-skew-x: 0;--tw-skew-y: 0;--tw-scale-x: 1;--tw-scale-y: 1;--tw-pan-x: ;--tw-pan-y: ;--tw-pinch-zoom: ;--tw-scroll-snap-strictness: proximity;--tw-gradient-from-position: ;--tw-gradient-via-position: ;--tw-gradient-to-position: ;--tw-ordinal: ;--tw-slashed-zero: ;--tw-numeric-figure: ;--tw-numeric-spacing: ;--tw-numeric-fraction: ;--tw-ring-inset: ;--tw-ring-offset-width: 0px;--tw-ring-offset-color: #fff;--tw-ring-color: rgb(59 130 246 / .5);--tw-ring-offset-shadow: 0 0 #0000;--tw-ring-shadow: 0 0 #0000;--tw-shadow: 0 0 #0000;--tw-shadow-colored: 0 0 #0000;--tw-blur: ;--tw-brightness: ;--tw-contrast: ;--tw-grayscale: ;--tw-hue-rotate: ;--tw-invert: ;--tw-saturate: ;--tw-sepia: ;--tw-drop-shadow: ;--tw-backdrop-blur: ;--tw-backdrop-brightness: ;--tw-backdrop-contrast: ;--tw-backdrop-grayscale: ;--tw-backdrop-hue-rotate: ;--tw-backdrop-invert: ;--tw-backdrop-opacity: ;--tw-backdrop-saturate: ;--tw-backdrop-sepia: ;--tw-contain-size: ;--tw-contain-layout: ;--tw-contain-paint: ;--tw-contain-style: }::backdrop{--tw-border-spacing-x: 0;--tw-border-spacing-y: 0;--tw-translate-x: 0;--tw-translate-y: 0;--tw-rotate: 0;--tw-skew-x: 0;--tw-skew-y: 0;--tw-scale-x: 1;--tw-scale-y: 1;--tw-pan-x: ;--tw-pan-y: ;--tw-pinch-zoom: ;--tw-scroll-snap-strictness: proximity;--tw-gradient-from-position: ;--tw-gradient-via-position: ;--tw-gradient-to-position: ;--tw-ordinal: ;--tw-slashed-zero: ;--tw-numeric-figure: ;--tw-numeric-spacing: ;--tw-numeric-fraction: ;--tw-ring-inset: ;--tw-ring-offset-width: 0px;--tw-ring-offset-color: #fff;--tw-ring-color: rgb(59 130 246 / .5);--tw-ring-offset-shadow: 0 0 #0000;--tw-ring-shadow: 0 0 #0000;--tw-shadow: 0 0 #0000;--tw-shadow-colored: 0 0 #0000;--tw-blur: ;--tw-brightness: ;--tw-contrast: ;--tw-grayscale: ;--tw-hue-rotate: ;--tw-invert: ;--tw-saturate: ;--tw-sepia: ;--tw-drop-shadow: ;--tw-backdrop-blur: ;--tw-backdrop-brightness: ;--tw-backdrop-contrast: ;--tw-backdrop-grayscale: ;--tw-backdrop-hue-rotate: ;--tw-backdrop-invert: ;--tw-backdrop-opacity: ;--tw-backdrop-saturate: ;--tw-backdrop-sepia: ;--tw-contain-size: ;--tw-contain-layout: ;--tw-contain-paint: ;--tw-contain-style: }*,:before,:after{box-sizing:border-box;border-width:0;border-style:solid;border-color:#e5e7eb}:before,:after{--tw-content: ""}html,:host{line-height:1.5;-webkit-text-size-adjust:100%;-moz-tab-size:4;-o-tab-size:4;tab-size:4;font-family:ui-sans-serif,system-ui,sans-serif,"Apple Color Emoji","Segoe UI Emoji",Segoe UI Symbol,"Noto Color Emoji";font-feature-settings:normal;font-variation-settings:normal;-webkit-tap-highlight-color:transparent}body{margin:0;line-height:inherit}hr{height:0;color:inherit;border-top-width:1px}abbr:where([title]){-webkit-text-decoration:underline dotted;text-decoration:underline dotted}h1,h2,h3,h4,h5,h6{font-size:inherit;font-weight:inherit}a{color:inherit;text-decoration:inherit}b,strong{font-weight:bolder}code,kbd,samp,pre{font-family:ui-monospace,SFMono-Regular,Menlo,Monaco,Consolas,Liberation Mono,Courier New,monospace;font-feature-settings:normal;font-variation-settings:normal;font-size:1em}small{font-size:80%}sub,sup{font-size:75%;line-height:0;position:relative;vertical-align:baseline}sub{bottom:-.25em}sup{top:-.5em}table{text-indent:0;border-color:inherit;border-collapse:collapse}button,input,optgroup,select,textarea{font-family:inherit;font-feature-settings:inherit;font-variation-settings:inherit;font-size:100%;font-weight:inherit;line-height:inherit;letter-spacing:inherit;color:inherit;margin:0;padding:0}button,select{text-transform:none}button,input:where([type=button]),input:where([type=reset]),input:where([type=submit]){-webkit-appearance:button;background-color:transparent;background-image:none}:-moz-focusring{outline:auto}:-moz-ui-invalid{box-shadow:none}progress{vertical-align:baseline}::-webkit-inner-spin-button,::-webkit-outer-spin-button{height:auto}[type=search]{-webkit-appearance:textfield;outline-offset:-2px}::-webkit-search-decoration{-webkit-appearance:none}::-webkit-file-upload-button{-webkit-appearance:button;font:inherit}summary{display:list-item}blockquote,dl,dd,h1,h2,h3,h4,h5,h6,hr,figure,p,pre{margin:0}fieldset{margin:0;padding:0}legend{padding:0}ol,ul,menu{list-style:none;margin:0;padding:0}dialog{padding:0}textarea{resize:vertical}input::-moz-placeholder,textarea::-moz-placeholder{opacity:1;color:#9ca3af}input::placeholder,textarea::placeholder{opacity:1;color:#9ca3af}button,[role=button]{cursor:pointer}:disabled{cursor:default}img,svg,video,canvas,audio,iframe,embed,object{display:block;vertical-align:middle}img,video{max-width:100%;height:auto}[hidden]:where(:not([hidden=until-found])){display:none}.container{width:100%}@media (min-width: 640px){.container{max-width:640px}}@media (min-width: 768px){.container{max-width:768px}}@media (min-width: 1024px){.container{max-width:1024px}}@media (min-width: 1280px){.container{max-width:1280px}}@media (min-width: 1536px){.container{max-width:1536px}}.pointer-events-none{pointer-events:none}.pointer-events-auto{pointer-events:auto}.visible{visibility:visible}.fixed{position:fixed}.absolute{position:absolute}.relative{position:relative}.inset-0{inset:0}.bottom-\\[150px\\]{bottom:150px}.bottom-\\[8px\\]{bottom:8px}.left-\\[8px\\]{left:8px}.right-0{right:0}.right-\\[120px\\]{right:120px}.right-\\[8px\\]{right:8px}.top-0{top:0}.top-\\[8px\\]{top:8px}.z-10{z-index:10}.z-20{z-index:20}.z-30{z-index:30}.z-40{z-index:40}.z-50{z-index:50}.col-span-2{grid-column:span 2 / span 2}.row-span-2{grid-row:span 2 / span 2}.-m-4{margin:-1rem}.mx-\\[10px\\]{margin-left:10px;margin-right:10px}.mb-0\\.5{margin-bottom:.125rem}.mb-1{margin-bottom:.25rem}.mb-\\[12px\\]{margin-bottom:12px}.mb-\\[16px\\]{margin-bottom:16px}.mb-\\[24px\\]{margin-bottom:24px}.mr-\\[6px\\]{margin-right:6px}.mt-\\[10px\\]{margin-top:10px}.mt-\\[12px\\]{margin-top:12px}.mt-\\[16px\\]{margin-top:16px}.mt-\\[9px\\]{margin-top:9px}.inline-block{display:inline-block}.inline{display:inline}.flex{display:flex}.grid{display:grid}.hidden{display:none}.aspect-\\[106\\/103\\]{aspect-ratio:106/103}.aspect-\\[106\\/137\\]{aspect-ratio:106/137}.aspect-\\[106\\/208\\]{aspect-ratio:106/208}.aspect-\\[162\\/240\\]{aspect-ratio:162/240}.aspect-\\[162\\/80\\]{aspect-ratio:162/80}.aspect-\\[214\\/208\\]{aspect-ratio:214/208}.aspect-\\[3\\/4\\]{aspect-ratio:3/4}.aspect-square{aspect-ratio:1 / 1}.h-2\\.5{height:.625rem}.h-6{height:1.5rem}.h-\\[108px\\]{height:108px}.h-\\[18px\\]{height:18px}.h-\\[23\\.999998092651367px\\]{height:23.999998092651367px}.h-\\[24px\\]{height:24px}.h-\\[36px\\]{height:36px}.h-\\[4\\.8px\\]{height:4.8px}.h-\\[40px\\]{height:40px}.h-\\[44px\\]{height:44px}.h-\\[52px\\]{height:52px}.h-\\[536px\\]{height:536px}.h-\\[6\\.4px\\]{height:6.4px}.h-\\[600px\\]{height:600px}.h-\\[8px\\]{height:8px}.h-full{height:100%}.max-h-\\[120px\\]{max-height:120px}.min-h-0{min-height:0px}.min-h-\\[103px\\]{min-height:103px}.min-h-\\[137px\\]{min-height:137px}.min-h-\\[208px\\]{min-height:208px}.min-h-\\[243px\\]{min-height:243px}.min-h-\\[24px\\]{min-height:24px}.min-h-\\[44px\\]{min-height:44px}.min-h-\\[80px\\]{min-height:80px}.w-2\\.5{width:.625rem}.w-6{width:1.5rem}.w-\\[202px\\]{width:202px}.w-\\[225px\\]{width:225px}.w-\\[23\\.999998092651367px\\]{width:23.999998092651367px}.w-\\[24px\\]{width:24px}.w-\\[342px\\]{width:342px}.w-\\[36px\\]{width:36px}.w-\\[4\\.8px\\]{width:4.8px}.w-\\[40px\\]{width:40px}.w-\\[44px\\]{width:44px}.w-\\[52px\\]{width:52px}.w-\\[6\\.4px\\]{width:6.4px}.w-\\[83px\\]{width:83px}.w-\\[8px\\]{width:8px}.w-full{width:100%}.min-w-0{min-width:0px}.min-w-\\[106px\\]{min-width:106px}.min-w-\\[120px\\]{min-width:120px}.min-w-\\[162px\\]{min-width:162px}.min-w-\\[180px\\]{min-width:180px}.min-w-\\[183px\\]{min-width:183px}.min-w-\\[214px\\]{min-width:214px}.min-w-\\[80px\\]{min-width:80px}.max-w-\\[85\\%\\]{max-width:85%}.max-w-sm{max-width:24rem}.flex-1{flex:1 1 0%}.flex-shrink-0{flex-shrink:0}.grow{flex-grow:1}.cursor-default{cursor:default}.cursor-not-allowed{cursor:not-allowed}.cursor-pointer{cursor:pointer}.resize-none{resize:none}.resize{resize:both}.grid-cols-2{grid-template-columns:repeat(2,minmax(0,1fr))}.grid-cols-\\[1fr_auto\\]{grid-template-columns:1fr auto}.grid-rows-2{grid-template-rows:repeat(2,minmax(0,1fr))}.grid-rows-3{grid-template-rows:repeat(3,minmax(0,1fr))}.flex-col{flex-direction:column}.flex-wrap{flex-wrap:wrap}.items-end{align-items:flex-end}.items-center{align-items:center}.justify-start{justify-content:flex-start}.justify-end{justify-content:flex-end}.justify-center{justify-content:center}.gap-0\\.5{gap:.125rem}.gap-2{gap:.5rem}.gap-2\\.5{gap:.625rem}.gap-3{gap:.75rem}.gap-\\[10px\\]{gap:10px}.gap-\\[12px\\]{gap:12px}.gap-\\[15px\\]{gap:15px}.gap-\\[3px\\]{gap:3px}.gap-\\[4px\\]{gap:4px}.gap-\\[6px\\]{gap:6px}.gap-\\[8px\\]{gap:8px}.space-y-3>:not([hidden])~:not([hidden]){--tw-space-y-reverse: 0;margin-top:calc(.75rem * calc(1 - var(--tw-space-y-reverse)));margin-bottom:calc(.75rem * var(--tw-space-y-reverse))}.self-stretch{align-self:stretch}.overflow-auto{overflow:auto}.overflow-hidden{overflow:hidden}.overflow-y-auto{overflow-y:auto}.truncate{overflow:hidden;text-overflow:ellipsis;white-space:nowrap}.whitespace-nowrap{white-space:nowrap}.whitespace-pre-wrap{white-space:pre-wrap}.text-nowrap{text-wrap:nowrap}.break-words{overflow-wrap:break-word}.rounded{border-radius:.25rem}.rounded-\\[100px\\]{border-radius:100px}.rounded-\\[12px\\]{border-radius:12px}.rounded-\\[20px\\]{border-radius:20px}.rounded-\\[24px\\]{border-radius:24px}.rounded-\\[2px\\]{border-radius:2px}.rounded-\\[32px\\]{border-radius:32px}.rounded-\\[8px\\]{border-radius:8px}.rounded-full{border-radius:9999px}.rounded-lg{border-radius:.5rem}.rounded-xl{border-radius:.75rem}.rounded-t-\\[20px\\]{border-top-left-radius:20px;border-top-right-radius:20px}.rounded-t-\\[32px\\]{border-top-left-radius:32px;border-top-right-radius:32px}.rounded-bl-\\[20px\\]{border-bottom-left-radius:20px}.rounded-br-\\[20px\\]{border-bottom-right-radius:20px}.rounded-tl-\\[20px\\]{border-top-left-radius:20px}.rounded-tr-\\[20px\\]{border-top-right-radius:20px}.border{border-width:1px}.border-none{border-style:none}.border-gray-200{--tw-border-opacity: 1;border-color:rgb(229 231 235 / var(--tw-border-opacity, 1))}.bg-\\[\\#0e0f19\\]{--tw-bg-opacity: 1;background-color:rgb(14 15 25 / var(--tw-bg-opacity, 1))}.bg-\\[\\#131525\\]{--tw-bg-opacity: 1;background-color:rgb(19 21 37 / var(--tw-bg-opacity, 1))}.bg-black\\/20{background-color:#0003}.bg-black\\/60{background-color:#0009}.bg-gray-200{--tw-bg-opacity: 1;background-color:rgb(229 231 235 / var(--tw-bg-opacity, 1))}.bg-gray-50{--tw-bg-opacity: 1;background-color:rgb(249 250 251 / var(--tw-bg-opacity, 1))}.bg-transparent{background-color:transparent}.bg-white{--tw-bg-opacity: 1;background-color:rgb(255 255 255 / var(--tw-bg-opacity, 1))}.object-cover{-o-object-fit:cover;object-fit:cover}.p-2{padding:.5rem}.p-4{padding:1rem}.p-\\[12px\\]{padding:12px}.p-\\[4px\\]{padding:4px}.px-1{padding-left:.25rem;padding-right:.25rem}.px-2{padding-left:.5rem;padding-right:.5rem}.px-3{padding-left:.75rem;padding-right:.75rem}.px-\\[12px\\]{padding-left:12px;padding-right:12px}.px-\\[14px\\]{padding-left:14px;padding-right:14px}.py-1{padding-top:.25rem;padding-bottom:.25rem}.py-2{padding-top:.5rem;padding-bottom:.5rem}.py-\\[10px\\]{padding-top:10px;padding-bottom:10px}.py-\\[12px\\]{padding-top:12px;padding-bottom:12px}.py-\\[4px\\]{padding-top:4px;padding-bottom:4px}.py-\\[8px\\]{padding-top:8px;padding-bottom:8px}.pb-0\\.5{padding-bottom:.125rem}.pl-\\[10px\\]{padding-left:10px}.pr-\\[4px\\]{padding-right:4px}.text-left{text-align:left}.text-center{text-align:center}.text-start{text-align:start}.font-golos{font-family:Golos Text,Inter,Segoe UI,Roboto,Helvetica Neue,Arial,sans-serif}.font-unbounded{font-family:Unbounded,Outfit,Inter,Segoe UI,Roboto,Helvetica Neue,Arial,sans-serif}.text-2xl{font-size:1.5rem;line-height:2rem}.text-\\[10px\\]{font-size:10px}.text-\\[12px\\]{font-size:12px}.text-\\[16px\\]{font-size:16px}.text-\\[24px\\]{font-size:24px}.text-sm{font-size:.875rem;line-height:1.25rem}.text-xs{font-size:.75rem;line-height:1rem}.font-medium{font-weight:500}.font-normal{font-weight:400}.font-semibold{font-weight:600}.leading-3{line-height:.75rem}.leading-6{line-height:1.5rem}.leading-\\[10px\\]{line-height:10px}.leading-\\[20px\\]{line-height:20px}.leading-\\[28px\\]{line-height:28px}.leading-tight{line-height:1.25}.text-\\[\\#64748B\\],.text-\\[\\#64748b\\]{--tw-text-opacity: 1;color:rgb(100 116 139 / var(--tw-text-opacity, 1))}.text-gray-600{--tw-text-opacity: 1;color:rgb(75 85 99 / var(--tw-text-opacity, 1))}.text-gray-800{--tw-text-opacity: 1;color:rgb(31 41 55 / var(--tw-text-opacity, 1))}.text-neutral-50{--tw-text-opacity: 1;color:rgb(250 250 250 / var(--tw-text-opacity, 1))}.text-slate-500{--tw-text-opacity: 1;color:rgb(100 116 139 / var(--tw-text-opacity, 1))}.text-white{--tw-text-opacity: 1;color:rgb(255 255 255 / var(--tw-text-opacity, 1))}.placeholder-\\[\\#64748b\\]::-moz-placeholder{--tw-placeholder-opacity: 1;color:rgb(100 116 139 / var(--tw-placeholder-opacity, 1))}.placeholder-\\[\\#64748b\\]::placeholder{--tw-placeholder-opacity: 1;color:rgb(100 116 139 / var(--tw-placeholder-opacity, 1))}.opacity-0{opacity:0}.opacity-50{opacity:.5}.shadow-lg{--tw-shadow: 0 10px 15px -3px rgb(0 0 0 / .1), 0 4px 6px -4px rgb(0 0 0 / .1);--tw-shadow-colored: 0 10px 15px -3px var(--tw-shadow-color), 0 4px 6px -4px var(--tw-shadow-color);box-shadow:var(--tw-ring-offset-shadow, 0 0 #0000),var(--tw-ring-shadow, 0 0 #0000),var(--tw-shadow)}.outline-none{outline:2px solid transparent;outline-offset:2px}.outline{outline-style:solid}.outline-4{outline-width:4px}.outline-\\[\\#0b0b0c\\]{outline-color:#0b0b0c}.filter{filter:var(--tw-blur) var(--tw-brightness) var(--tw-contrast) var(--tw-grayscale) var(--tw-hue-rotate) var(--tw-invert) var(--tw-saturate) var(--tw-sepia) var(--tw-drop-shadow)}.backdrop-blur{--tw-backdrop-blur: blur(8px);-webkit-backdrop-filter:var(--tw-backdrop-blur) var(--tw-backdrop-brightness) var(--tw-backdrop-contrast) var(--tw-backdrop-grayscale) var(--tw-backdrop-hue-rotate) var(--tw-backdrop-invert) var(--tw-backdrop-opacity) var(--tw-backdrop-saturate) var(--tw-backdrop-sepia);backdrop-filter:var(--tw-backdrop-blur) var(--tw-backdrop-brightness) var(--tw-backdrop-contrast) var(--tw-backdrop-grayscale) var(--tw-backdrop-hue-rotate) var(--tw-backdrop-invert) var(--tw-backdrop-opacity) var(--tw-backdrop-saturate) var(--tw-backdrop-sepia)}.backdrop-blur-\\[20px\\]{--tw-backdrop-blur: blur(20px);-webkit-backdrop-filter:var(--tw-backdrop-blur) var(--tw-backdrop-brightness) var(--tw-backdrop-contrast) var(--tw-backdrop-grayscale) var(--tw-backdrop-hue-rotate) var(--tw-backdrop-invert) var(--tw-backdrop-opacity) var(--tw-backdrop-saturate) var(--tw-backdrop-sepia);backdrop-filter:var(--tw-backdrop-blur) var(--tw-backdrop-brightness) var(--tw-backdrop-contrast) var(--tw-backdrop-grayscale) var(--tw-backdrop-hue-rotate) var(--tw-backdrop-invert) var(--tw-backdrop-opacity) var(--tw-backdrop-saturate) var(--tw-backdrop-sepia)}.backdrop-blur-\\[25px\\]{--tw-backdrop-blur: blur(25px);-webkit-backdrop-filter:var(--tw-backdrop-blur) var(--tw-backdrop-brightness) var(--tw-backdrop-contrast) var(--tw-backdrop-grayscale) var(--tw-backdrop-hue-rotate) var(--tw-backdrop-invert) var(--tw-backdrop-opacity) var(--tw-backdrop-saturate) var(--tw-backdrop-sepia);backdrop-filter:var(--tw-backdrop-blur) var(--tw-backdrop-brightness) var(--tw-backdrop-contrast) var(--tw-backdrop-grayscale) var(--tw-backdrop-hue-rotate) var(--tw-backdrop-invert) var(--tw-backdrop-opacity) var(--tw-backdrop-saturate) var(--tw-backdrop-sepia)}.backdrop-blur-sm{--tw-backdrop-blur: blur(4px);-webkit-backdrop-filter:var(--tw-backdrop-blur) var(--tw-backdrop-brightness) var(--tw-backdrop-contrast) var(--tw-backdrop-grayscale) var(--tw-backdrop-hue-rotate) var(--tw-backdrop-invert) var(--tw-backdrop-opacity) var(--tw-backdrop-saturate) var(--tw-backdrop-sepia);backdrop-filter:var(--tw-backdrop-blur) var(--tw-backdrop-brightness) var(--tw-backdrop-contrast) var(--tw-backdrop-grayscale) var(--tw-backdrop-hue-rotate) var(--tw-backdrop-invert) var(--tw-backdrop-opacity) var(--tw-backdrop-saturate) var(--tw-backdrop-sepia)}.transition-colors{transition-property:color,background-color,border-color,text-decoration-color,fill,stroke;transition-timing-function:cubic-bezier(.4,0,.2,1);transition-duration:.15s}.transition-opacity{transition-property:opacity;transition-timing-function:cubic-bezier(.4,0,.2,1);transition-duration:.15s}.transition-transform{transition-property:transform;transition-timing-function:cubic-bezier(.4,0,.2,1);transition-duration:.15s}.duration-200{transition-duration:.2s}@keyframes dotFade1{0%,20%{opacity:1}20%,to{opacity:0}}@keyframes dotFade2{0%,33%{opacity:0}33%,53%{opacity:1}53%,to{opacity:0}}@keyframes dotFade3{0%,66%{opacity:0}66%,86%{opacity:1}86%,to{opacity:0}}.hover\\:scale-105:hover{--tw-scale-x: 1.05;--tw-scale-y: 1.05;transform:translate(var(--tw-translate-x),var(--tw-translate-y)) rotate(var(--tw-rotate)) skew(var(--tw-skew-x)) skewY(var(--tw-skew-y)) scaleX(var(--tw-scale-x)) scaleY(var(--tw-scale-y))}.hover\\:text-neutral-50:hover{--tw-text-opacity: 1;color:rgb(250 250 250 / var(--tw-text-opacity, 1))}.focus\\:border-none:focus{border-style:none}.focus\\:shadow-none:focus{--tw-shadow: 0 0 #0000;--tw-shadow-colored: 0 0 #0000;box-shadow:var(--tw-ring-offset-shadow, 0 0 #0000),var(--tw-ring-shadow, 0 0 #0000),var(--tw-shadow)}.focus\\:outline-none:focus{outline:2px solid transparent;outline-offset:2px}.focus\\:ring-0:focus{--tw-ring-offset-shadow: var(--tw-ring-inset) 0 0 0 var(--tw-ring-offset-width) var(--tw-ring-offset-color);--tw-ring-shadow: var(--tw-ring-inset) 0 0 0 calc(0px + var(--tw-ring-offset-width)) var(--tw-ring-color);box-shadow:var(--tw-ring-offset-shadow),var(--tw-ring-shadow),var(--tw-shadow, 0 0 #0000)}.focus-visible\\:outline-none:focus-visible{outline:2px solid transparent;outline-offset:2px}.active\\:scale-95:active{--tw-scale-x: .95;--tw-scale-y: .95;transform:translate(var(--tw-translate-x),var(--tw-translate-y)) rotate(var(--tw-rotate)) skew(var(--tw-skew-x)) skewY(var(--tw-skew-y)) scaleX(var(--tw-scale-x)) scaleY(var(--tw-scale-y))}.disabled\\:cursor-not-allowed:disabled{cursor:not-allowed}.group:hover .group-hover\\:opacity-100{opacity:1}', eg = ":host{font-family:Golos Text,Inter,Segoe UI,Roboto,Helvetica Neue,Arial,sans-serif}.font-golos{font-family:Golos Text,Inter,Segoe UI,Roboto,Helvetica Neue,Arial,sans-serif}.font-outfit{font-family:Outfit,Inter,Segoe UI,Roboto,Helvetica Neue,Arial,sans-serif}.font-unbounded{font-family:Unbounded,Outfit,Inter,Segoe UI,Roboto,Helvetica Neue,Arial,sans-serif}#chatContainer,og-chat{font-family:Golos Text,Inter,Segoe UI,Roboto,Helvetica Neue,Arial,sans-serif}.chat-content,.chat-message,.chat-input,.chat-button{font-family:Golos Text,Inter,Segoe UI,Roboto,Helvetica Neue,Arial,sans-serif}*{font-family:inherit}";
function tg() {
  if (document.querySelector('link[href*="fonts.googleapis.com"][href*="Golos+Text"]'))
    return;
  const t = document.createElement("link");
  t.rel = "preconnect", t.href = "https://fonts.googleapis.com", document.head.appendChild(t);
  const n = document.createElement("link");
  n.rel = "preconnect", n.href = "https://fonts.gstatic.com", n.crossOrigin = "anonymous", document.head.appendChild(n);
  const r = document.createElement("link");
  r.rel = "stylesheet", r.href = "https://fonts.googleapis.com/css2?family=Golos+Text:wght@400..900&family=Outfit:wght@100..900&family=Unbounded:wght@200..900&display=swap", document.head.appendChild(r);
}
const ng = new ra({
  defaultOptions: {
    queries: {
      retry: 2,
      staleTime: 15e3,
      refetchOnWindowFocus: !1,
      gcTime: 3e4
    },
    mutations: {
      retry: 1
    }
  }
}), rg = /* @__PURE__ */ jc(_p, {
  styles: [eg, $p],
  configureApp(e) {
    e.use(xu, { queryClient: ng });
  }
});
tg();
customElements.define("og-chat", rg);
export {
  rg as default
};
