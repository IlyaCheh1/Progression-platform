...
describe("directions slide layout", () => {
  ...
  it("keeps slide copy below the header and does not clip titles", () => {
    ...
    assert.doesNotMatch(tsx, /school-slide-facts/);
    assert.doesNotMatch(tsx, /school-slide-side/);
    assert.doesNotMatch(tsx, /uppercase tracking-\[0\.08em\] text-white">Направления/);
    assert.doesNotMatch(tsx, /uppercase tracking-\[0\.08em\] text-white">Арсенал/);
    assert.match(tsx, /flex flex-1 flex-col items-center justify-center/);
    assert.match(tsx, /school-slide-band w-full items-start/);
    assert.match(css, /#directions \.school-slide-band \{\s*display:\s*flex;/);
    assert.doesNotMatch(css, /grid-template-columns:\s*minmax\(0,\s*1fr\) auto minmax\(0,\s*1fr\)/);
    assert.doesNotMatch(css, /school-slide-side/);
    assert.match(css, /#directions \.school-slide-band \{\s*display:\s*flex;[\s\S]*min-height:\s*16\.6875rem;/);
